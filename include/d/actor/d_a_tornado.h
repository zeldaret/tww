...
public:
    float getJointXPos(int jno) const { if (mpModel) return mpModel->getAnmMtx(jno)[0][3]; else return current.pos.x; }
    float getJointYPos(int jno) const { if (mpModel) return mpModel->getAnmMtx(jno)[1][3]; else return current.pos.y; }
    float getJointZPos(int jno) const { if (mpModel) return mpModel->getAnmMtx(jno)[2][3]; else return current.pos.z; }
    void getScaleEnd() {}
