...
static BOOL land_area_check(cXyz* param_1, float param_2) {
...
static void eff_hane_set(bdk_class* i_this, cXyz* offset, int param_3, signed char param_4) {
...
static void anm_init(bdk_class* i_this, int bckFileIdx, f32 morf, u8 loopMode, f32 speed, int soundFileIdx, unsigned char param7) {
    if (param7 == 0) {
    ...
    if (soundFileIdx >= 0) {
        i_this->mpMorf->setAnm(
            (J3DAnmTransform*)dComIfG_getObjectRes("Bdk", bckFileIdx), loopMode, morf, speed, 0.0f, -1.0f, dComIfG_getObjectRes("Bdk", soundFileIdx)
        );
    } else {
        i_this->mpMorf->setAnm((J3DAnmTransform*)dComIfG_getObjectRes("Bdk", bckFileIdx), loopMode, morf, speed, 0.0f, -1.0f, NULL);
    }
}
...
static void tail_control(bdk_class* i_this, bdk_tail_s* tail) {
    cXyz sp10;
    cXyz sp28;
    cXyz sp1C;
...
static void tail_draw(bdk_class* i_this, bdk_tail_s* tail) {
    fopAc_ac_c* actor = &i_this->actor;
    if (i_this->m2F6 != 0) {
        i_this->m2F6--;
    } else {
...
static void kamen_break_draw(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    J3DModel* model;
...
static void obj_draw(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
...
static void eff_hane_draw(bdk_class* i_this, bdk_eff_s* eff) {
    fopAc_ac_c* actor = &i_this->actor;
...
static void eff_Grock_draw(bdk_class* i_this, bdk_eff_s* eff) {
    J3DModel* model = eff->m044;
    g_env_light.setLightTevColorType(model, &i_this->m6224);
    mDoExt_modelUpdateDL(model);
}
...
static void my_effect_draw(bdk_class* i_this) {
...
static BOOL daBdk_Draw(bdk_class* i_this) {
    ...
    if (!dComIfGs_isStageBossEnemy()) {
        ...
        cXyz shadow_pos(actor->current.pos.x, actor->current.pos.y + (REG0_F(1) * 10.0f), actor->current.pos.z);
        f32 p5 = REG0_F(2) * 10.0f + 2000;
        i_this->m2C0 = dComIfGd_setRealShadow(i_this->m2C0, 1, i_this->mp2BC, &shadow_pos, p5, 0.0f, &actor->tevStr);
...
static void pos_move(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz diff;
    cXyz out;
    ...
    cLib_addCalcAngleS2(&actor->current.angle.y, targetAngleXZ, REG0_S(3) + 10, i_this->m2DC * i_this->m2E0);
    ...
    cLib_addCalcAngleS2(&actor->current.angle.z, (s16)sVar1, REG0_S(3) + 10, i_this->m2DC * i_this->m2E0 * 0.5f);

    cLib_addCalcAngleS2(&actor->current.angle.x, targetAngleY, REG0_S(3) + 10, i_this->m2DC * i_this->m2E0);
    ...
    if (i_this->m2591 == 0) {
        ...
        if (i_this->mpMorf->getFrame() < 20.0f) {
            actor->speed.y -= DEMO_SELECT(REG14_F(9), REG0_F(9)) + 1.5f;
        } else {
            actor->speed.y += DEMO_SELECT(REG14_F(10), REG0_F(10)) + 1.5f;
        }
    }
...
static void ground_move(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz diff;
    cXyz out;
...
static void up_fly(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz diff;
    cXyz out;
        ...
        case 1:
            {
                ...
                if (frame == DEMO_SELECT(REG12_S(7), REG0_S(7)) + 0x17) {
                    i_this->mState = 2;
                    actor->speedF = DEMO_SELECT(REG12_F(8), REG0_F(8)) + 50.0f;
                    i_this->m2E4 = 40.0f;
                    i_this->m2E8 = DEMO_SELECT(REG12_F(9), REG0_F(9)) + 1.0f;
...
static void fly(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz diff;
    i_this->mF10 = 1;
    switch (i_this->m2CA) {
    ...
    switch (i_this->mState) {
        case 0:
            ...
            if (i_this->m2FC == 0) {
                actor->home.pos.x = DEMO_SELECT(REG14_F(0), REG0_F(0)) + 6500.0f;
                actor->home.pos.y = DEMO_SELECT(REG14_F(1), REG0_F(1)) + 11000.0f;
                actor->home.pos.z = DEMO_SELECT(REG14_F(2), REG0_F(2)) + 2600.0f;

                i_this->m2CC.x = actor->home.pos.x + cM_rndFX(DEMO_SELECT(REG14_F(3), REG0_F(3)) + 3000.0f);
                i_this->m2CC.y = actor->home.pos.y + 1000.0f;
                i_this->m2CC.z = actor->home.pos.z + cM_rndFX(DEMO_SELECT(REG14_F(3), REG0_F(3)) + 3000.0f);
            } else if (i_this->m2FC == 1) {
                actor->home.pos.x = DEMO_SELECT(REG14_F(0), REG0_F(0)) + 9800.0f;
                actor->home.pos.y = DEMO_SELECT(REG14_F(1), REG0_F(1)) + 12000.0f;
                actor->home.pos.z = DEMO_SELECT(REG14_F(2), REG0_F(2)) + -1500.0f;

                i_this->m2CC.x = actor->home.pos.x + cM_rndFX(DEMO_SELECT(REG14_F(3), REG0_F(3)) + 1000.0f);
                i_this->m2CC.y = actor->home.pos.y + cM_rndF(1000.0f);
                i_this->m2CC.z = actor->home.pos.z + cM_rndFX(DEMO_SELECT(REG14_F(3), REG0_F(3)) + 1000.0f);
            } else if (i_this->m2FC == 2) {
                actor->home.pos.x = DEMO_SELECT(REG14_F(0), REG0_F(0)) + -1000.0f;
                actor->home.pos.y = DEMO_SELECT(REG14_F(1), REG0_F(1)) + 12000.0f;
                actor->home.pos.z = DEMO_SELECT(REG14_F(2), REG0_F(2)) + -2900.0f;
        ...
        case 1:
            ...
            if (diff.abs() < 1500.0f) {
                ...
                if (i_this->m2FC == 0 || cM_rndF(1.0f) < 0.3f || i_this->m2593 >= 4 || i_this->m8F8 < 2) {
                    ...
                    i_this->m2593 = 0;
#else
                if (i_this->m2FC == 0 || cM_rndF(1.0f) < 0.3f || i_this->m2593 >= 4) {
                    i_this->mAction = ACTION_LANDING;
                    i_this->m2593 = 0;
...
static void landing(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
        ...
        case 0:
            {
                ...
                if (land_area_check(&i_this->m2CC, 1400.0f)) {
                    ...
                    if (i_this->mState == 0) {
                        ...
                        i_this->m2CC.y += REG0_F(2) + 1000.0f;
                    }
            ...
            if (i_this->m2586 != 0 || i_this->mAcch.ChkGroundHit() || actor->current.pos.y <= i_this->mAcch.GetGroundH() + 1.0f) {
                ...
                if (i_this->mState == 3) {
                    ...
                    dComIfGp_particle_set(
                        dPa_name::ID_IT_SN_DK_TYAKUTI_ROCK00,
                        &actor->current.pos,
                        NULL,
                        NULL,
                        0xFF,
                        ...
                        &i_this->m6224.mColorK0,
                        &i_this->m6224.mColorK0
                    );
                    if (i_this->m6078[0] == 0) {
                        i_this->m6078[0] = 100;
                        dComIfGp_particle_setToon(
                            dPa_name::ID_IT_ST_DK_TYAKUTI_SMOKE00, &actor->current.pos, NULL, NULL, 0xB9, &i_this->m6080[0], (s8)fopAcM_GetRoomNo(actor)
                        );
                    }
                }
            }
...
static void wait(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    fopAc_ac_c* player = dComIfGp_getPlayer(0);
    ...
    switch (i_this->mState) {
            ...
            if (angle > 0x3000 || angle < -0x3000) {
                ...
                if ((i_this->m2EC[0] == 0) && (i_this->m2CA == 3)) {
                    if (l_HIO.m014 != 0) {
                        ...
                        i_this->m2592++;
                        if (dist < DEMO_SELECT(REG12_F(11), REG0_F(11)) + 450.0f) {
                            i_this->mAction = ACTION_JIDA_ATTACK;
                            i_this->mState = 0;
                        } else if (dist < DEMO_SELECT(REG12_F(12), REG0_F(12)) + 550.0f) {
#if VERSION > VERSION_DEMO
                            if (i_this->m8F8 < 2) {
                                i_this->mAction = ACTION_KUTI_ATTACK;
                            } else
        ...
        case 2:
            ...
            i_this->m2E8 = 1.0f;
            if (dist < DEMO_SELECT(REG12_F(12), REG0_F(12)) + 530.0f || dist > 1350.0f) {
                i_this->mState = 1;
            }
            break;
        case 5:
...
static void jump(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz diff;
    cXyz out;
    fopAc_ac_c* player = dComIfGp_getPlayer(0);
    ...
    switch (i_this->mState) {
        case 0:
            ...
            break;
        case 1:
            if (DEMO_SELECT(REG12_S(7), REG0_S(7)) + 0x17 != (s32)i_this->mpMorf->getFrame()) {
                break;
            }
...
static void jida_attack(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    fopAc_ac_c* player = dComIfGp_getPlayer(0);
        ...
        case 1:
            {
                ...
                if (frame == 1 || frame == 0xD) {
                    ...
                    dComIfGp_particle_set(
                        dPa_name::ID_IT_SN_DK_JIDANDA_ROCK00,
                        &i_this->m1174[index],
                        NULL,
                        NULL,
                        0xFF,
                    ...
                    if (i_this->m6078[1] == 0) {
                        i_this->m6078[1] = 0xA;
                        dComIfGp_particle_setToon(
                            dPa_name::ID_IT_ST_DK_JIDANDA_SMOKE00, &i_this->m1174[index], 0, NULL, 0xB9, &i_this->m6080[1], (s8)fopAcM_GetRoomNo(actor)
                        );
                    }
                }
                ...
                if ((frame >= 4 && frame <= 0xB) || (frame >= 0x10 && frame <= 0x17)) {
                    ...
                    if (frame == 4 || frame == 0x10) {
                        dComIfGp_getVibration().StartShock(DEMO_SELECT(REG6_S(2), REG0_S(2)) + 4, -0x21, cXyz(0.0f, 1.0f, 0.0f));
                    }
                } else {
                    vec1.z = 0.0f;
                }
...
static void kuti_attack(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz vec1;
    ...
    switch (i_this->mState) {
        ...
        case 2:
            ...
            if ((s16)(REG0_S(5) + 0x1E) == (s16)i_this->mpMorf->getFrame()) {
                ...
                dComIfGp_getVibration().StartShock(DEMO_SELECT(REG6_S(2), REG0_S(2)) + 6, -0x21, cXyz(0.0f, 1.0f, 0.0f));
                ...
                dComIfGp_particle_set(dPa_name::ID_IT_SN_DK_KUTI_SENKO00, &i_this->m261C[0x27].m004, NULL, NULL, 0xFF, NULL, -1);
    ...
    if (bVar2) {
        if (i_this->m6078[2] == 0) {
            ...
            dComIfGp_particle_setToon(
                dPa_name::ID_IT_ST_DK_KUTI_SMOKE00, &i_this->m261C[0x27].m004, 0, NULL, 0xB9, &i_this->m6080[2], (s8)fopAcM_GetRoomNo(actor)
            );
        }
        dComIfGp_particle_set(
            dPa_name::ID_IT_SN_DK_KUTI_ROCK00,
            &i_this->m261C[0x27].m004,
            NULL,
            NULL,
            0xFF,
            NULL,
...
static void wind_set(bdk_class* i_this, cXyz* param2) {
    ...
    if ((i_this->m2C4 & 1) == 0) {
        ...
        vec1.y = 0.0f;
        vec1.x = 0.0f;
        vec1.z = DEMO_SELECT(REG6_F(4), REG0_F(4)) + 100.0f;
        ...
        case 10:
            ...
            if (i_this->m6078[3] == 0) {
                ...
                emitter = dComIfGp_particle_setToon(
                    dPa_name::ID_IT_ST_DK_BUTSUKARI_SMOKE00,
                    &actor->current.pos,
                    &actor->shape_angle,
                    NULL,
                    0xB9,
            ...
            emitter = dComIfGp_particle_set(
                dPa_name::ID_IT_SN_DK_DMG_HANE_M00,
                &actor->current.pos,
                &actor->shape_angle,
                NULL,
                0xFF,
    ...
    if (i_this->mAction == ACTION_FLY_ATTACK && (i_this->mState < 10)) {
        ...
        if (i_this->mAcch.ChkGroundHit() && (i_this->m2619 == 0)) {
            for (s32 i = 0; i <= 1; i++) {
                ...
                i_this->mp6214[0 + i] = dComIfGp_particle_set(dPa_name::ID_IT_SN_DK_TSUME_HIBANA_A00, &foot_eff_pos[i], &actor->shape_angle);

                i_this->mp6214[2 + i] = dComIfGp_particle_set(dPa_name::ID_IT_SN_DK_TSUME_HIBANA_B00, &foot_eff_pos[i], &actor->shape_angle);
            }
            i_this->m2619++;
        }
...
static void wind_attack(bdk_class* i_this) {
    fopAc_ac_c* player = dComIfGp_getPlayer(0);
    fopAc_ac_c* actor = &i_this->actor;
    cXyz offset;
    switch (i_this->mState) {
...
static void end_set(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
        ...
        if (hit_type == 1) {
            ...
            if (atInfo.mResultingAttackType == 9 && i_this->m1150.y < 10000.0f) {
                ...
                if (i_this->m2B4 == 1) {
                    ...
                    i_this->mState = 0;
                    i_this->m2FA = DEMO_SELECT(REG6_S(3), REG0_S(3)) + 10;
                    i_this->m2F8 = 0x1E;
...
static void kamen_demo(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz pos;
    cXyz dist;
    ...
    switch (i_this->mState) {
        ...
        case 1:
            ...
            for (s32 i = 0; i < (s32)ARRAY_SIZE(i_this->m90C); i++) {
                ...
                cMtx_YrotS(*calc_mtx, i_this->m9A0[i].y);
                pos.x = (1.0f + DEMO_SELECT(REG14_F(7), REG0_F(7))) * kamen_break_sd_x[i];
                pos.y = 0.0f;
                pos.z = (1.0f + DEMO_SELECT(REG14_F(7), REG0_F(7))) * kamen_break_sd_z[i];
            ...
            break;
        case 2:
            if (i_this->m25A6 > DEMO_SELECT(REG14_S(8), REG0_S(8)) + 0x2d) {
                i_this->m1134 = 500;
                i_this->m113A = 1;
            }
    ...
    switch (i_this->mState) {
        ...
        case 1:
            if (i_this->m25D8 != 0) {
                ...
                dComIfGp_particle_set(
                    dPa_name::ID_IT_SN_DK_DEMO_HANE00,
                    &actor->current.pos,
                    &actor->shape_angle,
                    NULL,
                    0xFF,
        ...
        case 2:
            actor->current.pos.y += actor->speed.y;
            actor->speed.y -= DEMO_SELECT(REG13_F(13), REG0_F(13)) + 1.0f;
        ...
        case 3:
            ...
            if (i_this->m2EC[0] == (s16)(REG8_S(8) + 5)) {
                ...
                for (s32 i = 0; i < 2; i++) {
                    JPABaseEmitter* emitter = dComIfGp_particle_set(
                        dPa_name::ID_IT_SN_DK_DEMO_HANE01,
                        &actor->current.pos,
                        &actor->shape_angle,
                        NULL,
                        0xFF,
...
static void end(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    s8 bVar1 = 0;
    u16 particle_id = 0;
    i_this->m1134 = 4000;
    ...
    switch (i_this->mState) {
        ...
        case 5:
            if (i_this->mpMorf->isStop()) {
                ...
                for (s32 i = 0; i <= 1; i++) {
                    JPABaseEmitter* emitter = dComIfGp_particle_set(
                        dPa_name::ID_IT_SN_DK_DEAD_HANE_A00,
                        &actor->current.pos,
                        NULL,
                        NULL,
                        0xFF,
        ...
        case 6:
            ...
            cLib_addCalc2(&actor->current.pos.y, REG8_F(18) + 15800.0f, 0.1f, REG8_F(19) + 20.0f);
            ...
            if (i_this->m2EC[0] == 0 && DEMO_SELECT(REG6_S(4), REG0_S(4)) == 0) {
                ...
                dComIfGp_particle_set(
                    dPa_name::ID_IT_SN_DK_DEAD_HANE_B00,
                    &actor->current.pos,
                    NULL,
                    NULL,
                    0xFF,
    ...
    if ((u16)particle_id) {
        JPABaseEmitter* emitter = dComIfGp_particle_set(
            dPa_name::ID_IT_SN_DK_LASTDMG_HANE_M00,
            &actor->current.pos,
            &actor->shape_angle,
            NULL,
            0xFF,
...
static void t_fly(bdk_class* i_this) {
    ...
    switch (i_this->mState) {
        ...
        case 10:
            frame = i_this->mpMorf->getFrame();
            if (frame >= REG0_S(3) + 0x17 && (s32)i_this->mpMorf->getFrame() <= (REG0_S(4) + 0x32)) {
                wind_set(i_this, &actor->current.pos);
    ...
    if (player->current.pos.y > DEMO_SELECT(REG13_F(3), REG0_F(3)) + 9300.0f - 50.0f) {
...
static void t_landing(bdk_class* i_this) {
    i_this->mF10 = 1;
    switch (i_this->mState) {
        case 0:
...
static void t_lastattack(bdk_class* i_this) {
    ...
    fVar2 = (DEMO_SELECT(REG13_F(9), REG0_F(9)) * 0.01f) + -3872.0f;
    fVar1 = (DEMO_SELECT(REG13_F(8), REG0_F(8)) * 0.01f) + 9579.0f;
    i_this->m2CC.x = DEMO_SELECT(REG13_F(7), REG0_F(7)) * 0.01f + 2825.0f - 40.0f;
    i_this->m2CC.y = fVar1;
    i_this->m2CC.z = fVar2;
    ...
    switch (i_this->mState) {
        ...
        case 4:
            ...
            if ((s16)(REG0_S(5) + 0xD) == (s16)i_this->mpMorf->getFrame()) {
                fopAcM_seStart(actor, JA_SE_CM_DK_PECK, 0);
                dComIfGp_getVibration().StartShock(DEMO_SELECT(REG6_S(2), REG0_S(2)) + 3, -0x21, cXyz(0.0f, 1.0f, 0.0f));
                eff_hane_set(i_this, &i_this->m1168, 5, 0);
            }
            if (i_this->mpMorf->isStop()) {
    ...
    if (player->current.pos.y < DEMO_SELECT(REG13_F(3), REG0_F(3)) + 9290.0f - 50.0f) {
        i_this->mAction = ACTION_T_FLY;
        i_this->mState = -10;
        actor->speed.y = DEMO_SELECT(REG13_F(7), REG0_F(7)) + 70.0f;
    }
}

/* 00006C10-00007008       .text t_down__FP9bdk_class */
static void t_down(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz offset;
...
static void move(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
...
static void col_set(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz offset;
    cXyz dist;
    J3DModel* model;
    ...
    for (s32 i = 0; i < 2; i++) {
        ...
        if (i_this->mAction == ACTION_T_LASTATTACK) {
            ...
            offset.x = DEMO_SELECT(REG12_F(3), REG0_F(3)) + 100.0f;
            offset.y = DEMO_SELECT(REG12_F(4), REG0_F(4)) + 200.0f;
        } else {
            i_this->mFootCCSph[i].SetR(DEMO_SELECT(REG12_F(9), REG0_F(9)) + 60.0f);
            i_this->mFootCCSph[i].OnTgSetBit();
            offset.x = DEMO_SELECT(REG12_F(3), REG0_F(3)) + 50.0f;
            offset.y = DEMO_SELECT(REG12_F(4), REG0_F(4));
        }
        offset.z = DEMO_SELECT(REG12_F(5), REG0_F(5));
        MtxPosition(&offset, &i_this->m1174[i]);
        i_this->mFootCCSph[i].SetC(i_this->m1174[i]);

        if (i_this->m2584 == 2) {
            i_this->mFootCCSph[i].OnAtSetBit();
            if (i_this->mAction == ACTION_FLY_ATTACK || i_this->mAction == ACTION_T_FLY || i_this->mAction == ACTION_JIDA_ATTACK) {
...
static void kankyo_cont(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    ...
    for (s32 i = 0; i < 4; i++) {
        if (i_this->m90C[i] != 0) {
            ...
            if (i_this->m9E8[i] == 0) {
                ...
                gndChk.SetPos(&vec3);
                f32 ground_y = dComIfG_Bgsp()->GroundCross(&gndChk) + 15.0f + DEMO_SELECT(REG12_F(11), REG0_F(11));
                if (i_this->m910[i].y <= ground_y) {
                    ...
                    cLib_addCalc0(&i_this->m9FC[i], 1.0f, DEMO_SELECT(REG14_F(5), REG0_F(5)) + 0.2f);
                    ...
                    if (i_this->mA50[i].ChkTgHit() && i_this->m9FC[i] < 1.0f) {
                        ...
                        if (hit_atInfo.mResultingAttackType == 8) {
                            i_this->m9FC[i] = DEMO_SELECT(REG14_F(4), REG0_F(4)) + 7.0f + cM_rndF(2.0f);
...
static void obj_move(bdk_class* i_this) {
    if (dComIfGs_isStageBossEnemy()) {
        cLib_addCalc2(&i_this->m62D4, -500.0f, 1.0f, 20.0f);
    }
    MtxTrans(
        DEMO_SELECT(REG6_F(10), REG0_F(10)) + 4600.0f + 155.0f,
        DEMO_SELECT(REG6_F(11), REG0_F(11)) + 9800.0f + i_this->m62D4,
        DEMO_SELECT(REG6_F(12), REG0_F(12)) - 4700.0f,
        0
    );
    cMtx_YrotM(*calc_mtx, DEMO_SELECT(REG6_S(0), REG0_S(0)) + -7000);
...
static void demo_camera(bdk_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    ...
    switch (i_this->m25A0) {
        ...
        case 0x2:
            {
                ...
                local_104.x = DEMO_SELECT(REG12_F(5), REG0_F(5)) + 400.0f;
                local_104.y = DEMO_SELECT(REG12_F(6), REG0_F(6)) + -100.0f;
                local_104.z = DEMO_SELECT(REG12_F(7), REG0_F(7)) + 400.0f;
                ...
                player->setPlayerPosAndAngle(&local_108, actor->shape_angle.y + 20000 + DEMO_SELECT(REG6_S(6), REG0_S(6)));

                s16 r4 = i_this->m25A6;
                if (i_this->m25A6 > DEMO_SELECT(REG6_S(7), REG0_S(7)) + 0x3C) {
                    dVar16 = 20.0f;
                    dVar15 = 1.0f;
                    dVar14 = 0.5f;
                } else if ((s16)r4 > 0) {
        ...
        case 0xB:
            ...
            if (i_this->m25A6 > (s16)(DEMO_SELECT(REG13_S(1), REG0_S(1)) + 0x1E)) {
                cLib_addCalc2(&i_this->m25CC, DEMO_SELECT(REG13_F(1), REG0_F(1)) + 1.0f, 1.0f, DEMO_SELECT(REG13_F(2), REG0_F(2)) + 0.015f);
            }
            if (i_this->m25A6 >= (s16)(DEMO_SELECT(REG13_S(6), REG0_S(6)) + 0xBE)) {
                if (i_this->m25A6 == (s16)(DEMO_SELECT(REG13_S(6), REG0_S(6)) + 0xD2)) {
                ...
                if (i_this->m25A6 == (s16)(DEMO_SELECT(REG13_S(6), REG0_S(6)) + 0xBE)) {
                    ...
                    for (s32 i = 0; i <= 2; i++) {
                        ...
                        dComIfGp_particle_setToon(
                            dPa_name::ID_IT_ST_DK_FUTATOJI_SMOKE_A00, &center_pos, &local_14C, NULL, 0xB9, &i_this->m6130[i], (s16)fopAcM_GetRoomNo(actor)
                        );
                    }
                    dComIfGp_particle_setToon(
                        dPa_name::ID_IT_ST_DK_FUTATOJI_SMOKE_B00, &center_pos2, NULL, NULL, 0xB9, &i_this->m6130[3], (s16)fopAcM_GetRoomNo(actor)
                    );
            ...
            if (i_this->m25A6 > (s16)(DEMO_SELECT(REG13_S(2), REG0_S(2)) + 0xFA)) {
                i_this->m25A0 = 0xC;
                ...
                local_108.x = 2580.0f;
                local_108.y = player_actor->current.pos.y;
                local_108.z = -3670.0f;
        ...
        case 0xC:
            ...
            if (i_this->m25A6 == (s16)(DEMO_SELECT(REG13_S(7), REG0_S(7)) + 0xA0)) {
                i_this->m25A6 = 0;
                i_this->m25A0 = 0xD;
            }
            ...
            cLib_addCalc2(&i_this->m25CC, DEMO_SELECT(REG13_F(3), REG0_F(3)) + 1.5f, 1.0f, DEMO_SELECT(REG13_F(4), REG0_F(4)) + 0.02f);
        ...
        case 0xD:
            ...
            if (i_this->m25A6 == (s16)(DEMO_SELECT(REG13_S(8), REG0_S(8)) + 0x37)) {
                i_this->m25D8 = 1;
            }
            cLib_addCalc2(&i_this->m25B4.x, i_this->m1150.x, 0.1f, 50.0f);
            cLib_addCalc2(&i_this->m25B4.y, (i_this->m1150.y - 200.0f) + DEMO_SELECT(REG13_F(11), REG0_F(11)), 0.1f, 50.0f);
            ...
            if (i_this->m25A6 == (s16)(DEMO_SELECT(REG13_S(8), REG0_S(8)) + 0xF0)) {
                i_this->m25A6 = 0;
                i_this->m25A0 = 0xE;
            }
            r27 = 1;
        ...
        case 0xE:
            ...
            i_this->m25A6 = 0;
            i_this->m25CC = 0.0f;
            i_this->m6320 = DEMO_SELECT(REG13_F(17), REG0_F(17)) + -100.0f;
            i_this->m631C = 0.0f;
        case 0xF:
            r27 = 1;
            // Fall-through
        case 0x10:
            cLib_addCalc0(&i_this->m6320, 1.0f, DEMO_SELECT(REG6_F(11), REG0_F(11)) + 1.5f);
            if (std::fabsf(i_this->m6320) < 1.0f) {
                cLib_addCalc0(&i_this->m6324, 1.0f, DEMO_SELECT(REG6_F(12), REG0_F(12)) + 1.0f);
                if (i_this->m25A0 == 0xF) {
                    ...
                    for (s32 i = 0; i <= 2; i++) {
                        ...
                        dComIfGp_particle_setToon(
                            dPa_name::ID_IT_ST_DK_FUTATOJI_SMOKE_C00, &center_pos, &local_154, NULL, 0xB9, &i_this->m6130[i], (u8)fopAcM_GetRoomNo(actor)
                        );
                    }
            ...
            cLib_addCalc2(&i_this->m25CC, DEMO_SELECT(REG13_F(3), REG0_F(3)) + 1.2f, 1.0f, DEMO_SELECT(REG13_F(4), REG0_F(4)) + 0.015f);
        ...
        case 0x65:
            ...
            i_this->m25B4 = i_this->m1150;
            i_this->m25B4.y += DEMO_SELECT(REG6_F(16), REG0_F(16)) + -100.0f;
            local_104.x = 0.0f;
            local_104.y = 0.0f;
            local_104.z = i_this->m25D0 + DEMO_SELECT(REG6_F(17), REG0_F(17));
            ...
            if (i_this->m25A2 == 0) {
                i_this->m25A8.y = DEMO_SELECT(REG6_F(18), REG0_F(18)) + 9950.0f;
            } else {
                cLib_addCalc2(&i_this->m25A8.y, local_108.y + DEMO_SELECT(REG6_F(13), REG0_F(13)), 0.1f, 100.0f);
            }
        ...
        case 0x6F:
            {
                ...
                if (fVar17 < 30000.0f) {
                    i_this->m25B4.y = (fVar17 - 6000.0f) + DEMO_SELECT(REG6_F(19), REG0_F(19));
                }
                i_this->m25B4.z = -3800.0f;
                cLib_addCalc2(&i_this->m25A8.z, 30449.0f, 0.02f, i_this->m25CC * 75.33f);
                if (i_this->m25A6 > DEMO_SELECT(REG6_S(2), REG0_S(2))) {
                    cLib_addCalc2(&i_this->m25CC, DEMO_SELECT(REG6_S(1), REG0_S(1)) + 1.5f, 1.0f, DEMO_SELECT(REG6_F(2), REG0_F(2)) + 0.02f);
                }
                local_108.x = 4444.0f;
                local_108.y = 9800.0f;
                local_108.z = -4500.0f;
        ...
        case 0x71:
            ...
            if (i_this->m25A6 > DEMO_SELECT(REG12_S(7), REG0_S(7)) + 0x43) {
                cLib_addCalc2(&i_this->m25B4.y, DEMO_SELECT(REG12_F(4), REG0_F(4)) + 9900.0f, 0.5f, DEMO_SELECT(REG12_F(6), REG0_F(6)) + 200.0f);
            }
            i_this->m25A8 = i_this->m2CC;
            i_this->m25A8.y = DEMO_SELECT(REG12_F(5), REG0_F(5)) + 10000.0f;
            i_this->m25A8.x += DEMO_SELECT(REG12_F(7), REG0_F(7)) + 500.0f;
            ...
            if (i_this->m25A6 == DEMO_SELECT(REG6_S(2), REG0_S(2)) + 0x6E) {
                player->changeDemoMode(daPy_demo_c::DEMO_SMILE_e);
            }
            if (i_this->m25A6 == DEMO_SELECT(REG6_S(2), REG0_S(2)) + 0x82) {
                player->changeDemoMode(daPy_demo_c::DEMO_UNK_029_e);
            }
            if (i_this->m25A6 == DEMO_SELECT(REG6_S(4), REG0_S(4)) + 0xBE) {
                i_this->m25A0++;
                i_this->m25A6 = 0;
            }
            break;
    ...
    if (i_this->m25A0 != 0 && r28) {
        ...
        if (i_this->m25D4 > 0.1f) {
            r20 = (50.0f + DEMO_SELECT(REG13_F(6), REG0_F(6))) * cM_scos(i_this->m25A4 * 0x1c00);
        }
...
static void eff_hane_move(bdk_class* i_this, bdk_eff_s* i_eff) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz vec1;
    cXyz vec2;
    cXyz vec3;
    ...
    vec1.y += 200.0f;
    gndChk.SetPos(&vec1);
    height = dComIfG_Bgsp()->GroundCross(&gndChk) + 20.0f;
    ...
    if (i_eff->m048.ChkTgHit() != NULL && i_eff->m040 == 0) {
        fopAc_ac_c* tgActor = i_eff->m048.GetTgHitObj()->GetAc();
        if (tgActor != NULL) {
            vec3 = i_eff->m004 - tgActor->current.pos;

            if (i_eff->m020 < 5.0f) {
                i_eff->m030.y = cM_atan2s(vec3.x, vec3.z) + (s16)cM_rndFX(4000.0f);
                i_eff->m020 = 10.0f + cM_rndF(5.0f) + DEMO_SELECT(REG13_F(8), REG0_F(8));
                i_eff->m01C = 10.0f + cM_rndF(5.0f) + DEMO_SELECT(REG13_F(9), REG0_F(9));
            }

            i_eff->m036.z = cM_rndFX(DEMO_SELECT(REG13_F(10), REG0_F(10)) + 1500.0f);
            i_eff->m036.x = cM_rndFX(DEMO_SELECT(REG13_F(16), REG0_F(16)) + 1500.0f);
...
static void eff_Grock_move(bdk_class*, bdk_eff_s* eff) {
    f32 fVar1 = 0.0f;
    if (eff->m03C == 1) {
        fVar1 = -10.0f;
    }
...
static void my_effect_move(bdk_class* i_this) {
    bdk_eff_s* eff = &i_this->m261C[0];
    for (s32 i = 0; i < (s32)ARRAY_SIZE(i_this->m261C); i++, eff++) {
        if (eff->m000 != 0) {
            eff->m03C++;
...
static BOOL daBdk_Execute(bdk_class* i_this) {
    ...
    if (i_this->m113A) {
        ...
        s16 iVar4 = (DEMO_SELECT(REG14_F(5), REG0_F(5)) + -2000.0f) * i_this->m1140 * cM_ssin(i_this->m1130 * (REG8_S(4) + 2500));
        s16 iVar11 = i_this->m113C * 4000.0f;
        cLib_addCalc2(&i_this->m113C, 1.0f, 1.0f, DEMO_SELECT(REG14_F(4), REG0_F(4)) + 0.01f);
        if (i_this->m25A6 > DEMO_SELECT(REG14_S(9), REG0_S(9)) + 0x41) {
            cLib_addCalc2(&i_this->m1140, 1.0f, 1.0f, DEMO_SELECT(REG14_F(0x2), REG0_F(0x2)) + 0.4f);
        }
    ...
    offset.y = 0.0f;
    offset.z = DEMO_SELECT(REG6_F(12), REG0_F(12)) + -120.0f;
    ...
    if (i_this->m2618 != 0 && (i_this->m2618--, i_this->m2618 == 0)) {
        dComIfGp_particle_setToon(
            dPa_name::ID_IT_ST_DK_TOBITATI_SMOKE00, &i_this->m260C, &actor->shape_angle, NULL, 0xB9, &i_this->m6110, (s8)fopAcM_GetRoomNo(actor)
        );
    }
    if (i_this->m2619 != 0) {
        ...
        for (s32 i = 0; i < ARRAY_SIZE(i_this->m1174); i++) {
            foot_eff_pos[i] = i_this->m1174[i];
            foot_eff_pos[i].y -= DEMO_SELECT(REG6_F(9), REG0_F(9)) + 200.0f;
...
static cPhs_State daBdk_Create(fopAc_ac_c* a_this) {
    ...
    if (phase_state == cPhs_COMPLEATE_e) {
        ...
        if (dComIfGs_isStageBossEnemy() || DEMO_SELECT(REG17_S(0), REG0_S(0) != 0)) {
            dComIfGs_onStageBossEnemy();
