...
void mDoExt_J3DModelPacketS::draw() {
    ...
    J3DMatPacket* pJVar1 = NULL;
    int* piVar2;
    J3DPacket* uVar3;
    MtxP MStack_48 = NULL;
    J3DMaterial* mesh = NULL;

    cMtx_copy(j3dSys.mViewMtx, MStack_48);
    mDoMtx_stack_c::transS(REG0_F(0xe), REG0_F(0xf) + -30.0f, REG0_F(0x10));
    mDoMtx_stack_c::scaleM(1.0f, -1.0f, 1.0f);
    MTXConcat(j3dSys.mViewMtx, mDoMtx_stack_c::now, mDoMtx_stack_c::now);
    cMtx_copy(mDoMtx_stack_c::now, j3dSys.mViewMtx);
    for (uVar3 = 0; uVar3 < this->getNextPacket(); uVar3++) {
        while (true) {
            j3dSys.setTexture(pJVar1->mpTexture);
            j3dSys.setMatPacket(j3dSys.getModel()->getMatPacket(mesh->getIndex()));
            uVar3->draw();
            setMaterial();
            this_00 = pJVar1->mpInitShapePacket;
            this_00->mpShape->loadPreDrawSetting();
            for (; this_00 != (J3DShapePacket*)0x0; this_00 = (J3DShapePacket*)this_00->mpNextPacket) {
                this_01 = this_00->mpDisplayListObj;
    ...
    mDoMtx_stack_c::copy(j3dSys.mViewMtx);
};
...
void water0_disp(bgn_class* i_this) {
    ...
    pJVar8->getBlend()->getType();
    pJVar8->getZMode()->getFunc();
    pJVar8->getTevColor(0);
    pJVar8->setTevKColor(0, &tev_kcol2);
    pJVar8->setTevKColor(3, &tev_kcol2);
    pJVar8->getZMode()->setUpdateEnable(1);
    tev_kcol2.mColor.r = 0;
    tev_kcol2.mColor.g = 1;
    tev_kcol2.mColor.b = 2;
    tev_col.mColor.r = 0;
    tev_col.mColor.g = 0;
    tev_col.mColor.b = 0;
    tev_col.mColor.a = 0;
    ...
    pJVar8->setTevKColor((char)(int)(fVar1 * 2.559f), NULL);
...
void water1_disp(bgn_class* i_this) {
    ...
    pJVar8->getBlend()->getType();
    pJVar8->getZMode()->setUpdateEnable(0);
...
static BOOL daBgn2_Draw(bgn2_class* i_this) {
    ...
    if (l_HIO.m00C != 0) {
        i_this->m02C8 = i_this->m02B4->getModel();
        dComIfGd_getOpaListSky()->entryImm(&i_this->m02B8, 0);
    }
    i_this->m02CC->entryDL();
    if (l_HIO.m00C != 0) {
        i_this->m02E0 = i_this->m02CC->getModel();
...
static BOOL daBgn3_Draw(bgn3_class* i_this) {
    ...
    if (l_HIO.m00C != 0) {
        i_this->m002C8 = i_this->m002B4->getModel();
    ...
    for (i = 0; i < 9; i++, ppVar8++) {
        ...
        if (pJVar6 != NULL) {
            ...
            if (l_HIO.m00C != 0) {
                i_this->mParts[i].m0014 = i_this->mParts[i].m0000;
...
void part_control_0Z(bgn_class* i_this, int param_2, part_s* param_3, move_s* param_4, f32 param_5) {
    ...
    if (param_3 == i_this->m0C04) {
        ...
        local_f8.y = 0.0f;
        local_f8.z = 0.0f;
        MtxPosition(&local_f8, &local_11c);
    ...
    for (iVar7 = 1; iVar7 < param_2 + 1; iVar7++, ppVar6++) {
        ...
        if (dVar11 <= dVar10) {
            ...
            if ((iVar7 == param_2) && ((i_this->mC746 & 7U) == 0)) {
                local_128 = ppVar6->m0D4;
                if (!(gr_check(i_this, &local_128))) {
                    dComIfGp_particle_setSimple(dPa_name::ID_SCENE_8443, &local_128, 0xff, g_whiteColor, g_whiteColor, 0);
                    if (i_this->m0304 == 0) {
                        fVar16 = cM_rndF(20.0f);
                        i_this->m0304 = (s16)(int)(fVar16 + 20.0f);
                        i_this->m0308 = local_128;
                        mDoAud_seStart(JA_SE_CM_BGN_BODY_RIPPLE, &i_this->m0308, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                    }
                }
            }
        }
        local_134.x = local_11c.x * (param_2 - iVar7);
        local_134.z = local_11c.z * (param_2 - iVar7);
        fVar16 = local_134.x + (ppVar6->m0D4.x - ppVar6[-1].m0D4.x);
        dVar13 = fVar16;
        dVar12 = (dVar11 - ppVar6[-1].m0D4.y);
        fVar1 = local_134.z + (ppVar6->m0D4.z - ppVar6[-1].m0D4.z);
        dVar11 = fVar1;
        ...
        local_f8.x = i_this->mCC80 * (param_5 * ppVar6[-1].m0F4);
        local_f8.y = 0.0f;
        local_f8.z = 0.0f;
...
void part_control_2(bgn_class* i_this, int param_2, part_s* param_3, f32 param_4) {
    ...
    for (; iVar3 >= 0; iVar3 = iVar3 + -1, ppVar6--) {
        ...
        local_84.x = (param_4 * ppVar6->m0F4);
        local_84.y = 0.0f;
        local_84.z = 0.0f;
        MtxPosition(&local_84, &cStack_90);
        ppVar6->m0D4 = ppVar6[1].m0D4 + cStack_90;
    }
    local_84 = ppVar6[1].m0D4 - local_9c;
    for (iVar3 = 0; iVar3 < param_2; iVar3++, ppVar6++) {
        ppVar6[1].m0D4 -= local_84;
    }
}
...
void damage_check(bgn_class* i_this) {
    ...
    if (i_this->mC7B6 == 0) {
        ...
        sph = &i_this->m078C[0].m0F8;
        if (sph->ChkTgHit()) {
            local_48.mpObj = sph->GetTgHitObj();
            cVar5 = 3;
        }
        for (s32 i = 0; i < 0x14; i++) {
            pdVar6 = &i_this->m0C04[i].m0F8;
            if (pdVar6->ChkTgHit()) {
                local_48.mpObj = pdVar6->GetTgHitObj();
                cVar5 = 4;
            }
            pdVar6 = &i_this->m3AF0[i].m0F8;
            if (pdVar6->ChkTgHit()) {
                local_48.mpObj = pdVar6->GetTgHitObj();
                cVar5 = 5;
            }
        ...
        for (s32 i = 0; i < 3; i++) {
            pdVar6 = &i_this->m69DC[i].m0F8;
            if (pdVar6->ChkTgHit()) {
                local_48.mpObj = pdVar6->GetTgHitObj();
                cVar5 = 6;
            }
            pdVar6 = &i_this->m72CC[i].m0F8;
            if (pdVar6->ChkTgHit()) {
                local_48.mpObj = pdVar6->GetTgHitObj();
                cVar5 = 7;
            }
...
void size_set(bgn_class* i_this) {
    ...
    for (s32 i = 1; i < BGN_HAND_MAX; i++) {
        fVar1 = fVar2 * (s32)(i - 1U);
        i_this->m0C04[i].m0F4 = l_HIO.m104 - fVar1;
        i_this->m3AF0[i].m0F4 = l_HIO.m104 - fVar1;
    }
    i_this->m0C04[BGN_HAND_MAX].m0F4 = l_HIO.m108;
    i_this->m3AF0[BGN_HAND_MAX].m0F4 = l_HIO.m108;
    fVar2 = (l_HIO.m104 - l_HIO.m108) * 0.5f + l_HIO.m108;
    for (s32 i = 0; i < BGN_HAND_MAX; i++) {
    ...
    fVar2 = l_HIO.m10C - l_HIO.m110;
    for (s32 i = 0; i < 3; i++) {
        fVar1 = fVar2 * 0.5f * (s32)(i);
        i_this->m69DC[i].m0F4 = l_HIO.m10C - fVar1;
        i_this->m72CC[i].m0F4 = l_HIO.m10C - fVar1;
    }
    i_this->m69DC[3].m0F4 = l_HIO.m110;
    i_this->m72CC[3].m0F4 = l_HIO.m110;
    uVar6 = BGN_TAIL_MAX - 1;
    fVar2 = l_HIO.m114 - l_HIO.m118;
    for (s32 i = 0; i < BGN_TAIL_MAX; i++) {
        i_this->m7BBC[i].m0F4 = l_HIO.m114 - (1.0f / (s32)(uVar6 - 1)) * (fVar2) * (s32)(i);
    }
...
void punch_LR(bgn_class* i_this) {
    ...
    fopAc_ac_c* player = dComIfGp_getPlayer(0);
    bVar2 = false;
    cLib_addCalc0(&i_this->mC728.y, 1.0f, 20.0f);
    bVar3 = false;
    uVar11 = 0;
    ...
    case 5:
        ...
        if (i_this->mC7AC[0] == 0x14) {
            ...
            local_84 = local_90 - i_this->mC788;
            cMtx_YrotS(*calc_mtx, cM_atan2s(local_84.x, local_84.z));
            fVar1 = std::sqrtf(local_84.x * local_84.x + local_84.x * local_84.z);
            cMtx_XrotM(*calc_mtx, -cM_atan2s(local_84.y, fVar1));
            local_6c.x = 0.0f;
            local_6c.y = 0.0f;
            local_6c.z = REG0_F(7) + 200.0f;
    ...
    if (uVar11 != 0) {
        if (uVar11 <= 2) {
            cLib_addCalc2(&i_this->mC324[uVar11 - 1], REG0_F(0xe) + 1.0f, 0.2f, 0.05f);
        } else {
            for (s32 i = 0; i < 2; i++) {
...
void himo_control(bgn_class* i_this, move_s* param_2, cXyz* param_3) {
    ...
    pcVar5 = param_3 + 1;
    for (s32 i = 0; i < 60; i++) {
        dVar13 = (param_2->m000[0].x - *(f32*)&param_2[-1].m300);
        fVar17 = param_2->m000[0].z - *(f32*)&param_2[-1].m308;
        dVar12 = fVar17;
        dVar10 = ((param_2->m000[0].y - 40.0f) + REG0_F(0xd));
        if (dVar10 < 0.0f) {
            dVar10 = 0.0f;
            bVar9 = true;
        } else {
        ...
        fVar1 = (dVar10 - param_2[-1].m304);
        dVar11 = fVar1;
        dVar10 = param_2->m304;
        if (dVar10 > 0.01f) {
            iVar7 = (int)(dVar10 * cM_ssin(param_2->m2FA + i * (REG0_S(3) + 2000)));
            iVar6 = (int)(dVar10 * cM_scos(param_2->m2FC + i * (REG0_S(4) + 0x9c4)));
        }
        cMtx_XrotS(*calc_mtx, iVar7 - cM_atan2s(fVar1, fVar17));
        dVar10 = std::sqrtf((dVar11 * dVar11) + (dVar12 * dVar12));
        cMtx_YrotM(*calc_mtx, iVar6 + cM_atan2s(dVar13, dVar10));
        MtxPosition(&local_ac, &local_b8);
        param_2->m000[0].x = *(f32*)&param_2[-1].m300 + local_b8.x;
        param_2->m000[0].y = param_2[-1].m304 + local_b8.y;
        param_2->m000[0].z = *(f32*)&param_2[-1].m308 + local_b8.z;
        *pcVar5 = param_2->m000[0];
        if (((bVar9) && (((int)i_this->mC746 & 7U) == 0)) && (((uint)(int)i_this->mC746 >> 3 & 0xf) == (i & 0xf))) {
            local_c4 = param_2->m000[0];
...
void last_himo_control(bgn_class* i_this) {
    ...
    *pcVar8 = i_this->mC450[0];
    ...
    pcVar5 = i_this->mC450;
    for (s32 i = 0; i < 60; i++, pcVar5++, puVar8++) {
        ...
        dVar10 = i_this->mC724;
        if (dVar10 > 0.01f) {
            iVar9 = (int)(dVar10 * cM_ssin(i_this->mC746 * 2000 + i * (REG0_S(3) + 2000)));
            iVar6 = (int)(dVar10 * cM_scos(i_this->mC746 * 0x8fc + i * (REG0_S(4) + 0x9c4)));
        }
        cMtx_XrotS(*calc_mtx, iVar9 - cM_atan2s(fVar1, fVar2));
...
void move(bgn_class* i_this) {
    ...
    if (i_this->m02B5 != 0) {
        ...
        if (i_this->mAAA8[3].m2D0 == 0) {
            ...
            sVar1 = (int)(i_this->mAAA8[3].m2D4.y * (REG0_F(0xc) + 5.0f));
            local_110.y += (int)(i_this->mAAA8[3].m2D4.z * -(REG0_F(0xd) + 5.0f));
        }
        local_110.z = local_110.z + sVar1;
        if (i_this->mAAA8[4].m2D0 == 0) {
            sVar1 = REG0_S(0) + 3000;
        } else {
            sVar1 = (int)(i_this->mAAA8[4].m2D4.y * -(REG0_F(0xc) + 5.0f));
            local_110.y = local_110.y + (int)(i_this->mAAA8[4].m2D4.z * (REG0_F(0xd) + 5.0f));
        }
        local_110.z = local_110.z + sVar1;
        ...
        for (s32 i = 0; i < 2; i++) {
            if ((i == 0) && (i_this->mC778 != 0)) {
                local_e4 = i_this->m0C04[BGN_HAND_MAX].m0D4 - i_this->m078C[BGN_HAND_MAX + 1].m0D4;
                fVar9 = local_e4.abs();
            } else if ((i == 1) && (i_this->mC779 != 0)) {
                local_e4 = i_this->m3AF0[BGN_HAND_MAX].m0D4 - i_this->m3AF0[BGN_HAND_MAX - 1].m0D4;
                fVar9 = local_e4.abs();
            }
            if (fVar9 > (200.0f + REG0_F(5))) {
                fVar9 -= (200.0f + REG0_F(5)) * (REG0_F(6) + 0.2f);
            } else {
                fVar9 = 0.0f;
            }
...
void demo_camera(bgn_class* i_this) {
    ...
    pcVar10 = dComIfGp_getCamera(dComIfGp_getPlayerCameraID(0));
    switch ((u8)i_this->mCA62) {
    case 1:
        ...
        i_this->mCA62++;
        pcVar10->mCamera.Stop();
        pcVar10->mCamera.SetTrimSize(2);
        i_this->mCA64 = 0;
        i_this->mCAA0 = 55.0f;
        i_this->mCA98 = 0.0f;
    ...
    case 0x16:
        ...
        break;
    case 0x96:
        pcVar10->mCamera.SetTrimSize(0);
        pcVar10->mCamera.Start();
        dMeter_mtrShow();
    ...
    if (i_this->mCA62 != 0) {
        local_98.x = i_this->mCA68.x + i_this->mCA9C * cM_ssin(i_this->mCA64 * 0x3300);
        local_98.y = i_this->mCA68.y + i_this->mCA9C * cM_scos(i_this->mCA64 * 0x3000);
        local_98.z = i_this->mCA68.z;
        local_8c.x = i_this->mCA80.x + i_this->mCA9C * cM_ssin(i_this->mCA64 * 0x3300);
        dVar11 = local_8c.x;
        local_8c.y = i_this->mCA80.y + i_this->mCA9C * cM_scos(i_this->mCA64 * 0x3000);
        local_8c.z = i_this->mCA80.z;
        iVar7 = (int)(i_this->mCA9C * cM_scos(i_this->mC746 * 0x1c00) * 7.5f);
        pcVar10->mCamera.Set(local_8c, local_98, (short)iVar7, i_this->mCAA0);
        cLib_addCalc0(&i_this->mCA9C, (REG0_F(0x10) + 1.0f), dVar11);
