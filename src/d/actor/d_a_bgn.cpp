    ...
    if (model != NULL) {
        ...
        if (local_38.abs() > l_HIO.m008 * param_2->m0F4) {
            ...
            if (actor->health <= 2 && param_2 == &i_this->mTailParts[BGN_TAIL_MAX - 1]) {
                if (actor->health == 1) {
                    i_this->mJyakutenCBrkAnm->entry(model->getModelData());
                } else {
                    i_this->mJyakutenBBrkAnm->entry(model->getModelData());
                }
            }
...
static void water0_disp(bgn_class* i_this) {
    ...
    dComIfGd_setListSky();
    J3DMaterial* mat = i_this->mpWater0Model->getModelData()->getMaterialNodePointer(0);
    J3DBlend* blend = mat->getBlend();
    blend->getType();
    J3DZMode* zMode = mat->getZMode();
    zMode->getFunc();
    J3DGXColorS10* tev_col = (J3DGXColorS10*)&mat->getTevColor(0)->mColor;
    J3DGXColor* tev_kcol = (J3DGXColor*)&mat->getTevKColor(0)->mColor;
    J3DGXColor* tev_kcol2 = (J3DGXColor*)&mat->getTevKColor(3)->mColor;
...
static void water1_disp(bgn_class* i_this) {
    ...
    dComIfGd_setListSky();
    J3DMaterial* mat = i_this->mpWater1Model->getModelData()->getMaterialNodePointer(0);
    J3DBlend* blend = mat->getBlend();
    blend->getType();
    J3DZMode* zMode = mat->getZMode();
...
static BOOL daBgn2_Draw(bgn2_class* i_this) {
    ...
    if (actor->health != 0) {
        if (actor->health == 3) {
            model = i_this->mpJyakutenModel[2];
        } else if (actor->health == 1) {
            model = i_this->mpJyakutenModel[0];
            i_this->mJyakutenCBrkAnm->entry(model->getModelData());
        } else {
            model = i_this->mpJyakutenModel[1];
            i_this->mJyakutenBBrkAnm->entry(model->getModelData());
        }
        g_env_light.setLightTevColorType(model, &actor->tevStr);
        mDoExt_modelUpdateDL(model);
        if (l_HIO.m00C != 0) {
            i_this->m02E4.setModel(model);
            i_this->m02E4.update();
        }
    }
...
#endif
    J3DModel* model = i_this->m002CC;
    g_env_light.setLightTevColorType(model, &actor->tevStr);
    mDoExt_modelUpdateDL(model);
#if VERSION == VERSION_DEMO
    i_this->m002D0.setModel(model);
    i_this->m002D0.update();
#else
    if (l_HIO.m00C != 0) {
        i_this->m002D0.setModel(model);
        i_this->m002D0.update();
...
static void part_mtx_set(bgn_class* i_this, int param_2, part_s* param_3, int param_4, int param_5) {
    ...
    local_8c.x = local_8c.y = local_8c.z = 0.0f;
    ...
    if (i_this->mC7B6 == 0) {
        ...
        if (i_this->mCoreSph.ChkTgHit()) {
            ...
            if (actor->health != 0) {
                ...
                scale.x = scale.y = scale.z = REG0_F(5) + 2.0f;
                if (actor->health == 0) {
                    mDoAud_bgmStop(30);
                    i_this->mC748 = 6;
                    i_this->mC74A = 0;
        ...
        if (i_this->mC7FC.ChkTgHit()) {
            atInfo.mpObj = i_this->mC7FC.GetTgHitObj();
            cVar5 = 1;
        }
        ...
        for (s32 i = 0; i < 20; i++) {
            if (i_this->mLeftArmParts[i].mPartSph.ChkTgHit()) {
                atInfo.mpObj = i_this->mLeftArmParts[i].mPartSph.GetTgHitObj();
                cVar5 = 4;
            }
            if (i_this->mRightArmParts[i].mPartSph.ChkTgHit()) {
                atInfo.mpObj = i_this->mRightArmParts[i].mPartSph.GetTgHitObj();
                cVar5 = 5;
            }
        }
        for (s32 i = 0; i < 3; i++) {
            if (i_this->mLeftLegParts[i].mPartSph.ChkTgHit()) {
                atInfo.mpObj = i_this->mLeftLegParts[i].mPartSph.GetTgHitObj();
                cVar5 = 6;
            }
            if (i_this->mRightLegParts[i].mPartSph.ChkTgHit()) {
                atInfo.mpObj = i_this->mRightLegParts[i].mPartSph.GetTgHitObj();
                cVar5 = 7;
            }
        }
        for (s32 i = 0; i < 20; i++) {
            if (i_this->mTailParts[i].mPartSph.ChkTgHit()) {
                atInfo.mpObj = i_this->mTailParts[i].mPartSph.GetTgHitObj();
                cVar5 = 8;
            }
        }
...
static void shape_calc(bgn_class* i_this) {
    ...
    if (i_this->mAAA8[0].m2D0 == 0 || i_this->mAAA8[1].m2D0 == 0) {
        cMtx_YrotS(*calc_mtx, i_this->mC314.y);
        local_8c.x = l_HIO.m054;
        local_8c.y = l_HIO.m058;
        local_8c.z = l_HIO.m05C;
    ...
    MtxPull();
    MtxPush();
    local_8c.x = l_HIO.m144;
    local_8c.y = l_HIO.m148 + 20.0f;
    local_8c.z = l_HIO.m14C;
    ...
    if (i_this->mAAA8[2].m2D0 == 0) {
        cMtx_YrotS(*calc_mtx, i_this->mC314.y);
        local_8c.x = l_HIO.m060;
        local_8c.y = l_HIO.m064;
        local_8c.z = l_HIO.m068;
    ...
    MtxPull();
    MtxPush();
    local_8c.x = l_HIO.m138;
    local_8c.y = l_HIO.m13C;
    local_8c.z = l_HIO.m140;
    local_8c.x *= i_this->mCC80;
    local_8c.y *= i_this->mCC80;
    local_8c.z *= i_this->mCC80;
    ...
    MtxScale(fVar3, fVar3, fVar3, true);
    MtxPush();
    local_8c.x = l_HIO.m150;
    local_8c.y = l_HIO.m154;
    local_8c.z = l_HIO.m158;
    local_8c.x *= i_this->mCC80;
    local_8c.y *= i_this->mCC80;
    local_8c.z *= i_this->mCC80;
    ...
    if (i_this->mAAA8[5].m2D0 == 0) {
        cMtx_YrotS(*calc_mtx, i_this->mC314.y);
        local_8c.x = l_HIO.m084;
        local_8c.y = l_HIO.m088;
        local_8c.z = l_HIO.m08C;
    ...
    if (i_this->mAAA8[6].m2D0 == 0) {
        cMtx_YrotS(*calc_mtx, i_this->mC314.y);
        local_8c.x = l_HIO.m090;
        local_8c.y = l_HIO.m094;
        local_8c.z = l_HIO.m098;
    ...
    MtxPull();
    local_8c.x = l_HIO.m15C;
    local_8c.y = l_HIO.m160;
    local_8c.z = l_HIO.m164;
    local_8c.x *= i_this->mCC80;
    local_8c.y *= i_this->mCC80;
    local_8c.z *= i_this->mCC80;
    MtxPosition(&local_8c, &cStack_98);
...
static void himo_control(bgn_class* i_this, move_s* param_2, cXyz* param_3) {
    ...
    for (; i < 60; i++, pcVar5++, param_3++) {
        ...
        f32 y = y2 - pcVar5[-1].y;
        if (param_2->m304 > 0.01f) {
            j = (param_2->m304 * cM_ssin(param_2->m2FA + i * (REG0_S(3) + 2000)));
            k = (param_2->m304 * cM_scos(param_2->m2FC + i * (REG0_S(4) + 0x9c4)));
        }
        cMtx_XrotS(*calc_mtx, j - cM_atan2s(y, z));
        cMtx_YrotM(*calc_mtx, k + cM_atan2s(x, std::sqrtf(SQUARE(y) + SQUARE(z))));
...
static void last_himo_control(bgn_class* i_this) {
    ...
    *pcVar8 = i_this->mC450[0];
    ...
    for (; i < 60; i++, pcVar5++, var_r25++, var_r28++) {
        ...
        f32 y = pcVar5->y - pcVar5[-1].y - 35.0f + REG0_F(13);
...
static void action_s(bgn_class* i_this, move_s* param_2, int param_3) {
    ...
    param_2->m2FA += (s16)((param_3 * 3 + 2600) + REG0_S(5));
    param_2->m2FC += (s16)((param_3 * 3 + 2300) + REG0_S(6));
    if (param_2->m2FE != 0) {
        param_2->m2FE--;
    }
    if (l_HIO.m025 != 0) {
        ...
        switch (param_3) {
            case 0:
                param_2->m2E0.x = l_HIO.m0AA;
                param_2->m2E0.y = l_HIO.m0AC;
                param_2->m2E0.z = l_HIO.m0AE;
                break;
            case 2:
                param_2->m2E0.x = l_HIO.m0B0;
                param_2->m2E0.y = l_HIO.m0B2;
                param_2->m2E0.z = l_HIO.m0B4;
                break;
            case 3:
                param_2->m2E0.x = l_HIO.m0B6;
                param_2->m2E0.y = l_HIO.m0B8;
                param_2->m2E0.z = l_HIO.m0BA;
                break;
            case 4:
                param_2->m2E0.x = l_HIO.m0BC;
                param_2->m2E0.y = l_HIO.m0BE;
                param_2->m2E0.z = l_HIO.m0C0;
                break;
            case 5:
                param_2->m2E0.x = l_HIO.m0C2;
                param_2->m2E0.y = l_HIO.m0C4;
                param_2->m2E0.z = l_HIO.m0C6;
                break;
            case 6:
                param_2->m2E0.x = l_HIO.m0C8;
                param_2->m2E0.y = l_HIO.m0CA;
                param_2->m2E0.z = l_HIO.m0CC;
                break;
            case 7:
...
static cPhs_State daBgn_Create(fopAc_ac_c* a_this) {
    ...
    if (res == cPhs_COMPLEATE_e) {
        ...
        if (i_this->m02B4 != 0xFF) {
            ...
            i_this->mHeadParts[0].mPartSph.SetStts(&i_this->mStts);
            i_this->mHeadParts[0].mPartSph.OffAtSetBit();
#if VERSION > VERSION_DEMO
            i_this->mHeadParts[0].mPartTevStr = a_this->tevStr;
#endif
            for (s32 i = 0; i < 1; i++) {
                i_this->mPelvisParts[i].mPartSph.Set(cc_sph_src);
                i_this->mPelvisParts[i].mPartSph.SetStts(&i_this->mStts);
                i_this->mPelvisParts[i].mPartSph.OffAtSetBit();
#if VERSION > VERSION_DEMO
                i_this->mPelvisParts[i].mPartTevStr = a_this->tevStr;
#endif
            }
            for (s32 i = 0; i < 20; i++) {
