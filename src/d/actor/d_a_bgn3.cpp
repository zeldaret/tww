...
#include "d/dolzel_rel.h" // IWY pragma: keep
...
static BOOL daBgn3_Draw(bgn3_class* i_this) {
    ...
    if (((bgn != NULL) && (bgn->m02B5 == 2)) && (l_HIO.m3E != 0)) {
        ...
        for (s32 i = 0; i < 8; i++, part++) {
            part->mPart3LineMat.update(5, l_HIO.m48, ke_color, 3, &actor->tevStr);
            dComIfGd_set3DlineMat(&part->mPart3LineMat);
        }
    }
    return TRUE;
}
...
static s32 gr_check(bgn3_class* i_this, cXyz* param_2) {
    fopAc_ac_c* actor = &i_this->actor;
    dBgS_LinChk linChk;
...
static void tail_eff_set(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz local_38;

    if ((i_this->m0FD88 + 2 & 3) == 0) {
        local_38 = actor->current.pos;
        if (!gr_check(i_this, &local_38)) {
            dComIfGp_particle_setSimple(dPa_name::ID_AK_SN_O_KGTCOMMONHAMON03, &local_38, 0xFF, g_whiteColor, g_whiteColor, 0);
            if (i_this->m0FD94 == 0) {
                i_this->m0FD94 = cM_rndF(20.0f) + 20.0f;
                fopAcM_seStart(actor, JA_SE_CM_BGN_M_HEAD_RIPPLE, 0);
            }
        }
    }
    for (s32 i = 0; i < 9; i++) {
        if ((i_this->m0FD88 + i & 3) == 0) {
            local_38 = i_this->mParts[i].m00D4;
            if (!gr_check(i_this, &local_38)) {
                dComIfGp_particle_setSimple(dPa_name::ID_AK_SN_O_KGTCOMMONHAMON00, &local_38, 0xFF, g_whiteColor, g_whiteColor, 0);
...
static void move_splash_set(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz local_28;
    ...
    if ((i_this->m0FD88 + 2 & 3) == 0) {
        ...
        if (!gr_check(i_this, &local_28)) {
            dComIfGp_particle_setSimple(dPa_name::ID_AK_SN_O_KGTCOMMONSPLASH01, &local_28, 0xFF, g_whiteColor, g_whiteColor, 0);
        }
    }
    for (s32 i = 0; i < 9; i++) {
        if ((i_this->m0FD88 + i & 7) == 0) {
            local_28 = i_this->mParts[i].m00D4;
            if (!gr_check(i_this, &local_28)) {
                dComIfGp_particle_setSimple(dPa_name::ID_AK_SN_O_KGTCOMMONSPLASH00, &local_28, 0xFF, g_whiteColor, g_whiteColor, 0);
            }
        }
    }
}
...
static void drop_eff_set(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz local_18;
...
static void part_control(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    f32 fVar1;
    f32 fVar2;
    f32 dVar11;
    ...
    s32 i = 1;
    part_s3* ppVar4 = &i_this->mParts[1];
    ...
    for (i = 1; i < 10; i++, ppVar4++) {
        fVar1 = (70.0f + REG0_F(19)) * ppVar4[-1].m00F4;
        ...
        ppVar4[-1].m00E0 = -cM_atan2s(dVar11, std::sqrtf(SQUARE(fVar1) + SQUARE(fVar2)));
...
static s32 checkWall(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    f32 fVar5;
    cXyz local_ac;
    cXyz cStack_b8;
...
static s32 checkGround(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
...
static void pos_move(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz local_1c;
    cXyz local_28;
...
static void move0(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    ...
    if ((int)i_this->m0FD90 != 0) {
        ...
        if (i_this->m0FDB0 != 0) {
            ...
            sVar3 = sVar3 + (s16)((REG0_F(18) + 4000.0f) * dVar6);
        }
    ...
    if (i_this->m0FDAE == 0) {
        i_this->m0FDAE = cM_rndF(50.0f) + 30.0f;
        bVar1 = true;
    }
    if (bVar1) {
        if (actor->health == 3) {
...
static void mahi(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    switch (i_this->m0FD8C) {
        ...
        // fallthrough
        case 1:
            i_this->mpMorf->play(&actor->current.pos, 0, 0);
...
static void damage(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz local_34;
    ...
    actor->current.angle.x = -cM_atan2s(local_34.y, std::sqrtf(SQUARE(local_34.x) + SQUARE(local_34.z)));
    if (actor->current.angle.x > 0x3000) {
        actor->current.angle.x = 0x3000;
    } else if (actor->current.angle.x < -0x3000) {
        actor->current.angle.x = -0x3000;
    }
    checkGround(i_this);
    if ((checkWall(i_this)) && (i_this->m0FDAA[1] == 0)) {
...
static void end(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    cXyz local_44;
    ...
    switch (i_this->m0FD8C) {
        ...
        case 5:
            ...
            cLib_addCalc0(&i_this->m100A0, 1.0f, REG0_F(10) + DEMO_SELECT(100.0f, 50.0f));
            break;
    }
    pos_move(i_this);
    local_44 = actor->current.pos - actor->old.pos;
    actor->current.angle.x = -cM_atan2s(local_44.y, std::sqrtf(SQUARE(local_44.x) + SQUARE(local_44.z)));
    if (actor->current.angle.x > 0x3000) {
        actor->current.angle.x = 0x3000;
    } else if (actor->current.angle.x < -0x3000) {
        actor->current.angle.x = -0x3000;
    ...
    i_this->m0FDFC.OffAtSetBit();
#if VERSION == VERSION_DEMO
    cLib_addCalc0(&actor->current.pos.x, 0.05f, 30.0f);
    cLib_addCalc0(&actor->current.pos.z, 0.05f, 30.0f);
#else
    cLib_addCalc2(&actor->current.pos.x, -15.0f, 0.05f, 30.0f);
    cLib_addCalc2(&actor->current.pos.z, 375.17f, 0.05f, 30.0f);
#endif
}

/* 00002524-00002C18       .text damage_check__FP10bgn3_class */
static void damage_check(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    daPy_py_c* player = (daPy_py_c*)dComIfGp_getPlayer(0);
    char cVar5;
    ...
    if (i_this->m0FDB4 == 0) {
        ...
        if (i_this->m0FDFC.ChkTgHit()) {
            atInfo.mpObj = i_this->m0FDFC.GetTgHitObj();
            ...
            if (atInfo.mResultingAttackType == 9) {
                ...
                if ((dCcD_GetGObjInf(atInfo.mpObj)->GetAtSpl()) == 1) {
                    bVar10 = 4;
            ...
            if (bVar10 != 0) {
                ...
                if (bVar10 >= 4) {
                    ...
                    dComIfGp_particle_set(dPa_name::ID_AK_JN_OK, i_this->m0FDFC.GetTgHitPosP(), &local_78, &local_58);
        ...
        if (i_this->mParts[8].mPart3Sph.ChkTgHit()) {
            i_this->m0FDB4 = 0x14;
#if VERSION > VERSION_DEMO
            i_this->m0FD9E = 1;
#endif
            ...
            if (actor->health != 0) {
                ...
                if (actor->health == 0) {
                    mDoAud_bgmStop(30);
                    emitter = dComIfGp_particle_set(dPa_name::ID_AK_SN_KGTBREAKWEAKPOINT00, &i_this->m10054);
                    if (emitter != NULL) {
                        JGeometry::TVec3<f32> scale(dVar11, dVar11, dVar11);
                        emitter->setGlobalScale(scale);
                    }
                    emitter = dComIfGp_particle_set(dPa_name::ID_AK_SN_KGTBREAKWEAKPOINT01, &i_this->m10054);
                    if (emitter != NULL) {
                        JGeometry::TVec3<f32> scale(dVar11, dVar11, dVar11);
                        emitter->setGlobalScale(scale);
                    }
                    ...
                    i_this->m0FDB4 = 10000;
#if VERSION == VERSION_DEMO
                    fopAcM_monsSeStart(actor, JA_SE_CV_BGN_HIT_LAST, 0);
                    ...
                    emitter = dComIfGp_particle_set(dPa_name::ID_AK_SN_KGTHITWEAKPOINT00, &i_this->m10054);
                    if (emitter != NULL) {
                        JGeometry::TVec3<f32> scale(dVar11, dVar11, dVar11);
                        emitter->setGlobalScale(scale);
                    }
        ...
        if (i_this->m0FF28.ChkTgHit()) {
            atInfo.mpObj = i_this->m0FF28.GetTgHitObj();
            cVar5 = 1;
        }
        for (s32 i = 0; i < 8; i++) {
            if (i_this->mParts[i].mPart3Sph.ChkTgHit()) {
                atInfo.mpObj = i_this->mParts[i].mPart3Sph.GetTgHitObj();
                cVar5 = 2;
            }
        }
...
static void ke_control(bgn3_ke_s* param_1, int, f32 param_3) {
    f32 fVar_x;
    f32 fVar_y;
    f32 fVar_z;
    cXyz local_90;
    ...
    s32 i = 1;
    cXyz* pcVar3 = &param_1->m00[1];
    local_90.x = 0.0f;
    local_90.y = 0.0f;
    local_90.z = param_3;
    for (i = 1; i < 5; i++, pcVar3++) {
        f32 fVar1 = (0.1f * (s32)(5 - i));
        local_a8.x = param_1->m78.x * fVar1;
        local_a8.y = param_1->m78.y * fVar1;
        local_a8.z = param_1->m78.z * fVar1;
        fVar_y = local_a8.x + (pcVar3->x - pcVar3[-1].x);
        fVar_z = local_a8.z + (pcVar3->z - pcVar3[-1].z);
        fVar_x = (l_HIO.m4C + (pcVar3->y + local_a8.y)) - pcVar3[-1].y;
        s16 iVar4 = -cM_atan2s(fVar_x, fVar_z);
        int iVar5 = cM_atan2s(fVar_y, std::sqrtf(SQUARE(fVar_x) + SQUARE(fVar_z)));
...
static void ke_move(mDoExt_3DlineMat0_c* param_1, bgn3_ke_s* param_2, int param_3, f32 param_4) {
    cXyz* pcVar3;
...
static void move_se_set(bgn3_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    u32 uVar2;
    cXyz local_28;
...
static void move(bgn3_class* i_this) {
#if VERSION > VERSION_DEMO
    ...
    cMtx_YrotS(*calc_mtx, actor->shape_angle.y);
    cMtx_XrotM(*calc_mtx, actor->shape_angle.x);
    local_d8.set(0.0f, 0.0f, REG0_F(3) + -300.0f);
    MtxPosition(&local_d8, &local_e4);
    i_this->m0FD7C = actor->current.pos + local_e4;
    for (s32 i = 0; i < 9; i++) {
        i_this->mParts[i].m00F4 = (REG0_F(11) + 1.7f) * size_d[i];
    }
    part_control(i_this);
    i_this->mpMorf->play(&actor->current.pos, 0, 0);
    ...
    if (i_this->mArrowHitEffectTimer != 0) {
        ...
        fVar1 = i_this->mArrowHitEffectTimer * 100.0f * (REG8_F(8) + 0.001f);
        if (i_this->mpArrowHitEmitter1 == NULL) {
            i_this->mpArrowHitEmitter1 = dComIfGp_particle_set(dPa_name::ID_AK_JN_CCTHUNDER00, &local_e4);
        } else {
            i_this->mpArrowHitEmitter1->setGlobalTranslation(local_e4.x, local_e4.y, local_e4.z);
        ...
        if (i_this->mpArrowHitEmitter2 == NULL) {
            local_d8.setall(fVar1);
            i_this->mpArrowHitEmitter2 = dComIfGp_particle_set(dPa_name::ID_AK_JN_CCTHUNDER01, &local_e4);
        } else {
            i_this->mpArrowHitEmitter2->setGlobalTranslation(local_e4.x, local_e4.y, local_e4.z);
            JGeometry::TVec3<f32> scale(fVar1, fVar1, fVar1);
    ...
    for (s32 i = 0; i < 40; i++) {
        MtxPush();
        cMtx_XrotM(*calc_mtx, -((REG0_S(5) + 0x3FFF & (s32)i * (REG0_S(4) + 1000)) - (REG0_S(6) + 0x2000)));
        cMtx_ZrotM(*calc_mtx, -((REG0_S(8) + 0xFFF & (s32)i * (REG0_S(7) + 5000)) - (REG0_S(9) + 0x800)));
        local_d8.y = (100.0f + REG13_F(13));
    ...
    dVar22 = i_this->m100A8 * (REG0_F(14) + 500.0f);
    s16 y = dVar22 * cM_ssin(i_this->m0FD88 * 0x2100);
    s16 x = dVar22 * cM_scos(i_this->m0FD88 * 0x2300);
...
#endif
    part_s3* ppVar18 = i_this->mParts;
    for (s32 i = 0; i < 9; i++, ppVar18++) {
        ...
        if (i == 8) {
            if (actor->health == 3) {
                ppVar18->mpPart3Model = i_this->mpJyakutenAModel;
            } else if (actor->health == 2) {
                ppVar18->mpPart3Model = i_this->mpJyakutenBModel;
            } else if (actor->health == 1) {
                ppVar18->mpPart3Model = i_this->mpJyakutenCModel;
            } else {
                ppVar18->mpPart3Model = NULL;
            }
            i_this->m10054 = local_e4;
        }
        if (ppVar18->mpPart3Model != NULL) {
            ppVar18->mpPart3Model->setBaseTRMtx(*calc_mtx);
        }
        ppVar18->mPart3Sph.SetC(local_e4);
        ppVar18->mPart3Sph.SetR(ppVar18->m00F4 * (100.0f + REG0_F(12)));
        if (bVar5) {
            ppVar18->mPart3Sph.OnAtSetBit();
        } else {
            ppVar18->mPart3Sph.OffAtSetBit();
        }
        dComIfG_Ccsp()->Set(&ppVar18->mPart3Sph);
        MtxPull();
        if ((i < 7) && (l_HIO.m3E != 0)) {
            for (s32 j = 0; j < ke_max[i]; j++) {
                MtxPush();
                cMtx_XrotM(*calc_mtx, (REG0_S(5) + 0x3FFF & j * (REG0_S(4) + 1000)) - (REG0_S(6) + 0x2000));
                cMtx_ZrotM(*calc_mtx, (REG0_S(8) + 0xFFF & j * (REG0_S(7) + 5000)) - (REG0_S(9) + 0x800));
                local_d8.y = 100.0f + REG13_F(13);
                local_d8.y = ppVar18->m00F4 * (60.0f + REG13_F(12));
                MtxPosition(&local_d8, ppVar18->mPart3Hairs[j].m00);
                local_d8.y = l_HIO.m40;
                MtxPosition(&local_d8, &ppVar18->mPart3Hairs[j].m78);
                ppVar18->mPart3Hairs[j].m78 -= local_e4;
                ke_move(&ppVar18->mPart3LineMat, &ppVar18->mPart3Hairs[j], j, ppVar18->m00F4 * l_HIO.m44);
                MtxPull();
            }
        }
#if VERSION > VERSION_DEMO
        if (i_this->m0FD9E == fl_check_d[i]) {
            ppVar18->mPart3ArrowHitFlashTimer = 50;
            ppVar18->mPart3ArrowHitEffectTimer = 100;
        }
        if (ppVar18->mPart3ArrowHitFlashTimer != 0) {
            ppVar18->mPart3ArrowHitFlashTimer--;
        }
        if (ppVar18->mPart3ArrowHitEffectTimer != 0) {
            ppVar18->mPart3ArrowHitEffectTimer--;
            fVar1 = (REG8_F(0) + 0.04f) * (ppVar18->m00F4 * ppVar18->mPart3ArrowHitEffectTimer);
            if (ppVar18->mpPart3ArrowHitEmitter1 == NULL) {
                ppVar18->mpPart3ArrowHitEmitter1 = dComIfGp_particle_set(dPa_name::ID_AK_JN_CCTHUNDER00, &ppVar18->m00D4);
            } else {
                ppVar18->mpPart3ArrowHitEmitter1->setGlobalTranslation(ppVar18->m00D4.x, ppVar18->m00D4.y, ppVar18->m00D4.z);
                JGeometry::TVec3<f32> scale(fVar1, fVar1, fVar1);
                ppVar18->mpPart3ArrowHitEmitter1->setGlobalScale(scale);
            }
            if (ppVar18->mpPart3ArrowHitEmitter2 == NULL) {
                local_d8.setall(fVar1);
                ppVar18->mpPart3ArrowHitEmitter2 = dComIfGp_particle_set(dPa_name::ID_AK_JN_CCTHUNDER01, &ppVar18->m00D4);
            } else {
                ppVar18->mpPart3ArrowHitEmitter2->setGlobalTranslation(ppVar18->m00D4.x, ppVar18->m00D4.y, ppVar18->m00D4.z);
                JGeometry::TVec3<f32> scale(fVar1, fVar1, fVar1);
                ppVar18->mpPart3ArrowHitEmitter2->setGlobalScale(scale);
            }
        } else {
            if (ppVar18->mpPart3ArrowHitEmitter1 != NULL) {
                ppVar18->mpPart3ArrowHitEmitter1->becomeInvalidEmitter();
                ppVar18->mpPart3ArrowHitEmitter1 = NULL;
            }
            if (ppVar18->mpPart3ArrowHitEmitter2 != NULL) {
                ppVar18->mpPart3ArrowHitEmitter2->becomeInvalidEmitter();
                ppVar18->mpPart3ArrowHitEmitter2 = NULL;
            }
        }
#endif
    }
    ...
    MTXCopy(i_this->m002CC->getBaseTRMtx(), *calc_mtx);
    local_d8.setall(0.0f);
...
static s32 ki_check(bgn3_class* i_this) {
    ki_all_count = 0;
...
static BOOL daBgn3_Execute(bgn3_class* i_this) {
    ...
    if (i_this->m0FD8A == 10) {
        ...
        i_this->m0FD8A = 0;
        i_this->m0FD90 = DEMO_SELECT(90, 180);
        i_this->m0FDB0 = DEMO_SELECT(90, 180);
    }
...
static BOOL useHeapInit(fopAc_ac_c* a_this) {
    ...
    if (!i_this->mLineMat.init(0x28, 5, 0)) {
        return FALSE;
    }
    for (s32 i = 0; i < 9; i++) {
        i_this->mParts[i].mpPart3Model = mDoExt_J3DModel__create(modelData, 0, 0x11020203);
        if (i_this->mParts[i].mpPart3Model == 0) {
            return false;
        }
        if ((i < 7) && (!i_this->mParts[i].mPart3LineMat.init(ke_max[i], 5, 0))) {
            return false;
        }
    }
...
static cPhs_State daBgn3_Create(fopAc_ac_c* a_this) {
    ...
    if (res == cPhs_COMPLEATE_e) {
        ...
        i_this->m0FF28.SetStts(&i_this->mStts);
        i_this->m0FF28.OffAtSetBit();
        i_this->m0FDFC.Set(cc_sph_src);
        i_this->m0FDFC.SetStts(&i_this->mStts);
        i_this->m0FDFC.OffAtSetBit();
        for (s32 i = 0; i < 9; i++) {
            if (i == 8) {
                i_this->mParts[i].mPart3Sph.Set(core_sph_src);
            } else {
                i_this->mParts[i].mPart3Sph.Set(cc_sph_src);
            }
            i_this->mParts[i].mPart3Sph.SetStts(&i_this->mStts);
            i_this->mParts[i].mPart3Sph.OffAtSetBit();
        }
        a_this->health = 3;
        a_this->max_health = 3;
        ke_color.r = l_HIO.m50;
        ke_color.g = l_HIO.m52;
        ke_color.b = l_HIO.m54;
        ke_color.a = 0xFF;
#if VERSION > VERSION_DEMO
        for (s32 i = 0; i < 9; i++) {
            i_this->mParts[i].mPart3TevStr = a_this->tevStr;
        }
#endif
