...
void ke_control(bgn3_ke_s* param_1, int param_2, float param_3) {
    ...
    local_9c.y = 0.0f;
    local_9c.z = param_3;
    for (s32 i = 1; i < 5; i++, pcVar3++) {
        fVar1 = (0.1f * (s32)(5U - i));
        dVar9 = (param_1->m78.x * fVar1 + pcVar3->x) - param_1[-1].m78.x;
        fVar2 = (param_1->m78.z * fVar1 + pcVar3->z) - param_1[-1].m78.z;
        fVar1 = (l_HIO.m4C + pcVar3->y + param_1->m78.y * fVar1) - param_1[-1].m78.y;
        s16 iVar4 = -cM_atan2s(fVar1, fVar2);
        int iVar5 = cM_atan2s(dVar9, std::sqrtf((fVar1 * fVar1) + (fVar2 * fVar2)));
        cMtx_XrotS(*calc_mtx, (int)iVar4);
        cMtx_YrotM(*calc_mtx, iVar5);
        MtxPosition(&local_9c, &local_a8);
        pcVar3->x = param_1[-1].m78.x + local_a8.x;
        pcVar3->y = param_1[-1].m78.y + local_a8.y;
        pcVar3->z = param_1[-1].m78.z + local_a8.z;
    }
}
...
void move(bgn3_class* i_this) {
    ...
    for (i = 0; i < 9; i++, ppVar18++) {
        ...
        MtxPush();
        fVar1 = ppVar18->m00F4;
        MtxScale(fVar1, fVar1, fVar1, true);
        local_d8.setall(0.0f);
        MtxPosition(&local_d8, &local_e4);
        if (i == 8) {
            if (actor->health == 3) {
    ...
    for (s32 i = 0; i < 0x3C; i++) {
        fVar1 = i_this->m100A0 * (cM_ssin(cM_rad2s(0.053247336f * (s32)i)) * (0.01666667f * (s32)(0x3b - i)));
