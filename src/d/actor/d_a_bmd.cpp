...
void mk_draw(bmd_class* i_this) {
    if (i_this->m2DC != 0) {
        ...
        g_env_light.setLightTevColorType(i_this->m2CC, &i_this->tevStr);
...
static BOOL daBmd_Draw(bmd_class* i_this) {
    ...
    g_env_light.setLightTevColorType(pJVar11, &i_this->tevStr);
    if (i_this->mB70 != 0) {
    ...
    if (i_this->m306 < 100) {
        g_env_light.setLightTevColorType(i_this->m2C0->getModel(), &i_this->tevStr);
        i_this->m2C0->entryDL();
    } else if (i_this->m306 < 0x6e) {
        g_env_light.setLightTevColorType(i_this->m2C4->getModel(), &i_this->tevStr);
        i_this->m2C4->entryDL();
    }
    mk_draw(i_this);
    dComIfGd_setSimpleShadow2(&i_this->current.pos, i_this->mAcch.GetGroundH(), 400.0f, i_this->mAcch.m_gnd, 0, 1.0f, &dDlst_shadowControl_c::mSimpleTexObj);
    ...
    dSnap_RegistFig(DSNAP_TYPE_BMD, i_this, 1.0f, 1.0f, 1.0f);
    return TRUE;
}
...
void damage(bmd_class* i_this) {
    ...
    case 1:
        ...
        if (i_this->current.pos.y <= i_this->m328 + 5.0f) {
            ...
            i_this->m93C = sVar5;
            i_this->mBD8 = 2000.0f;
            dComIfGp_particle_set(dPa_name::ID_SCENE_80F3, &i_this->current.pos);
        ...
        break;
    case 2:
        if (((s16)(REG0_S(1) + 10) == (s16)i_this->mpMorf->mFrameCtrl.getFrame()) && (fopAcM_searchPlayerDistance(i_this) < (REG0_F(8) + 400.0f))) {
        ...
        if ((s16)iVar7 == 30) {
            for (s32 i = 0; i < (s32)ARRAY_SIZE(jno); i++) {
                pJVar4 = dComIfGp_particle_setToon(dPa_name::ID_SCENE_A0F4, &i_this->current.pos, NULL, NULL, 0xB9, &i_this->mA90[i], fopAcM_GetRoomNo(i_this));
                if (pJVar4 != NULL) {
    ...
    case 3:
        ...
        if (i_this->m308[0] == 0) {
            ...
            i_this->m304 = 2;
            mDoAud_seStart(JA_SE_CM_BKM_FLW_WAVING, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        cLib_addCalc2(&i_this->mBDC, 1.0f, 1.0f, 0.1f);
        break;
    case 4:
        iVar7 = i_this->mpMorf->mFrameCtrl.getFrame();
        if ((s16)iVar7 == 23) {
            mDoAud_seStart(JA_SE_CM_BKM_BODY_CLOSE, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        iVar7 = i_this->mpMorf->mFrameCtrl.getFrame();
        if ((s16)iVar7 == 27) {
            dComIfGp_particle_set(dPa_name::ID_SCENE_8100, &i_this->current.pos);
            if (dComIfGp_getStartStageName()[0] == 'X') {
    ...
    cLib_addCalcAngleS2(&i_this->shape_angle.x, 0, 0x10, 0x200);
    cLib_addCalcAngleS2(&i_this->shape_angle.z, 0, 0x10, 0x200);
    mDoMtx_YrotS(*calc_mtx, i_this->current.angle.y);
    local_40.y = 0.0f;
    local_40.x = 0.0f;
    local_40.z = i_this->speedF;
    MtxPosition(&local_40, &local_4c);
    i_this->current.pos.x += local_4c.x;
    i_this->current.pos.z += local_4c.z;
    i_this->current.pos.y += i_this->speed.y;
    i_this->speed.y = i_this->speed.y + i_this->gravity;
    if (i_this->current.pos.y < i_this->m328) {
        i_this->current.pos.y = i_this->m328;
        if (i_this->speed.y < -20.0f) {
            dComIfGp_getVibration().StartShock(REG0_S(2) + 5, -0x21, cXyz(0.0f, 1.0f, 0.0f));
            mDoAud_seStart(JA_SE_CM_BKM_BODY_FALL, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
            i_this->speed.y = 10.0f;
            dComIfGp_particle_set(dPa_name::ID_SCENE_80F1, &i_this->current.pos);
            dComIfGp_particle_setToon(dPa_name::ID_SCENE_A0F2, &i_this->current.pos, NULL, NULL, 0xB9, &i_this->mA90[5], (u8)i_this->current.roomNo);
        } else {
            i_this->speed.y = 0.0f;
        }
        cLib_addCalc0(&i_this->speedF, 1.0f, 0.2f);
    }
    if ((bVar1) && (fopAcM_searchPlayerDistance(i_this) < (REG8_F(8) + 200.0f))) {
        i_this->mMode = 5;
        i_this->m302 = 0;
...
void eat(bmd_class* i_this) {
    ...
    case 1:
        ...
        if (i_this->mpMorf->isStop()) {
            ...
            mDoAud_seStart(JA_SE_CM_BKM_BODY_EATING, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        break;
    case 2:
        if (i_this->m308[0] != 0) {
        ...
        mDoAud_seStart(JA_SE_CM_BKM_LINK_OUT, &apdVar3->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        return;
    case 3:
        if (i_this->mB78 == REG0_S(2) + 0x50) {
            dComIfGp_particle_set(dPa_name::ID_SCENE_80F8, &i_this->current.pos);
            dComIfGp_particle_set(dPa_name::ID_SCENE_80F7, &i_this->current.pos);
            i_this->m314 = 100;
        }
        if (i_this->mB78 == 0x55) {
            local_30 = i_this->current.pos;
    ...
    case 5:
        if (i_this->mB76 > (s16)(REG0_S(7) + 0x82)) {
            ...
            dVar5 = fopAcM_searchPlayerDistanceXZ(i_this);
            if (dVar5 < 300.0f) {
...
void move1(bmd_class* i_this) {
    ...
    local_28.y = 0.0f;
    local_28.x = 0.0f;
    local_28.z = i_this->speedF;
    MtxPosition(&local_28, &i_this->speed);
    i_this->current.pos += i_this->speed;
    cLib_addCalc2(&i_this->m324, cM_ssin(i_this->m2FE * 400) * 20.0f, 0.1f, 10.0f);
    cLib_addCalcAngleS2(&i_this->shape_angle.x, cM_ssin(i_this->m2FE * 500) * 700.0f, 0x10, 0x80);
    cLib_addCalcAngleS2(&i_this->shape_angle.z, cM_scos(i_this->m2FE * 300) * 700.0f, 0x10, 0x80);
}
...
void start(bmd_class* i_this) {
    ...
    case 1:
        i_this->mBDC = 1.0f;
        sVar2 = fopAcM_searchPlayerAngleY(i_this);
        i_this->m93C = sVar2;
        ...
        i_this->m2E0 += i_this->current.pos;
        i_this->m2FA = i_this->m93C;
        dVar4 = fopAcM_searchPlayerDistance(i_this);
        if (dVar4 < 500.0f) {
            i_this->m302 = 2;
            i_this->mB74 = 5;
            i_this->m940 = -0x4000;
    ...
    case 3:
        ...
        if (i_this->mB76 == 0x96) {
            ...
            i_this->mBD8 = 2000.0f;
            mDoAud_seStart(JA_SE_CM_BKM_FLW_TO_BUD, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        if (i_this->mB76 == 0xb1) {
            dComIfGp_particle_set(dPa_name::ID_SCENE_8100, &i_this->current.pos);
        }
        ...
        if (i_this->mB76 == 0xd2) {
            i_this->m332 = 5;
            mDoAud_seStart(JA_SE_CM_BKM_VINE_OUT, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        if (i_this->mB76 == 0x1c2) {
            i_this->m332 = 6;
            mDoAud_seStart(JA_SE_CM_BKM_ATKVINE_ENTER, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        if (i_this->mB76 == 0x168) {
            mDoAud_seStart(JA_SE_CM_BKM_BODY_UP, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        break;
    }
}
...
void end(bmd_class* i_this) {
    ...
    if (i_this->mB76 == REG0_S(4) + 0x17c) {
        ...
        mDoAud_seStart(JA_SE_CM_BKM_END_FLW_WAVING, &i_this->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
    }
    if (i_this->mB76 == REG0_S(5) + 0x1c2) {
        anm_init(i_this, BMD_BCK_HANA_DEAD3, 10.0f, J3DFrameCtrl::EMode_NONE, 1.0f, -1);
        mDoAud_seStart(JA_SE_CM_BKM_END_FLW_LIFTUP, &i_this->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        i_this->mBD8 = 2000.0f;
        i_this->m332 = 9;
        for (s32 i = 0; i < (s32)ARRAY_SIZE(jno); i++) {
            pJVar3 = dComIfGp_particle_set(dPa_name::ID_SCENE_80FF, &i_this->current.pos);
            if (pJVar3 != NULL) {
    ...
    if (i_this->mB76 == 0x230) {
        mDoAud_seStart(JA_SE_CM_BKM_END_FLW_DIE, &i_this->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
    }
    if (i_this->mB76 == REG0_S(6) + 0x276) {
        g_dComIfG_gameInfo.save.getMemory().getBit().onStageBossEnemy();
        fopAcM_createWarpFlower(&i_this->current.pos, 0, fopAcM_GetRoomNo(i_this), 0);
        i_this->m302 = 2;
        i_this->mB71 = 0;
    }
}
...
void core_damage_check(bmd_class* i_this) {
    ...
    if (i_this->mCoreSph.ChkTgHit() != 0) {
        i_this->m310 = 8;
        fopAcM_monsSeStart(i_this, JA_SE_CV_BKM_DAMAGE, 0);
        i_this->m306 = 1;
        local_2c.pParticlePos = NULL;
        local_2c.mpObj = i_this->mCoreSph.GetTgHitObj();
        local_2c.pParticlePos = i_this->mCoreSph.GetTgHitPosP();
        fopAc_ac_c* pfVar4 = cc_at_check(i_this, &local_2c);
        if (pfVar4 != NULL) {
            local_38.x = i_this->eyePos.x - pfVar4->current.pos.x;
            local_38.z = i_this->eyePos.z - pfVar4->current.pos.z;
    ...
    local_38 = pPlayer->current.pos - i_this->current.pos;
    local_38.y = 0.0f;
    if ((pPlayer->checkForestWaterUse()) && (i_this->m304 != 0)) {
        ...
        if (!bVar2) {
            mDoAud_seStart(JA_SE_LK_LAST_HIT, &i_this->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
    }
    i_this->m306 = 100;
    i_this->mMode = 0xb;
...
void core_move(bmd_class* i_this) {
    ...
    case 2:
        ...
        i_this->m306 = 0x65;
        i_this->m924 = i_this->current.pos;
        mDoMtx_YrotS(*calc_mtx, i_this->shape_angle.y + (REG0_S(4) + 3000));
        local_40.y = 0.0f;
        local_40.x = 0.0f;
        ...
        i_this->m93C = 0;
        i_this->m308[2] = 0;
        fopAcM_monsSeStart(i_this, JA_SE_CV_BKM_LAST_DAMAGE, 0);
        break;
    case 100:
        break;
    case 101:
        ...
        if (i_this->m924.y <= (i_this->m328 - 10.0f) + REG0_F(13)) {
            ...
            i_this->m930.x = 0.0f;
            bVar5 = true;
            fopAcM_monsSeStart(i_this, JA_SE_CV_BKM_FALLDOWN, 0);
            mDoAud_seStart(JA_SE_CM_BKM_END_CORE_DROP, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        break;
    case 0x66:
        ...
        if (iVar1 == 14) {
            bVar5 = true;
            fopAcM_monsSeStart(i_this, JA_SE_CV_BKM_JITABATA, 0);
        }
        i_this->mA8C = 3;
    ...
    case 0x69:
        iVar1 = i_this->m2C4->mFrameCtrl.getFrame();
        if (iVar1 == 37) {
            pJVar7 = dComIfGp_particle_set(dPa_name::ID_SCENE_80FE, &i_this->current.pos);
            if (pJVar7 != NULL) {
                pJVar7->setGlobalRTMatrix(i_this->m2C4->getModel()->getAnmMtx(5));
            }
            fopAcM_monsSeStart(i_this, JA_SE_CV_BKM_DIE, 0);
        }
        if (i_this->m2C4->isStop()) {
            local_58 = i_this->m924;
            local_58.z += REG0_F(2) + 80.0f;
            fopAcM_createDisappear(i_this, &local_58, 0xF, daDisItem_HEART_CONTAINER_e, 0xFF);
            i_this->m306 = 0x6e;
            i_this->m308[2] = 0xaa;
        }
        i_this->m924 += i_this->m930;
    ...
    if (i_this->m306 < 100) {
        ...
        i_this->m2C0->play(&i_this->eyePos, 0, 0);
        i_this->m2C0->calc();
        ...
        local_40.y = -20.0f;
        local_40.z = 30.0f;
        MtxPosition(&local_40, &i_this->eyePos);
        ...
        pJVar11->setBaseTRMtx(mDoMtx_stack_c::now);
        i_this->m2C4->play(&i_this->eyePos, 0, 0);
        i_this->m2C4->calc();
    ...
    i_this->mCoreSph.SetC(i_this->eyePos);
    i_this->mCoreSph.SetR(50.0f);
    dComIfG_Ccsp()->Set(&i_this->mCoreSph);
    if (bVar5) {
        dComIfGp_particle_setToon(dPa_name::ID_SCENE_A0FD, &i_this->m924, NULL, NULL, 0xB9, &i_this->mA90[6], (u8)i_this->current.roomNo);
        mDoAud_seStart(JA_SE_CM_BKM_END_CORE_LEAP, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
    }
}

/* 00002EDC-00002F40       .text mk_voice_set__FP9bmd_classUl */
void mk_voice_set(bmd_class* i_this, u32 param_2) {
    mDoAud_monsSeStart(param_2, &i_this->m2E0, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
}
    ...
    if (i_this->m2DC != 0) {
        ...
        case 10:
            ...
            if (iVar1 == 6) {
                ...
                local_28.y += 10.0f;
                pJVar3 = dComIfGp_particle_setToon(dPa_name::ID_SCENE_A0FD, &local_28, NULL, NULL, 0xB9, &i_this->mA90[6], (u8)i_this->current.roomNo);
                if (pJVar3 != NULL) {
...
void damage_check(bmd_class* i_this) {
    if ((i_this->m331 == 0) && (i_this->m312 == 1)) {
        i_this->mMode = 3;
        i_this->m302 = 0;
        fopAcM_monsSeStart(i_this, JA_SE_CV_BKM_CUT_VINE_ALL, 0);
    }
}
...
void wait(bmd_class* i_this) {
    ...
    case -1:
        break;
    case 0:
        i_this->m336 = i_this->shape_angle.y;
        i_this->m308[1] = 100;
        i_this->m308[2] = 0x28;
        if (i_this->m308[0] == 0) {
            i_this->m302 = 0;
    ...
    case 1:
        ...
        cLib_addCalc2(&i_this->speedF, i_this->m331 * 0.15f, 1.0f, 0.1f);
        local_30 = i_this->m318 - i_this->current.pos;
        cLib_addCalcAngleS2(&i_this->current.angle.y, cM_atan2s(local_30.x, local_30.z), 0x10, 0x800);
    ...
    case 2:
        i_this->m330 = 1;
        cLib_addCalc0(&i_this->speedF, 1.0f, 0.1f);
        if (i_this->m308[0] == 0) {
            i_this->m302 = 0;
        }
        break;
    ...
    if (i_this->m302 >= 0) {
        ...
        cLib_addCalcAngleS2(&i_this->shape_angle.y, i_this->m336, 0x20, i_this->m338);
        cLib_addCalc2(&i_this->m338, 64.0f, 1.0f, 1.0f);
        if (i_this->m308[2] == 0) {
            dVar6 = fopAcM_searchPlayerDistance(i_this);
            if (dVar6 < 1300.0f) {
...
void move(bmd_class* i_this) {
    ...
    if (i_this->mMode != 3) {
        ...
        cLib_addCalc2(&i_this->current.pos.y, fVar1 + (i_this->home.pos.y + i_this->m324), 0.1f, 5.0f);
        damage_check(i_this);
    } else {
        i_this->m324 = 0.0f;
    }
...
void eff_cont(bmd_class* i_this) {
    for (s32 i = 0; i < (s32)ARRAY_SIZE(i_this->mA7C); i++) {
        if (i_this->mA7C[i] != NULL) {
            ...
            i_this->mA7C[i] = dComIfGp_particle_set(eff_name[i], &i_this->current.pos);
        }
    }
}
...
void demo_camera(bmd_class* i_this) {
    ...
    case 3:
        ...
        if ((int)i_this->mB78 < REG0_S(0) + 0x55) {
            cLib_addCalc2(&i_this->mB88.x, i_this->current.pos.x, 0.1f, 200.0f);
            cLib_addCalc2(&i_this->mB88.y, apdVar5->current.pos.y + 300.0f + REG0_F(5), 0.2f, 200.0f);
            cLib_addCalc2(&i_this->mB88.z, i_this->current.pos.z, 0.2f, 200.0f);
            local_44.x = 0.0f;
            local_44.y = REG0_F(7) + 100.0f;
            local_44.z = REG0_F(6) - 1100.0f;
            MtxPosition(&local_44, &local_50);
            cLib_addCalc2(&i_this->mB7C.x, i_this->current.pos.x + local_50.x, 0.1f, 200.0f);
            cLib_addCalc2(&i_this->mB7C.y, apdVar5->current.pos.y + local_50.y, 0.1f, 200.0f);
            cLib_addCalc2(&i_this->mB7C.z, i_this->current.pos.z + local_50.z, 0.1f, 200.0f);
        } else if ((int)i_this->mB78 < REG0_S(1) + 100) {
            cLib_addCalc2(&i_this->mB88.y, i_this->current.pos.y + 1200.0f + REG0_F(5), 0.2f, REG0_F(6) + 100.0f);
        } else {
    ...
    case 0xA:
        ...
        i_this->mB7C.y = 115.0f;
        i_this->mB7C.z = 263.0f;
        i_this->mB88 = i_this->eyePos;
        i_this->mB88.y += REG0_F(11) + 30.0f;
        i_this->m2DC = 4;
        cLib_addCalc2(&i_this->mB88.y, i_this->eyePos.y + 30.0f, 0.2f, REG0_F(4) + 5.0f);
        if (i_this->mB78 == 0x1e) {
            ...
            mDoAud_seStart(JA_SE_CM_BKM_CORE_ENTER, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
            fopAcM_monsSeStart(i_this, JA_SE_CV_BKM_ENTER, 0);
        }
        if (i_this->mB78 > 0x1e) {
    ...
    case 0xD:
        if ((i_this->mB78 >= 0x14) && (i_this->mB78 <= 0x28)) {
            ...
            cLib_addCalc2(&i_this->mB88.x, i_this->eyePos.x, 0.2f, REG0_F(4) + 10.0f);
            cLib_addCalc2(&i_this->mB88.y, i_this->eyePos.y + 10.0f + REG0_F(14), 0.2f, REG0_F(4) + 10.0f);
            cLib_addCalc2(&i_this->mB88.z, i_this->eyePos.z, 0.2f, REG0_F(4) + 10.0f);
            if (i_this->m2C0->isStop()) {
                ...
                i_this->m308[0] = 10;
                fopAcM_monsSeStart(i_this, JA_SE_CV_BKM_LAUGH, 0);
                i_this->mB74 = 0xf;
                i_this->mB9C = REG0_F(8) + 55.0f;
                i_this->mB76 = REG0_S(7) + 0x6e;
                i_this->mB88 = i_this->current.pos;
                i_this->mB88.y = REG0_F(3) + 500.0f;
                mDoMtx_YrotS(*calc_mtx, (int)i_this->shape_angle.y);
    ...
    case 0x66:
        ...
        if (i_this->m924.y < i_this->m328 + 10.0f) {
            cLib_addCalc2(&i_this->mB88.x, i_this->eyePos.x, 0.2f, 200.0f);
            cLib_addCalc2(&i_this->mB88.y, i_this->eyePos.y + 20.0f + REG0_F(5), 0.5f, 200.0f);
            cLib_addCalc2(&i_this->mB88.z, i_this->eyePos.z, 0.2f, 200.0f);
        } else {
        ...
        if (0x3c < i_this->mB76) {
            mDoMtx_YrotS(*calc_mtx, i_this->shape_angle.y + (REG0_S(7) + 3000));
            local_44.x = REG0_F(11) + 200.0f;
            local_44.y = 0.0f;
            local_44.z = 0.0f;
            MtxPosition(&local_44, &local_50);
            local_50 += i_this->current.pos;
            apdVar5->setPlayerPosAndAngle(&local_50, i_this->shape_angle.y + (REG0_S(6) + 3000));
        }
        local_44.x = 0.0f;
        local_44.y = 0.0f;
        ...
        i_this->m2E0 += i_this->current.pos;
        i_this->m2FA = i_this->shape_angle.y + (REG0_S(4) + 3000);
        break;
    case 0x67: {
        i_this->mB9C = REG0_F(8) + 55.0f;
        i_this->mB96 = i_this->shape_angle.y + REG0_S(2);
        i_this->mB94 = -8000;
        i_this->mBA4 = 1500.0f;
        i_this->mBA0 = 0.0f;
        i_this->mB74 = 0x67;
        ...
        cStack_b8.y = i_this->shape_angle.y + -0x7448;
        fopAcM_create(PROC_NPC_CB1, 0, &i_this->m2E0, fopAcM_GetRoomNo(i_this), &cStack_b8);
        i_this->m2DC = 0;
        iVar6 = (int)(i_this->mBA0 * 22384.0f);
        cLib_addCalcAngleS2(&i_this->mB96, i_this->shape_angle.y + REG0_S(2) + 0x5770, 0x10, (s16)iVar6);
        ...
        MtxPosition(&local_44, &local_50);
        i_this->mB7C = i_this->current.pos + local_50;
        i_this->mB88 = i_this->current.pos;
...
static BOOL daBmd_Execute(bmd_class* i_this) {
    ...
    bmd_kankyo(i_this);
    eff_cont(i_this);
    i_this->mWindInfluence.mPos = i_this->current.pos;
    ...
    mk_move(i_this);
    if (i_this->mB71 != 0) {
        if (i_this->current.pos.y > 100.0f) {
...
static cPhs_State daBmd_Create(fopAc_ac_c* a_this) {
    ...
    bmd_class* i_this = (bmd_class*)a_this;
    fopAcM_SetupActor(i_this, bmd_class);
    ...
    if ((!dComIfGs_isStageBossEnemy()) || (dComIfGp_getStartStageName()[0] != 'X')) {
        if ((!dComIfGs_checkGetItem(dItem_PEARL_FARORE_e)) || (REG0_S(6) != 0)) {
            ...
            local_30.y = 0.0f;
            local_30.z = 800.0f;
            fopAcM_create(PROC_NPC_CB1, 0, &local_30, fopAcM_GetRoomNo(i_this));
        }
        res = cPhs_ERROR_e;
    } else if (!fopAcM_entrySolidHeap(i_this, solidHeapCB, 0x96000)) {
        res = cPhs_ERROR_e;
    } else {
        for (s32 i = 0; i < (s32)ARRAY_SIZE(i_this->pm_bgw); i++) {
            if (dComIfG_Bgsp()->Regist(i_this->pm_bgw[i], i_this)) {
                return cPhs_ERROR_e;
            }
        }
        l_HIO.mNo = mDoHIO_createChild("森ボス", &l_HIO); // Forest Boss
        i_this->health = 0xf;
        i_this->max_health = 0xf;
        for (s32 i = 0; i < 20; i++) {
        ...
        i_this->home.pos.y += REG0_F(2) + 20.0f;
        i_this->current.pos.y = i_this->home.pos.y;
        i_this->m328 = i_this->current.pos.y;
        ...
        i_this->mStts.Init(0xff, 0, i_this);
