...
void hand_draw(bmdhand_class* i_this) {
    if (i_this->m320 > 0.01f) {
        g_env_light.setLightTevColorType(i_this->m2B4->getModel(), &i_this->tevStr);
        i_this->m2B4->updateDL();
    }
    GXColor local_18 = {0xFF, 0xFF, 0xFF, 0xFF};
    i_this->m554.update(0x14, local_18, &i_this->tevStr);
...
void control1(bmdhand_class* i_this) {
    ...
    i_this->m324[0].m00 = i_this->current.pos;
    s32 i = 1;
...
void hand_calc(bmdhand_class* i_this) {
    ...
    if ((i_this->m2B8 & 0xfU) == 0) {
        ...
        local_ac.Set(&local_d0, &local_dc, i_actor);
    ...
    case 0:
        ...
        if (i_this->m30C > 0.9f) {
            ...
            mDoAud_seStart(JA_SE_CM_BKM_VINE_SET, &i_this->m2D8, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        break;
    case 1:
        mDoMtx_YrotS(*calc_mtx, i_this->current.angle.y);
        ...
        MtxPosition(&local_b8, &local_c4);
        i_this->m2E4.x = i_this->current.pos.x + local_c4.x;
        i_this->m2E4.z = i_this->current.pos.z + local_c4.z;
...
void start_hand_calc(bmdhand_class* i_this) {
    ...
    local_9c.Set(&local_c0, &local_cc, i_this);
    ...
    if (i_this->m2BC == 0) {
        ...
        i_this->m2E4 = i_this->current.pos + cStack_b4;
        i_this->m2E4.y = i_this->m2CC.y;
...
void hand_move(bmdhand_class* i_this) {
    ...
    if (boss != NULL) {
        i_this->current.angle.y = ((u16)i_this->base.mParameters & 0x1f) * -0xccc + REG8_S(4) + -13000 + boss->shape_angle.y;
        MTXCopy(boss->mpMorf->mpModel->getAnmMtx(boss_joint_d[(i_this->base.mParameters & 0x1fU)]), *calc_mtx);
        local_40.x = REG14_F(6);
        local_40.y = REG14_F(7);
        local_40.z = boss_joint_xad[(i_this->base.mParameters & 3U)];
        MtxPosition(&local_40, &i_this->current.pos);
        ...
        case 0:
            ...
            if ((i_this->m2BC > 0) && (i_this->m5CC.ChkTgHit() || (i_this->m6F8.ChkTgHit()))) {
                i_this->m2BA = 1;
                fopAcM_monsSeStart(i_this, JA_SE_CV_BKM_CUT_VINE_1, 0);
                mDoAud_seStart(JA_SE_CM_BKM_CUT_VINE, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
                local_58.setall(0.5f);
                dComIfGp_particle_set(dPa_name::ID_COMMON_0013, &i_this->eyePos, NULL, &local_58, 0xFF);
                dComIfGp_particle_set(dPa_name::ID_COMMON_0016, &i_this->eyePos, NULL, &local_58, 0xFF);
                if (boss->m331 > 0) {
                    boss->m331--;
                    boss->m312 = 3;
                }
    ...
    for (s32 i = 0; i < (s32)ARRAY_SIZE(i_this->m324); i++, pcVar9++, line_data++, line_size++) {
        ...
        *line_size = pcVar9->m18;
        ...
        if (i == 10) {
            i_this->eyePos = pcVar9->m00;
            i_this->attention_info.position = i_this->eyePos;
            i_this->m5CC.SetC(i_this->eyePos);
...
static cPhs_State daBmdhand_Create(fopAc_ac_c* a_this) {
    /* Nonmatching - "mr r26, r3" instruction in wrong place */
    ...
    bmdhand_class* i_this = (bmdhand_class*)a_this;
    fopAcM_SetupActor(i_this, bmdhand_class);
    ...
    if (!fopAcM_entrySolidHeap(i_this, solidHeapCB, 0x3040)) {
        return cPhs_ERROR_e;
    }
    if (hio_set == 0) {
        hio_set = 1;
    ...
    boss = NULL;
    i_this->m590.Init(0xFF, 0xFF, i_this);
