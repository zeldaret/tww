...
BOOL daBoomerang_c::draw() {
    ...
    if (daPy_getPlayerActorClass()->checkPlayerNoDraw() && fopAcM_GetParam(this) == Mode_0) {
    ...
    if (fopAcM_GetParam(this) == Mode_0 && daPy_getPlayerLinkActorClass()->checkFreezeState()) {
    ...
    if (mGroundY != -G_CM3D_F_INF && fopAcM_GetParam(this) == Mode_1) {
...
void daBoomerang_c::rockLineCallback(fopAc_ac_c* pHitActor) {
    if (fopAcM_GetParam(this) == Mode_0) {
        if (pHitActor) {
...
BOOL daBoomerang_c::procWait() {
    ...
    if (fopAcM_GetParam(this) == Mode_1) {
        speedF = 60.0f;
        mCps.ResetAtHit();
