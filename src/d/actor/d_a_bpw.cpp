...
void action_kougeki(bpw_class* i_this) {
    ...
    case 0x34:
        ...
        if (i_this->mpAnim->isStop()) {
            pfVar6 = (bpw_class*)fopAcM_SearchByID(i_this->m3FC);
            if (pfVar6 != NULL) {
                i_this->m400 = fopAcM_create(
                    PROC_BPW,
                    bpw_class::Actor_Type_DAMAGE_BALL_e | (bpw_class::Damage_Action_LINE_e << 8),
                    &pfVar6->m358,
    ...
    case 0x3d:
        ...
        if (i_this->mSomeCountdownTimers[1] == 0) {
            ...
            if (pfVar7 != NULL) {
                ...
                i_this->m400 = fopAcM_create(
                    PROC_BPW,
                    bpw_class::Actor_Type_DAMAGE_BALL_e | (bpw_class::Damage_Action_DAMAGE_BALL_e << 8),
                    &cStack_44,
                    fopAcM_GetRoomNo(actor),
                    &actor->current.angle
                );
...
void action_bunri_dousa(bpw_class* i_this) {
    ...
    case 0x7a:
        ...
        local_80.y = player->current.pos.y;
        local_80.z = -777.0f;
        player->setPlayerPosAndAngle(&local_80, fopAcM_searchPlayerAngleY(actor) + 0x8000);
        if (i_this->mSomeCountdownTimers[0] != 0) {
...
BOOL kantera_atari_check(bpw_class* i_this) {
    ...
    if (pcVar4->GetAtType() != AT_TYPE_SKULL_HAMMER) {
        actor->speedF = 60.0f;
        actor->current.angle.y = player->shape_angle.y;
        i_this->m3DF = 8;
        if (player->mCutType == 17) {
...
void action_b_fire_2_dousa(bpw_class* i_this) {
    ...
    local_38.x = actor->shape_angle.x;
    local_38.z = actor->shape_angle.z;
    switch (i_this->mActionState) {
    case 0x14:
        fopAcM_OnStatus(actor, fopAcStts_UNK4000_e);
        for (s32 i = 0; i < 10; i++) {
...
static BOOL damage_ball_execute(bpw_class* i_this) {
    ...
    case 0:
        ...
        if (i_this->mDamageBallCoSph.ChkAtHit() != 0) {
            player = (daPy_py_c*)dComIfGp_getPlayer(0);
            pdVar4 = i_this->mDamageBallCoSph.GetAtHitAc();
            if (pdVar4 != NULL && (pdVar4 == player)) {
                GOUEN_FIRE_HIT = 1;
            }
        }
...
static cPhs_State daBPW_Create(fopAc_ac_c* a_this) {
    ...
    if (res == cPhs_COMPLEATE_e) {
        ...
        case bpw_class::Actor_Type_BODY_e:
            ...
            draw_SUB(i_this);
            local_78.y += 0x4000;
            i_this->m3FC = fopAcM_createChild(
                PROC_BPW, fopAcM_GetID(i_this), bpw_class::Actor_Type_KANTERA_e, &i_this->mChildActorPos, fopAcM_GetRoomNo(actor), &local_78
            );
            i_this->mAttWaitTimer = 1;
            kantera_calc(i_this);
