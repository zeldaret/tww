...
static BOOL kantera_nodeCallBack(J3DNode* node, int calcTiming) {
    if (calcTiming == J3DNodeCBCalcTiming_In) {
        ...
        if (i_this != NULL) {
            ...
            if (jntNo == 1) {
                mDoMtx_YrotM(*calc_mtx, i_this->mKanteraDousaRot.y);
                mDoMtx_XrotM(*calc_mtx, i_this->mKanteraDousaRot.x);
                mDoMtx_ZrotM(*calc_mtx, i_this->mKanteraDousaRot.z);
                local_28.x = 0.0f;
                local_28.y = -100.0f;
                local_28.z = 0.0f;
                MtxPosition(&local_28, &i_this->m358);
...
void body_draw(bpw_class* i_this) {
    ...
    uVar2 = dComIfGd_setShadow(
        ...
        200.0f,
        actor->current.pos.y,
        i_this->mBodyBgS_Acch.GetGroundH(),
        i_this->mBodyBgS_Acch.m_gnd,
        &actor->tevStr,
        0,
        1.0f,
...
void anm_init(bpw_class* i_this, int bckFileIdx, f32 morf, u8 loopMode, f32 speed, int soundFileIdx) {
    ...
    if (soundFileIdx >= 0) {
        pSoundAnimRes = dComIfG_getObjectRes("BPW", soundFileIdx);
        pJVar1 = (J3DAnmTransform*)dComIfG_getObjectRes("BPW", bckFileIdx);
        i_this->mpAnim->setAnm(pJVar1, loopMode, morf, speed, 0.0f, -1.0f, pSoundAnimRes);
    } else {
        pJVar1 = (J3DAnmTransform*)dComIfG_getObjectRes("BPW", bckFileIdx);
        i_this->mpAnim->setAnm(pJVar1, loopMode, morf, speed, 0.0f, -1.0f, NULL);
    }
}
...
BOOL body_atari_check(bpw_class* i_this) {
    ...
    i_this->mBodyCcD_Stts.Move();
    if (!i_this->mBodySph.ChkTgHit()) {
        return 0;
    }
    pcVar5 = i_this->mBodySph.GetTgHitObj();
    if (pcVar5 == NULL) {
        return 0;
    }
    bVar2 = false;
    local_40 = i_this->mBodySph.GetTgHitObj();
    i_this->m3DF = 1;
    if (i_this->m3E0 == 0) {
        ...
        case AT_TYPE_LIGHT_ARROW:
            if ((i_this->m3DF = 0xb, i_this->mBodyAction != bpw_class::Action_DAMAGE_e)) {
                i_this->mBodyAction = bpw_class::Action_DAMAGE_e;
                i_this->mActionState = 0x50;
            }
            break;
    ...
    case AT_TYPE_DARKNUT_SWORD:
        mDoAud_seStart(JA_SE_LK_SW_HIT_S, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
    ...
    case AT_TYPE_HOOKSHOT:
        mDoAud_seStart(JA_SE_LK_MS_WEP_HIT, &actor->eyePos, 0x44, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
        i_this->m3DF = 0xc;
        anm_init(i_this, BPW_BCK_BOYON_S1, 2.0f, J3DFrameCtrl::EMode_NONE, 1.0f, -1);
        break;
    ...
    case AT_TYPE_BOOMERANG:
        i_this->m3DF = 5;
        mDoAud_seStart(JA_SE_LK_W_WEP_HIT, &actor->eyePos, 0x44, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
        anm_init(i_this, BPW_BCK_BOYON_S1, 2.0f, J3DFrameCtrl::EMode_NONE, 1.0f, -1);
        break;
    case AT_TYPE_SKULL_HAMMER:
        mDoAud_seStart(JA_SE_LK_HAMMER_HIT, &actor->eyePos, 0x44, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
        i_this->m3DF = 8;
    ...
    if (!bVar2) {
        local_4c = *i_this->mBodySph.GetTgHitPosP();
        local_58.x = 2.0f;
        local_58.y = 2.0f;
        local_58.z = 2.0f;
...
void BG_check(bpw_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    i_this->mDamageBallSpeed.SetWall(i_this->m498, i_this->m49C);
    actor->current.pos.y = actor->current.pos.y - i_this->m48C;
    actor->old.pos.y = actor->old.pos.y - i_this->m48C;
    i_this->mBodyBgS_Acch.CrrPos(*dComIfG_Bgsp());
    actor->current.pos.y = actor->current.pos.y + i_this->m48C;
    actor->old.pos.y = actor->old.pos.y + i_this->m48C;
}
...
s32 wall_HIT_check(bpw_class* i_this) {
    ...
    if (i_this->mDamageBallSpeed.ChkWallHit()) {
        dComIfG_Bgsp()->GetTriPnt(i_this->mDamageBallSpeed, &acStack_24, &cStack_30, &cStack_3c);
        if (dComIfG_Bgsp()->GetAttributeCode(i_this->mDamageBallSpeed) == dBgS_Attr_DAMAGE_e) {
...
void fuwafuwa_calc(bpw_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    i_this->m47C += 700;
    actor->current.pos.y = i_this->mBodyBgS_Acch.GetGroundH() + 30.0f;
    ...
    if ((!player->checkConfuse()) && (i_this->m47E != 0)) {
            ...
            ((i_this->mActionState != 10 && (i_this->mActionState != 0xb)) && (i_this->mActionState != 0xc && (i_this->mActionState != 0xd))))
        {
            if ((((i_this->m3F8 == 0x2a) || (i_this->m3F8 == 0x1d)) || (i_this->m3F8 == 0x1c || (i_this->m3F8 == 0x2b))) &&
                (dVar6 = fopAcM_searchPlayerDistance(actor), dVar6 < 530.0f))
            {
                ...
                i_this->mActionState = 0x4d;
                   ...
                   (i_this->mSomeCountdownTimers[3] == 0 && (i_this->mBodySph2.ChkAtHit())))
        {
            pdVar4 = i_this->mBodySph2.GetAtHitAc();
            if ((pdVar4 != NULL) && (pdVar4 == player)) {
                actor->speedF = 0.0f;
                i_this->m490 = 0.0f;
                i_this->mBodySph.OffTgSetBit();
                i_this->mBodySph.ClrTgHit();
                player->onConfuse();
                i_this->mSomeCountdownTimers[3] = 0x14;
                local_28 = actor->shape_angle;
                pcVar5 = dComIfGp_getCamera(dComIfGp_getPlayerCameraID(0));
                i_this->mFire1DousaPos = player->getHeadTopPos();
                dComIfGp_particle_set(dPa_name::ID_COMMON_STARS_BLOW, &i_this->mFire1DousaPos);
                local_28.y = pcVar5->mAngle.y;
                fopAcM_create(PROC_BPW, 3, &i_this->mFire1DousaPos, fopAcM_GetRoomNo(actor), &local_28);
            }
        }
    }
}
...
void fire_and_emitter_clear(bpw_class* i_this) {
    ...
    i_this->mFire1Dousa_Pa_followEcallBack.end();
    i_this->mFire1Dousa_Pa_followEcallBack2.end();
    i_this->m590.end();
    i_this->m540.end();
    i_this->m52C.end();
    i_this->m50C.end();
...
void next_status_clear(bpw_class* i_this, u8 param_2) {
    ...
    i_this->m47A = 0x500;
    i_this->mBodySph.OffTgShield();
    i_this->mBodySph2.OnAtSetBit();
    i_this->mBodySph2.OnAtHitBit();
    i_this->mBodySph.OnTgSetBit();
...
void maai_sub(bpw_class* i_this) {
    ...
    if (!(fopAcM_searchPlayerDistance(actor) > 500.0f)) {
        mDoMtx_YrotS(*calc_mtx, fopAcM_searchPlayerAngleY(actor));
        local_2c.x = 0.0f;
        local_2c.y = 0.0f;
        local_2c.z = -500.0f;
...
s32 kantera_pos_search(bpw_class* i_this) {
    ...
    if (i_this->m3FC != fpcM_ERROR_PROCESS_ID_e) {
        ...
        if (pfVar3 != NULL) {
            ...
            dVar5 = fVar2;
            mDoMtx_YrotS(*calc_mtx, (s16)cM_atan2s(fVar1, fVar2));
            local_44.x = -120.0f;
            local_44.y = -20.0f;
            local_44.z = -320.0f;
            ...
            local_50.x = actor->current.pos.x - local_50.x;
            local_50.y = actor->current.pos.y - local_50.y;
            local_50.z = actor->current.pos.z - local_50.z;
            fVar1 = std::sqrtf(local_50.z * local_50.z + local_50.x * local_50.x + local_50.y * local_50.y);
            if (fVar1 < 2.0f) {
                return 1;
            }
        }
...
void action_dousa(bpw_class* i_this) {
    ...
    case 0xb:
        ...
        if (i_this->m47E < 2) {
            i_this->m47E = 0;
            iVar5 = kantera_pos_search(i_this);
            if (iVar5 != 0) {
                i_this->mActionState++;
            }
        }
    ...
    case 0xd:
        ...
        iVar5 = i_this->mpAnim->mFrameCtrl.checkPass(12.0f);
        if (iVar5 != 0) {
            mDoAud_seStart(JA_SE_CM_BPW_GET_KANTERA, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
            if (i_this->m3FC != fpcM_ERROR_PROCESS_ID_e) {
...
void action_kougeki(bpw_class* i_this) {
    /* Nonmatching - "i_this->m3F6" switch case */
    fopAc_ac_c* actor = &i_this->actor;
    f32 fVar1;
    bpw_class* pfVar6;
    bpw_class* pfVar5;
    ...
    case 0x14:
        ...
        i_this->m47A = 0x500;
        i_this->m4A0 = 0.0f;
        i_this->mSomeCountdownTimers[0] = 0x1e;
        i_this->mActionState++;
        // fallthrough
    case 0x15:
    ...
    case 0x28:
        ...
        // fallthrough
    case 0x29:
        mDoAud_seStart(JA_SE_OBJ_BPW_BREATH_IN, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
        i_this->m476 = fopAcM_searchPlayerAngleY(actor);
        search_get_skull(i_this, 0);
        pJVar11 = i_this->m540.getEmitter();
        if (pJVar11 != NULL) {
    ...
    case 0x2a:
        ...
        if (i_this->mpAnim->isStop()) {
            ...
            if (i_this->m52C.getEmitter() == NULL) {
                dComIfGp_particle_set(dPa_name::ID_SCENE_82CF, &i_this->m394, NULL, NULL, 0xff, &i_this->m52C);
            }
            i_this->mFire1DousaPos = actor->current.pos;
            i_this->mFire1DousaPos.y = i_this->mBodyBgS_Acch.GetGroundH();
    ...
    case 0x2b:
        ...
        pJVar11 = i_this->m52C.getEmitter();
        if (pJVar11 != NULL) {
            pJVar11->setGlobalSRTMatrix(i_this->mpAnim->getModel()->getAnmMtx(2));
        }
        i_this->mFire1DousaPos = actor->current.pos;
    ...
    case 0x34:
        ...
        if (i_this->mpAnim->isStop()) {
            pfVar6 = (bpw_class*)fopAcM_SearchByID(i_this->m3FC);
            if (pfVar6 != NULL) {
                i_this->m400 = fopAcM_create(PROC_BPW, 2, &pfVar6->m358, fopAcM_GetRoomNo(actor), &actor->current.angle);
    ...
    case 0x3d:
        maai_sub(i_this);
        i_this->m476 = fopAcM_searchPlayerAngleY(actor);
        mDoAud_seStart(JA_SE_CM_BPW_SWING_KANTERA, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
        if (i_this->mSomeCountdownTimers[1] == 0) {
            mDoAud_seStart(JA_SE_OBJ_BPW_FIRE_OUT, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
            ...
            if (pfVar7 != NULL) {
                ...
                i_this->m400 = fopAcM_create(PROC_BPW, 0x102, &cStack_44, fopAcM_GetRoomNo(actor), &actor->current.angle);
...
void action_karada_taore(bpw_class* i_this) {
    ...
    case 0x47:
        i_this->m476 = fopAcM_searchPlayerAngleY(actor);
        if (i_this->mpAnim->mFrameCtrl.checkPass(26.0f)) {
            i_this->mFire1DousaPos = actor->current.pos;
            i_this->mFire1DousaPos.y = i_this->mBodyBgS_Acch.GetGroundH();
            i_this->m5E0[1].end();
            ...
            mDoAud_seStart(JA_SE_CM_BPW_J_BPRESS_JUMP, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
    ...
    case 0x48:
        ...
        break;
    case 0x49:
        if (i_this->mBodyBgS_Acch.ChkGroundHit())
            break;
        mDoAud_seStart(JA_SE_CM_BPW_J_BPRESS_PRESS, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
        actor->speed.setall(0.0f);
        actor->speedF = 0.0f;
    ...
    case 0x4e:
        ...
        if (i_this->mpAnim->mFrameCtrl.checkPass(40.0f)) {
            i_this->mFire1DousaPos = actor->current.pos;
            i_this->mFire1DousaPos.y = i_this->mBodyBgS_Acch.GetGroundH();
            i_this->m5E0[3].end();
            pJVar4 = dComIfGp_particle_setToon(
            ...
            actor->gravity = -3.0f;
            mDoAud_seStart(JA_SE_CM_BPW_BODY_PRESS, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
            i_this->m3E8 = 1;
...
void action_damage(bpw_class* i_this) {
    ...
    case 0x51:
        ...
        if ((uVar2 == 10) || (uVar2 == 0xb)) {
            if (uVar2 == 0xb) {
                mDoAud_seStart(JA_SE_CM_BPW_SOLID_END, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                i_this->m47E = 0xff;
            } else {
                i_this->m47E = i_this->m47E + 1;
                mDoAud_seStart(JA_SE_CM_BPW_BECOME_SOLID, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                if (i_this->mSomeCountdownTimers[9] == 0) {
                    fopAcM_monsSeStart(actor, JA_SE_CV_BPW_LIGHT_DAMAGE, 0);
                    i_this->mSomeCountdownTimers[9] = 0x1e;
                }
                if (i_this->m3E1 == 0) {
                    i_this->m3E1 = 1;
                    i_this->m3E2 = 0;
    ...
    case 0x52:
        ...
        if (i_this->mSomeCountdownTimers[0] == 0) {
            ...
            if (i_this->mBodyBgS_Acch.ChkGroundHit()) {
                actor->gravity = 0.0f;
                actor->speed.setall(0.0f);
                i_this->mBodySph.OnTgShield();
                i_this->mSomeCountdownTimers[6] = (s16)(int)(REG21_F(1) + 20.0f);
                i_this->m3E8 = 1;
                i_this->mBodySph.OnCoSetBit();
                i_this->m48C = -120.0f;
                i_this->mActionState = 0x53;
                mDoAud_seStart(JA_SE_CM_BPW_SOLID_FALL, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
    ...
    case bpw_class::Action_State_DAMAGE_THROWN_e:
        mDoAud_seStart(JA_SE_CM_BPW_ROLLING, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
        if (REG8_S(3) != 0) {
        ...
        if (pwSpreadAmount != 0) {
            ...
            if (pwSpreadAmount == 2) {
                mDoAud_seStart(JA_SE_CM_BPW_DAMAGE_TOGE, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                fopAcM_monsSeStart(actor, JA_SE_CV_BPW_DAMAGE, 0);
                i_this->mSomeCountdownTimers[6] = (s16)(int)(REG21_F(0) + 20.0f);
                i_this->m3E8 = 2;
                i_this->mActionState = 100;
            } else {
                mDoAud_seStart(JA_SE_CM_BPW_BOUND_NO_DMG, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                fopAcM_monsSeStart(actor, JA_SE_CV_BPW_LANDING, 0);
                actor->shape_angle.x = 0;
                i_this->mSomeCountdownTimers[6] = (s16)(int)(REG21_F(1) + 20.0f);
                i_this->m3E8 = 1;
    ...
    case bpw_class::Action_State_DAMAGE_HIT_e:
        ...
        if (i_this->mBodyBgS_Acch.ChkGroundHit()) {
            ...
            mDoAud_seStart(JA_SE_CM_BPW_DAMAGE_EXPLODE, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
            dComIfGp_particle_set(dPa_name::ID_SCENE_8444, &actor->current.pos);
            pwAngle = actor->shape_angle;
            local_58 = actor->current.pos;
            pwSpreadAmount = (int)(65536.0f / actor->health);
            local_58.y = i_this->mBodyBgS_Acch.GetGroundH() + 20.0f;
            currRandomAngle = 0;
            i_this->m3E9 = 0;
            for (s32 i = 0; i < 0xF; i++) {
            ...
            uVar14 = (uint)(short)(int)cM_rndF(4.99);
            uVar2 = uVar14 << 9 | 0xff000003;
            iVar17 = 0;
            maxPoeSpread = (pwSpreadAmount * 0.25f);
            for (iVar15 = 0; iVar15 < actor->health; iVar15++) {
                ...
                i_this->mChildPoeIds[iVar17] = childPoeId;
                currRandomAngle = currRandomAngle + pwSpreadAmount;
                pwAngle.y = (short)currRandomAngle;
                pwAngle.y += (short)(int)cM_rndFX((f32)maxPoeSpread);
                uVar14 = uVar14 + 1;
                if (5 < uVar14) {
                    uVar14 = uVar14 << 9 | 0xff000003;
                }
                iVar17 = iVar17 + 4;
            }
...
void action_bunri_dousa(bpw_class* i_this) {
    ...
    int childPoeId3;
    fpc_ProcID local_d4;
    fpc_ProcID childPoeId2;
    fpc_ProcID childPoeId;
    cXyz local_80;
    cXyz local_74;
    cXyz local_68;
    s8 currHp;
    ...
    case bpw_class::Action_State_SEPARATE_BUNRI_DOUSA_INIT_e:
        ...
        i_this->mBodySph2.OffAtSetBit();
        i_this->mBodySph2.ClrAtSet();
        i_this->mBodySph.OffTgSetBit();
        i_this->mBodySph.ClrCoSet();
        i_this->mBodySph.ClrTgHit();
    ...
    case bpw_class::Action_State_SEPARATE_BUNRI_DOUSA_EXECUTE_e:
        ...
        } else if ((i_this->mSomeCountdownTimers[0] == 0) || (currHp <= (char)hpAmountToEndPhaseAt)) {
            i_this->mBodySph.OnCoSetBit();
            fopAcM_OnStatus(actor, fopAcStts_UNK4000_e);
            i_this->mActionState++;
        }
    ...
    case 0x70:
        ...
        for (iVar5 = 0; iVar5 < actor->health; iVar5++) {
            childPoeId = i_this->mChildPoeIds[iVar6];
            childPoe = (pw_class*)fopAcM_SearchByID(childPoeId);
    ...
    case 0x71:
        if (actor->eventInfo.checkCommandDemoAccrpt()) {
            ...
            local_80.y = 0.0f;
            local_80.z = 1625.0f;
            player->setPlayerPosAndAngle(&local_80, fopAcM_searchActorAngleY(player, actor));
            i_this->mActionState++;
        }
    case bpw_class::Action_State_SEPARATE_BUNRI_DOUSA_END_INIT_e:
        ...
        if ((REG20_S(0) != 0) && (REG20_S(0) = 0, i_this->mSomeCountdownTimers[2] == 0)) {
            actor->current.angle.y = fopAcM_searchPlayerAngleY((pw_class*)i_this);
            ...
            if ((i_this->m3E9 == 0) && (actor->scale.x > 0.9f)) {
                ...
                for (iVar6 = 0; iVar6 < actor->health; iVar6++) {
                    childPoeId2 = i_this->mChildPoeIds[iVar5];
                    childPoe2 = (pw_class*)fopAcM_SearchByID(childPoeId2);
                    if (childPoe2 != NULL) {
    ...
    case bpw_class::Action_State_SEPARATE_BUNRI_DOUSA_END_FINISH_e:
        ...
        if ((REG20_S(0) != 0) && (REG20_S(0) = 0, i_this->mSomeCountdownTimers[0] == 0)) {
    ...
    case 0x78:
        ...
        anm_init(i_this, BPW_BCK_CORE1, 15.0f, J3DFrameCtrl::EMode_LOOP, 1.0f, -1);
        if (i_this->m3FC != fpcM_ERROR_PROCESS_ID_e) {
            local_d4 = i_this->m3FC;
            i_actor = fopAcM_SearchByID(local_d4);
        ...
        for (iVar6 = 0; iVar6 < (char)actor->max_health; iVar6++) {
            childPoeId3 = i_this->mChildPoeIds[iVar5];
            childPoe3 = (pw_class*)fopAcM_SearchByID(childPoeId3);
        ...
        i_this->mActionState++;
        childPoeId4 = i_this->mChildPoeIds[i_this->m462];
        childPoe4 = (pw_class*)fopAcM_SearchByID(childPoeId4);
        if (childPoe4 != NULL) {
            i_this->m40C = childPoe4->current.pos;
        }
    case 0x79:
        childPoeId5 = i_this->mChildPoeIds[i_this->m462];
        childPoe5 = (pw_class*)fopAcM_SearchByID(childPoeId5);
        if (childPoe5 != NULL) {
        ...
        mDoMtx_YrotS(*calc_mtx, (int)i_this->m464);
        local_68.x = 0.0f;
        local_68.y = 0.0f;
        local_68.z = 800.0f;
        MtxPosition(&local_68, &local_74);
        local_74.x = local_74.x + (i_this->m424).x;
        local_74.z = local_74.z + (i_this->m424).z;
        local_74.y = local_74.y + (i_this->m424).y * 0.3f;
        ...
        if (i_this->mSomeCountdownTimers[0] == 0) {
            ...
            if (REG20_S(0) != 0) {
                ...
                player->changeDemoMode(1);
                local_80.x = -36.0f;
                local_80.y = player->current.pos.y;
                local_80.z = -777.0f;
                iVar5 = cM_atan2s(player->current.pos.x - (i_this->m424).x, player->current.pos.z - (i_this->m424).z);
                player->setPlayerPosAndAngle(&local_80, (int)(short)iVar5);
                i_this->mActionState++;
            }
        }
        break;
    case 0x7a:
        actor->shape_angle.y = fopAcM_searchPlayerAngleY((pw_class*)pbVar4);
        actor->current.angle.y = actor->shape_angle.y;
        i_this->m476 = actor->shape_angle.y;
        ...
        local_80.y = player->current.pos.y;
        local_80.z = -777.0f;
        player->setPlayerPosAndAngle(&local_80, fopAcM_searchPlayerAngleY((pw_class*)pbVar4) + 0x8000);
        if (i_this->mSomeCountdownTimers[0] != 0) {
        ...
        if ((REG20_S(0) == 0) || (REG20_S(0) = 0, actor->scale.x < 0.9f))
            break;
        actor->scale.setall(1.0f);
    ...
    case 0x7b:
        if (i_this->mSomeCountdownTimers[0] != 0) {
            ...
            cLib_addCalc2(&i_this->m43C, 1.0f, 1.0f, 0.04f);
            if (i_this->m3F8 == 0x1b) {
                if (i_this->mpAnim->mFrameCtrl.checkPass(1.0f)) {
                    fopAcM_monsSeStart(actor, JA_SE_CV_BPW_LOOK_RIGHT, 0);
                }
                if (i_this->mpAnim->mFrameCtrl.checkPass(41.0f)) {
                    fopAcM_monsSeStart(actor, JA_SE_CV_BPW_LOOK_LEFT, 0);
                }
                if (i_this->mpAnim->mFrameCtrl.checkPass(76.0f)) {
    ...
    case 0x7e:
        ...
        local_80.z = -777.0f;
        pbVar4 = i_this;
        player->setPlayerPosAndAngle(&local_80, fopAcM_searchPlayerAngleY((pw_class*)i_this) + 0x8000);
    ...
    case 0x85:
        ...
        local_80.z = -777.0f;
        player->setPlayerPosAndAngle(&local_80, (fopAcM_searchPlayerAngleY((pw_class*)i_this) + 0x8000));
        if (!(REG6_F(0x13) + ((i_this->mBodyBgS_Acch).GetGroundH() + 1350.0f) < actor->current.pos.y)) {
            if (REG20_S(1) == 0) {
                REG20_S(0) = 1;
            }
            if (REG20_S(0) != 0) {
    ...
    case 0x86:
        ...
        cLib_addCalc0(&i_this->m3AC.y, 1.0f, 3.0f);
        cLib_addCalc2(&i_this->m48C, -120.0f, 1.0f, 3.0f);
        if (!(REG12_F(0xe) + (i_this->mBodyBgS_Acch).GetGroundH() < actor->current.pos.y)) {
            if (REG20_S(1) == 0) {
                REG20_S(0) = 1;
            }
            if (REG20_S(0) != 0) {
                ...
                local_80 = actor->current.pos;
                local_80.y += REG8_F(5) + 80.0f;
                fopAcM_createDisappear(actor, &local_80, 0xF, daDisItem_HEART_CONTAINER_e, 0xFF);
    ...
    case 0x87:
        if (i_this->mSomeCountdownTimers[0] != 0) {
            ...
            local_80.y = player->current.pos.y;
            local_80.z = -777.0f;
            iVar5 = cM_atan2s(player->current.pos.x - i_this->mBodyPos.x, player->current.pos.z - i_this->mBodyPos.z);
            player->setPlayerPosAndAngle(&local_80, (int)(short)((short)iVar5 + -0x8000));
    ...
    case 0x88:
        ...
        if (i_this->mSomeCountdownTimers[2] == 0) {
            if (i_this->mSomeCountdownTimers[1] == 1) {
                ...
                i_this->mSomeCountdownTimers[9] = REG6_S(5) + 0x1e;
            }
...
void action_start_demo(bpw_class* i_this) {
    ...
    player = daPy_getPlayerActorClass();
    pcVar9 = dComIfGp_getCamera(dComIfGp_getPlayerCameraID(0));
    switch (i_this->mActionState) {
    case 200:
        actor->scale.setall(0.0f);
        i_this->m47E = 0;
    ...
    case 0xce:
        ...
        if (i_this->mSomeCountdownTimers[0] == 0) {
            cLib_addCalc2(&i_this->m440, REG6_F(3) + 55.0f, 1.0f, std::fabsf(i_this->m440 - (REG6_F(3) + 55.0f)) * (REG18_F(2) + 0.05f) * i_this->m43C);
            cLib_addCalc2(&i_this->m40C.x, 0.0f, 0.1f, std::fabsf(i_this->m40C.x) * (REG18_F(2) + 0.05f) * i_this->m43C);
            cLib_addCalc2(&i_this->m40C.y, 233.0f, 0.1f, std::fabsf(i_this->m40C.y - 233.0f) * (REG18_F(2) + 0.05f) * i_this->m43C);
            cLib_addCalc2(&i_this->m40C.z, 552.0f, 0.1f, std::fabsf(i_this->m40C.z - 552.0f) * (REG18_F(2) + 0.05f) * i_this->m43C);
            cLib_addCalc2(&i_this->m418.x, 0.0f, 1.0f, std::fabsf(i_this->m418.x) * (REG18_F(2) + 0.05f) * i_this->m43C);
            cLib_addCalc2(&i_this->m418.y, 270.0f, 1.0f, std::fabsf(i_this->m418.y - 270.0f) * (REG18_F(2) + 0.05f) * i_this->m43C);
            cLib_addCalc2(&i_this->m418.z, 722.0f, 1.0f, std::fabsf(i_this->m418.z - 722.0f) * (REG18_F(2) + 0.05f) * i_this->m43C);
            cLib_addCalc2(&i_this->m43C, 1.0f, 1.0f, 0.04f);
            i_this->m3EA = 4;
            if (i_this->mSomeCountdownTimers[1] == 0) {
                if (REG20_S(1) == 0) {
                ...
                if (REG20_S(0) != 0) {
                    ...
                    i_this->m480 = 0x96;
                    mDoAud_seStart(JA_SE_OBJ_BPW_MASK_APPEAR, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                    i_this->mActionState++;
                }
            }
        }
    ...
    case 0xd0:
        if (i_this->m3F8 == 9) {
            ...
            mDoAud_seStart(JA_SE_OBJ_BPW_MASK_LAUGH, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
        } else {
    ...
    case 0xd1:
        if (i_this->mSomeCountdownTimers[1] == 0) {
            cLib_addCalc2(&i_this->m440, REG6_F(4) + 65.0f, 1.0f, std::fabsf(i_this->m440 - (REG6_F(4) + 65.0f)) * (REG18_F(8) + 0.08f) * i_this->m43C);
            cLib_addCalc2(&i_this->m40C.x, 281.0f, 0.1f, std::fabsf(i_this->m40C.x - 281.0f) * (REG18_F(8) + 0.08f) * i_this->m43C);
            cLib_addCalc2(&i_this->m40C.y, 93.0f, 0.1f, std::fabsf(i_this->m40C.y - 93.0f) * (REG18_F(8) + 0.08f) * i_this->m43C);
            cLib_addCalc2(&i_this->m40C.z, 725.0f, 0.1f, std::fabsf(i_this->m40C.z - 725.0f) * (REG18_F(8) + 0.08f) * i_this->m43C);
            cLib_addCalc2(&i_this->m418.x, 336.0f, 1.0f, std::fabsf(i_this->m418.x - 336.0f) * (REG18_F(8) + 0.08f) * i_this->m43C);
            cLib_addCalc2(&i_this->m418.y, 30.0f, 1.0f, std::fabsf(i_this->m418.y - 30.0f) * (REG18_F(8) + 0.08f) * i_this->m43C);
            cLib_addCalc2(&i_this->m418.z, 877.0f, 1.0f, std::fabsf(i_this->m418.z - 877.0f) * (REG18_F(8) + 0.08f) * i_this->m43C);
            cLib_addCalc2(&i_this->m43C, 1.0f, 1.0f, 0.04f);
            if (REG20_S(1) == 0) {
                REG20_S(0) = 1;
            }
            if ((REG20_S(0) != 0) && (REG20_S(0) = 0, i_this->mSomeCountdownTimers[0] == 0)) {
                ...
                mDoAud_seStart(JA_SE_CM_BPW_MASK_TO_BPW, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                i_this->mActionState++;
            }
        }
        break;
    ...
    case 0xd4:
        if (i_this->mSomeCountdownTimers[0] == 0) {
            ...
            i_this->m3F6 = 4;
            light_on_off(i_this);
            pcVar9->mCamera.Reset(i_this->m40C, i_this->m418);
            pcVar9->mCamera.Start();
            pcVar9->mCamera.SetTrimSize(0);
            player->cancelOriginalDemo();
    ...
    if ((i_this->m3F8 == 0x1f) && (i_this->mpAnim->mFrameCtrl.checkPass(170.0f))) {
    ...
    if (((i_this->m3F8 == 0x1f) && (i_this->mpAnim->getFrame() >= 60.0f)) || (i_this->m3F8 == 0x20)) {
        mDoAud_seStart(JA_SE_OBJ_BPW_FLYING, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
    }
    if (i_this->mActionState >= 0xCA) {
        pcVar9->mCamera.Set(i_this->m40C, i_this->m418, i_this->m440, 0);
    }
    if (i_this->mActionState >= 0xd1) {
        pfVar5 = (bpw_class*)fopAcM_SearchByID(i_this->m3FC);
        if (pfVar5 != NULL) {
            pfVar5->mpAnim->play(NULL, 0, 0);
            if (pfVar5->mpAnim->mFrameCtrl.checkPass(REG8_F(9) + 128.0f)) {
                mDoAud_seStart(JA_SE_CM_BPW_KANTERA_APPEAR, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                pfVar5->m3E7 = 0;
            }
        }
    }
...
void body_execute(bpw_class* i_this) {
    ...
    if (REG8_S(2) != 0) {
        ...
        i_this->mBodySph.SetC(local_28);
        i_this->mBodySph.SetR(240.0f);
        dComIfG_Ccsp()->Set(&i_this->mBodySph);
        local_28 = i_this->m370;
        i_this->mBodySph2.SetC(local_28);
        i_this->mBodySph2.SetR(i_this->m4A8);
        dComIfG_Ccsp()->Set(&i_this->mBodySph2);
    } else {
        ...
        if (i_this->m3E0 != 0) {
            ...
            if (i_this->m3E1 != 0) {
                ...
                if (i_this->m3E2 == 2) {
                    i_this->m2C8->play();
                    mDoAud_seStart(JA_SE_OBJ_BPW_FLYING_CURSE, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                } else if (i_this->m3E2 == 1) {
        ...
        i_this->mBodySph.SetC(local_28);
        i_this->mBodySph.SetR(REG0_S(6) + 260.0f);
        dComIfG_Ccsp()->Set(&i_this->mBodySph);
        local_28 = i_this->m370;
        i_this->mBodySph2.SetC(local_28);
        i_this->mBodySph2.SetR(i_this->m4A8);
...
BOOL kantera_atari_check(bpw_class* i_this) {
    /* Nonmatching - "pcVar4->GetAtType()" */
    fopAc_ac_c* actor = &i_this->actor;
    cCcD_Obj* pcVar4;

    i_this->mBodyCcD_Stts.Move();
    if (!i_this->mKanteraSph.ChkTgHit()) {
        return FALSE;
    } else {
        daPy_py_c* player = daPy_getPlayerActorClass();
        pcVar4 = i_this->mKanteraSph.GetTgHitObj();
        if (pcVar4 == NULL) {
            return FALSE;
        } else if (pcVar4->GetAtType() != AT_TYPE_SKULL_HAMMER) {
            actor->speedF = 60.0f;
            actor->current.angle.y = player->shape_angle.y;
            i_this->m3DF = 8;
            if (player->mCutType == 17) {
            ...
            mDoAud_seStart(JA_SE_LK_HAMMER_HIT, &actor->eyePos, 0x42, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
            return TRUE;
        } else {
            return FALSE;
        }
...
void action_kantera_dousa(bpw_class* i_this) {
    /* Nonmatching - "i_this->mActionState" switch case */
    fopAc_ac_c* actor = &i_this->actor;
    short sVar4;
    fopAc_ac_c* pfVar2;
    int iVar3;
    ...
    case 0:
        ...
        i_this->m498 = 50.0f;
        i_this->m49C = 200.0f;
        i_this->mKanteraSph.OffCoSetBit();
        i_this->m48C = 60.0f;
        i_this->mActionState++;
        break;
    case 1:
    ...
    case 2:
        ...
        case 0:
            if (i_this->mBodyBgS_Acch.ChkGroundHit()) {
                i_this->mKanteraSph.OnCoSetBit();
                actor->speed.y = 17.0f;
                actor->speedF = 0.0f;
                mDoAud_seStart(JA_SE_OBJ_BPW_KANTERA_FALL, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
                i_this->m462++;
            }
            break;
        case 1:
            ...
            if ((sVar4 < 0x100) && (i_this->mBodyBgS_Acch.ChkGroundHit())) {
                i_this->m462++;
        ...
        break;
    case 3:
        if (i_this->mBodyBgS_Acch.ChkWallHit()) {
    ...
    case 5:
        ...
        if ((i_this->mActionState != 4) && (iVar3 = kantera_atari_check(i_this), iVar3 != 0)) {
            i_this->m464 = 0x2000;
            i_this->mKanteraDousaRot.x = 0;
            i_this->mKanteraDousaRot.y = 0;
            i_this->mKanteraDousaRot.z = 0;
...
void kantera_execute(bpw_class* i_this) {
    ...
    local_28 = i_this->m358;
    if (i_this->mBodyAction != bpw_class::Action_MOVE_DOUSA_e) {
        action_kantera_dousa(i_this);
    }
    i_this->mKanteraSph.SetC(local_28);
    i_this->mKanteraSph.SetR(70.0f);
    dComIfG_Ccsp()->Set(&i_this->mKanteraSph);
    if (i_this->m3E7 == 0) {
        i_this->mKanteraAnim->play();
    }
    if (i_this->mKanteraSph.ChkTgHit()) {
        pcVar2 = i_this->mKanteraSph.GetTgHitObj();
        ...
        case AT_TYPE_DARKNUT_SWORD:
        case AT_TYPE_MOBLIN_SPEAR:
            mDoAud_seStart(JA_SE_LK_SW_HIT_S, &actor->eyePos, 0x42, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
            break;
        case AT_TYPE_BOOMERANG:
        case AT_TYPE_BOKO_STICK:
        case AT_TYPE_UNK2000:
        case AT_TYPE_STALFOS_MACE:
            mDoAud_seStart(JA_SE_LK_W_WEP_HIT, &actor->eyePos, 0x42, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
            break;
        default:
...
void action_b_fire_1_dousa(bpw_class* i_this) {
    ...
    actor->shape_angle.y += (REG6_S(2) + 2000.0f) * cM_ssin(i_this->m462);
    i_this->m304[0] = actor->current.pos;
    mDoMtx_YrotS(*calc_mtx, (int)actor->shape_angle.y);
    mDoMtx_XrotM(*calc_mtx, actor->shape_angle.x);
    local_c4.x = 0.0f;
    ...
    fVar1 = i_this->m3A0.x - local_c4.x;
    fVar2 = i_this->m3A0.y - local_c4.y;
    fVar3 = i_this->m3A0.z - local_c4.z;
    fVar1 = std::sqrtf(fVar3 * fVar3 + fVar1 * fVar1 + fVar2 * fVar2);
    if (fVar1 < 2.0f) {
        i_this->m464++;
        i_this->m464 &= 1;
    }
...
void action_b_fire_2_dousa(bpw_class* i_this) {
    /* Nonmatching - "local_38 = actor->shape_angle" */
    fopAc_ac_c* actor = &i_this->actor;
    daPy_py_c* player;
    int iVar4;
    csXyz local_38;
    ...
    case 0x15:
        ...
        i_this->mFire1DousaRot = actor->shape_angle;
        if (i_this->mBodyBgS_Acch.ChkGroundHit()) {
            mDoAud_seStart(JA_SE_OBJ_BPW_FIRE_EXPLODE, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
            local_30 = actor->current.pos;
            local_30.y = i_this->mBodyBgS_Acch.GetGroundH();
            ...
            i_this->mFireDousa2_Pa_followEcallBack.end();
            i_this->mFireDousa2_Pa_followEcallBack2.end();
            if (i_this->mSomeCountdownTimers[1] == 0) {
            ...
            if ((i_this->mBodyBgS_Acch.ChkWallHit()) || (i_this->mDamageBallCcD_Sph.ChkAtHit())) {
    ...
    case 0x16:
        ...
        if (i_this->mSomeCountdownTimers[0] == 0) {
            ...
            mDoAud_seStart(JA_SE_OBJ_BAR_FRAME_BURN, &actor->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(actor)));
        }
        break;
    case 0x17:
        i_this->mFireDousa2_Pa_followEcallBack.end();
...
static BOOL damage_ball_execute(bpw_class* i_this) {
    ...
    case 0:
        ...
        if (i_this->mDamageBallCcD_Sph.ChkAtHit() != 0) {
            player = daPy_getPlayerActorClass();
            pdVar4 = i_this->mDamageBallCcD_Sph.GetAtHitAc();
            if (pdVar4 != NULL && (pdVar4 == player)) {
                GOUEN_FIRE_HIT = 1;
            }
        }
    ...
    i_this->mDamageBallCcD_Sph.SetC(local_18);
    i_this->mDamageBallCcD_Sph.SetR(i_this->m494);
    dComIfG_Ccsp()->Set(&i_this->mDamageBallCcD_Sph);
}
...
void torituki_execute(bpw_class* i_this) {
    ...
    case 2:
        if (!(actor->scale.x < 0.3f)) {
            anm_init(i_this, 0x29, 15.0f, 1.0f, 2, -1);
        ...
        // fallthrough
    case 3:
        actor->shape_angle.y = actor->shape_angle.y + 1000;
        if (i_this->mSomeCountdownTimers[8] != 0) {
            player = daPy_getPlayerActorClass();
            if (((i_this->mSomeCountdownTimers[8] == 1 || (dComIfGp_getDetect().chk_light(&player->current.pos) != 0)) ||
                 (dComIfGp_checkPlayerStatus1(0, 0x2000))) ||
                ((s16)player->mDamageWaitTimer != 0 || ((player->mResetFlg0 & 0x2000000) != 0)))
            {
                player->offConfuse();
...
static BOOL daBPW_Execute(bpw_class* i_this) {
    ...
    switch (i_this->mType) {
    case bpw_class::Actor_Type_BODY_e:
        if (i_this->mBodySph.ChkCoSet()) {
            fopAcM_posMove(actor, i_this->mBodyCcD_Stts.GetCCMoveP());
        } else {
...
static BOOL daBPW_Delete(bpw_class* i_this) {
    ...
    dComIfG_resDelete((request_of_phase_process_class*)&i_this->m2AC, "BPW");
...
void body_create_init(bpw_class* i_this) {
    ...
    i_this->mBodyBgS_Acch.Set(&actor->current.pos, &actor->old.pos, actor, 1, &i_this->mDamageBallSpeed, &actor->speed);
    i_this->mBodyCcD_Stts.Init(0xfe, 1, actor);
    actor->attention_info.distances[4] = 0x25;
    fopAcM_OnStatus(actor, fopAcStts_UNK10000_e);
    i_this->mBodySph.Set(body_co_sph_src);
    i_this->mBodySph.SetStts(&i_this->mBodyCcD_Stts);
    i_this->mBodySph.OffCoSetBit();
    i_this->mBodySph2.Set(body_at_sph_src);
    i_this->mBodySph2.SetStts(&i_this->mBodyCcD_Stts);
...
static BOOL kantera_useHeapInit(fopAc_ac_c* a_this) {
    ...
    i_this->mpAnim = morf;
    ;
    if ((i_this->mpAnim == NULL) || (pJVar1 = i_this->mpAnim->getModel(), pJVar1 == NULL)) {
        return FALSE;
    } else {
        i_this->mpAnim->getModel()->setUserArea((u32)i_this);
        for (u16 uVar6 = 0; uVar6 < i_this->mpAnim->getModel()->getModelData()->getJointNum(); uVar6++) {
        ...
        pmVar2 = new mDoExt_brkAnm();
        i_this->mKanteraAnim = pmVar2;
...
void kantera_create_init(bpw_class* i_this) {
    ...
    i_this->mBodyBgS_Acch.Set(&actor->current.pos, &actor->old.pos, actor, 1, &i_this->mDamageBallSpeed, &actor->speed);
    i_this->mBodyCcD_Stts.Init(100, 1, actor);
    i_this->mKanteraSph.Set(kantera_co_sph_src);
    i_this->mKanteraSph.SetStts(&i_this->mBodyCcD_Stts);
    i_this->mKanteraSph.OffCoSetBit();
    i_this->mBodyAction = bpw_class::Action_MOVE_DOUSA_e;
    i_this->mActionState = 0;
...
void damage_ball_create_init(bpw_class* i_this) {
    ...
    i_this->mBodyBgS_Acch.Set(&actor->current.pos, &actor->old.pos, actor, 1, &i_this->mDamageBallSpeed, &actor->speed);
    i_this->mBodyCcD_Stts.Init(0xfe, 1, actor);
    i_this->mDamageBallCcD_Sph.Set(damage_ball_co_sph_src);
    i_this->mDamageBallCcD_Sph.SetStts(&i_this->mBodyCcD_Stts);
}
...
void tori_create_init(bpw_class* i_this) {
    ...
    actor->attention_info.flags = 0;
    actor->current.pos = player->getHeadTopPos();
    i_this->mBodyCcD_Stts.Init(0, 1, actor);
}
...
static cPhs_State daBPW_Create(fopAc_ac_c* a_this) {
    ...
    bpw_class* i_this = (bpw_class*)a_this;
    fopAc_ac_c* actor = &i_this->actor;
    local_78 = actor->shape_angle;
    fopAcM_SetupActor(a_this, bpw_class);
    res = dComIfG_resLoad(&i_this->m2AC, "BPW");
    if (res == cPhs_COMPLEATE_e) {
        i_this->mType = (bpw_class::Actor_Type_e)actor->base.mParameters;
        i_this->mUnknownParam2 = (bpw_class::Damage_Action_e)((uint)actor->base.mParameters >> 8);
        i_this->mLightState = (u8)((uint)actor->base.mParameters >> 0x10);
        ...
        switch (i_this->mType) {
        case bpw_class::Actor_Type_BODY_e:
            if (g_dComIfG_gameInfo.save.getMemory().getBit().isStageBossEnemy()) {
                return cPhs_ERROR_e;
            }
            ...
            draw_SUB(i_this);
            local_78.y += 0x4000;
            i_this->m3FC = fopAcM_createChild(PROC_BPW, fopAcM_GetID(i_this), 1, &i_this->mChildActorPos, fopAcM_GetRoomNo(actor), &local_78);
            i_this->mAttWaitTimer = 1;
            kantera_calc(i_this);
            if ((dComIfGp_getStartStageName()[0] != 'X') && (REG0_S(9) != 0 || (!dComIfGs_isStageBossDemo()))) {
                fopAcM_OnStatus(actor, fopAcStts_UNK4000_e);
                local_80 = actor->shape_angle;
                local_70 = actor->current.pos;
                iVar2 = (65536.0f / actor->health);
                local_70.y = i_this->mBodyBgS_Acch.GetGroundH() + 20.0f;
                iVar8 = (int)cM_rndFX(32768.0f);
                uVar7 = (uint)(s16)(int)cM_rndF(4.99f);
                iVar6 = uVar7 << 9 | 0xff000004;
                for (s32 i = 0; i < 0xF; i++) {
                    dVar9 = ((s16)iVar2 * 0.25f);
                    pwID = fopAcM_createChild(PROC_PW, fopAcM_GetID(i_this), iVar6, &local_70, fopAcM_GetRoomNo(actor), &local_80);
                    i_this->mChildPoeIds[i] = pwID;
                    iVar8 = iVar8 + iVar2;
                    local_80.y = (s16)iVar8;
                    local_80.y += (int)cM_rndFX(dVar9);
                    uVar7++;
                    if (5 < uVar7) {
                        uVar7 = uVar7 << 9 | 0xff000004;
                    }
                }
                i_this->m3F6 = 3;
                light_on_off(i_this);
        ...
        case bpw_class::Actor_Type_DAMAGE_BALL_e:
            ...
            case bpw_class::Damage_Action_LINE_e:
                i_this->mDamageBallCcD_Sph.SetAtSpl(dCcG_At_Spl_UNK9);
                i_this->mDamageBallCcD_Sph.SetAtAtp(6);
                i_this->mBodyAction = bpw_class::Action_MOVE_DOUSA_e;
                i_this->mActionState = 10;
                break;
            case bpw_class::Damage_Action_DAMAGE_BALL_e:
                dKy_plight_set(&i_this->m4EC);
                i_this->mDamageBallCcD_Sph.SetAtSpl(dCcG_At_Spl_UNK8);
                i_this->mBodyAction = bpw_class::Action_ATTACK_KOUGEKI_e;
                i_this->mActionState = 0x14;
                break;
            }
