...
static void anm_init(bst_class* i_this, int param_2, float morf, u8 loop_mode, float play_speed, int sound_idx) {
    if (sound_idx >= 0) {
        ...
        i_this->m02B8->setAnm(bckAnm, loop_mode, morf, play_speed, 0.0f, -1.0f, sound);
    } else {
        J3DAnmTransform* bckAnm = (J3DAnmTransform*)dComIfG_getObjectRes("Bst", param_2);
        i_this->m02B8->setAnm(bckAnm, loop_mode, morf, play_speed, 0.0f, -1.0f, NULL);
    }
}
...
static BOOL nodeCallBackHead(J3DNode* node, int calcTiming) {
    if (calcTiming == J3DNodeCBCalcTiming_In) {
        ...
        if (i_this != NULL) {
            ...
            if (jnt_no == 9) {
                mDoMtx_YrotM(*calc_mtx, -i_this->m2E78[1]);
            } else if (jnt_no == 12) {
                mDoMtx_YrotM(*calc_mtx, i_this->m2E78[1]);
            } else if (jnt_no == 10) {
                mDoMtx_YrotM(*calc_mtx, -i_this->m2E78[0]);
            } else if (jnt_no == 11) {
                mDoMtx_YrotM(*calc_mtx, i_this->m2E78[0]);
            }
...
static BOOL daBst_Draw(bst_class* i_this) {
    ...
    mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(0), *calc_mtx);
    temp.setall(0.0f);
    MtxPosition(&temp, &shadow_pos);
    shadow_pos.y += REG0_F(1) * 10.0f;
    i_this->mShadowID = dComIfGd_setRealShadow(i_this->mShadowID, TRUE, model_388, &shadow_pos, (REG0_F(2) * 10.0f) + 2000.0f, 0.0f, &i_this->tevStr);
...
static void pos_move(bst_class* i_this, u8 param_2) {
    ...
    if (!param_2) {
        ...
        cLib_addCalcAngleS2(&i_this->current.angle.y, y_angle, REG0_S(3) + 5, i_this->m10EC.x * i_this->m10EC.y);
        cLib_addCalcAngleS2(&i_this->current.angle.x, unk_angle, REG0_S(3) + 5, i_this->m10EC.x * i_this->m10EC.y);
    }
...
static void stay(bst_class* i_this) {
    ...
    case 5:
        if (i_this->mBstPartType != bst_class::Type_HEAD_e && i_this->m10FC[3] != 0) {
            for (s32 i = 2; i <= 16; i++) {
                mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(i), *calc_mtx);
                cXyz pos_vec;
...
static void fly(bst_class* i_this) {
    ...
    if (boss->m2E9A != 0 || (i_this->mUpdateLastFacingDirIfMultipleOf32 & 0x1F) == 0) {
    ...
    case 2: {
        mDoMtx_YrotS(*calc_mtx, i_this->mLastCheckedPlayerFacingDir);
        cXyz vec;
...
static void down_attack(bst_class* i_this) {
    ...
    case 1: {
        ...
        target_diff = i_this->mTargetPos - i_this->current.pos;
    ...
    case 2: {
        ...
        if (target_diff.abs() < REG0_F(13) + 50.0f) {
            ...
            i_this->m10FC[0] = 30;
            if (REG0_S(0) == 1) {
                i_this->m2E6E = i_this->shape_angle.y + 0x4000;
            } else {
                i_this->m2E6E = i_this->shape_angle.y - 0x4000;
            }
    ...
    case 3:
        ...
        cLib_addCalcAngleS2(&i_this->shape_angle.y, i_this->m2E6E, 4, 0x1000);
        ...
        if (i_this->m10FC[0] == 0) {
            ...
            if (i_this->current.pos.y <= REG0_F(4) + 150.0f) {
                ...
                i_this->m10FC[0] = 30;
                cXyz particle_pos;
                particle_pos = i_this->current.pos;
                particle_pos.y = 1.0f;
                dComIfGp_particle_setProjection(dPa_name::ID_SCENE_C1D6, &particle_pos);
                dComIfGp_particle_setToon(dPa_name::ID_SCENE_A1D8, &particle_pos, NULL, NULL, 0xB9, &i_this->mPa_smokeEcallBack, fopAcM_GetRoomNo(i_this));
            }
        } else {
...
static void paa_attack(bst_class* i_this) {
    ...
    case 0:
        ...
        // fallthrough
    case 1:
        local_38 = i_this->mTargetPos - i_this->current.pos;
        if (local_38.abs() > REG0_F(0xc) + 200.0f) {
            cLib_addCalc2(&i_this->speedF, 40.0f, 1.0f, REG0_F(0xd) + 2.0f);
        } else {
            cLib_addCalc0(&i_this->speedF, 1.0f, (REG0_F(0xd) + 2.0f));
        }
        i_this->m10EC.x = REG0_F(5) + 8000.0f;
        mDoMtx_YrotS(*calc_mtx, (int)i_this->m111C);
        ...
        cLib_addCalcAngleS2(&i_this->shape_angle.y, i_this->m111C + sVar6, 10, 0x800);
        if (i_this->m10FC[0] == 0) {
            i_this->mDamage++;
            i_this->mTargetPos = player->current.pos;
            sVar6 = fopAcM_searchPlayerAngleY(i_this);
            i_this->current.angle.y = sVar6;
        }
        break;
    case 2:
        cLib_addCalc2(&i_this->speedF, REG0_F(0xe) + 140.0f, 1.0f, REG0_F(0xf) + 10.0f);
    ...
    case 3:
        ...
        if (i_this->m10FC[0] >= 0x1C) {
            if (i_this->m10FC[0] == 0x1c) {
                if (i_this->mBstPartType == bst_class::Type_LEFT_HAND_e) {
                    fopAcM_seStartCurrent(i_this, JA_SE_CM_BST_HAND_CLAP, 0);
                    csXyz local_70;
                    local_70.x = i_this->shape_angle.x;
                    local_70.z = i_this->shape_angle.z;
                    local_70.y += 0x4000;
                    dComIfGp_particle_setProjection(dPa_name::ID_SCENE_C1D7, &i_this->mTargetPos, &local_70);
                    dComIfGp_particle_setToon(
                        dPa_name::ID_SCENE_A1DA, &i_this->mTargetPos, &local_70, NULL, 0xB9, &i_this->mPa_smokeEcallBack, fopAcM_GetRoomNo(i_this)
                    );
                }
        ...
        cLib_addCalcAngleS2(&i_this->shape_angle.y, i_this->m111C + sVar6, 1, 0x1000);
        cLib_addCalc2(&i_this->current.pos.x, i_this->mTargetPos.x, 1.0f, std::fabsf(i_this->speed.x));
        cLib_addCalc2(&i_this->current.pos.z, i_this->mTargetPos.z, 1.0f, std::fabsf(i_this->speed.z));
        if (i_this->m10FC[0] == 0) {
            i_this->mDamage++;
            i_this->speedF = 0.0f;
        ...
        break;
    case 4:
        cLib_addCalc2(&i_this->speedF, 40.0f, 1.0f, REG0_F(0xd) + 1.0f);
        i_this->m10EC.x = REG0_F(5) + 2000.0f;
        if (i_this->m10FC[0] == 0) {
            bVar3 = true;
        }
    ...
    if (i_this->current.pos.y <= REG0_F(4) + 30.0f) {
        i_this->current.pos.y = REG0_F(4) + 30.0f;
    }
    if ((bVar3) || (i_this->mBgS_Acch.ChkWallHit())) {
        i_this->mActionType = bst_class::ACTION_FLY_e;
        i_this->mDamage = 0;
...
static void kumi_attack(bst_class* i_this) {
    ...
    case 1:
        ...
        MtxPosition(&local_38, &cStack_44);
        i_this->mTargetPos = player->current.pos + cStack_44;
        cLib_addCalcAngleS2(&i_this->shape_angle.y, i_this->m111C + sVar5, 10, 0x800);
        if (i_this->m10FC[0] == 0) {
            i_this->mDamage++;
            i_this->mTargetPos = player->current.pos;
            i_this->mTargetPos.y += REG0_F(3) + 500.0f;
            sVar5 = fopAcM_searchPlayerAngleY(i_this);
            i_this->current.angle.y = sVar5;
        }
        break;
    case 2:
        cLib_addCalc2(&i_this->speedF, REG0_F(0xe) + 100.0f, 1.0f, REG0_F(0xf) + 8.0f);
        i_this->m10EC.x = REG0_F(5) + 10000.0f;
        local_38 = i_this->mTargetPos - i_this->current.pos;
    ...
    case 3:
        ...
        cLib_addCalcAngleS2(&i_this->shape_angle.y, i_this->m111C + sVar5, 1, 0x1000);
        cLib_addCalc2(&i_this->current.pos.x, i_this->mTargetPos.x, 1.0f, std::fabsf(i_this->speed.x));
        cLib_addCalc2(&i_this->current.pos.y, i_this->mTargetPos.y, 1.0f, std::fabsf(i_this->speed.y) + 10.0f);
        cLib_addCalc2(&i_this->current.pos.z, i_this->mTargetPos.z, 1.0f, std::fabsf(i_this->speed.z));
        local_38 = hand[0]->current.pos - hand[1]->current.pos;
        if (local_38.abs() < 1.0f) {
            i_this->mDamage++;
            i_this->m10FC[0] = REG0_S(3) + 10;
            fopAcM_seStartCurrent(i_this, JA_SE_CM_BST_HAND_SHAKE, 0);
            i_this->speed.y = 0.0f;
        }
        break;
    case 4:
        ...
        if (i_this->m10FC[0] <= 1) {
            ...
            cLib_addCalcAngleS2(&i_this->shape_angle.z, i_this->mFacingDir, 1, 0x1000);
            set_hand_AT(i_this, 1);
            i_this->speed.y = i_this->speed.y - (REG0_F(3) + 60.0f);
            if (i_this->current.pos.y <= REG0_F(4) + 150.0f) {
                i_this->current.pos.y = REG0_F(4) + 150.0f;
                i_this->speed.y = 0.0f;
                if (i_this->mBstPartType == bst_class::Type_LEFT_HAND_e) {
                    ...
                    dComIfGp_particle_setToon(dPa_name::ID_SCENE_A1D9, &local_50, NULL, NULL, 0xb9, &i_this->mPa_smokeEcallBack, fopAcM_GetRoomNo(i_this));
                    dComIfGp_getVibration().StartShock(REG0_S(2) + 5, -0x21, cXyz(0.0f, 1.0f, 0.0f));
                    fopAcM_seStartCurrent(i_this, JA_SE_CM_BST_2_HAND_ATTACK, 0);
                }
                i_this->m112A = REG0_S(7) + 10;
                i_this->mDamage++;
                i_this->m10FC[0] = 0x28;
            ...
            i_this->speed.y += REG0_F(0xe) + 1.0f;
            if (i_this->mBstPartType == bst_class::Type_RIGHT_HAND_e) {
                i_this->mFacingDir = -0x1000;
            } else {
                i_this->mFacingDir = 0x1000;
            }
            cLib_addCalcAngleS2(&i_this->shape_angle.z, i_this->mFacingDir, 10, 0x200);
        }
        break;
    case 5:
        bVar1 = false;
    ...
    if ((bVar3) || (i_this->mBgS_Acch).ChkWallHit()) {
        ...
        i_this->mDamage = 0;
        i_this->speedF = 0.0f;
        i_this->current.angle.y = i_this->current.angle.y + 0x8000;
        i_this->mStateTimer = (s16)(int)(cM_rndF(100.0f) + 100.0f);
        i_this->mState = 0x28;
        fopAcM_seStartCurrent(i_this, JA_SE_CM_BST_HAND_RELEASE, 0);
        if (!bVar3) {
            s32 hand_index = 2 - i_this->mBstPartType;
            if (hand[hand_index]->mActionType == bst_class::ACTION_KUMI_ATTACK_e) {
                ...
                hand[hand_index]->mDamage = 0;
                hand[hand_index]->speedF = 0.0f;
                hand[hand_index]->current.angle.y += 0x8000;
                hand[hand_index]->mStateTimer = (s16)(int)(cM_rndF(100.0f) + 100.0f);
                hand[hand_index]->mState = 0x28;
                fopAcM_seStartCurrent(hand[hand_index], JA_SE_CM_BST_HAND_RELEASE, 0);
            }
        }
    }
}
...
static void harai_attack(bst_class* i_this) {
    ...
    case 0:
        ...
        i_this->current.angle.y = sVar2;
        anm_init(i_this, fly_bck_d[i_this->mBstPartType], 10.0f, J3DFrameCtrl::EMode_LOOP, 1.0f, -1);
        // fallthrough
    case 1:
        cVar3 = 1;
        cLib_addCalc2(&i_this->speedF, 60.0f, 1.0f, REG0_F(0xd) + 2.0f);
        i_this->m10EC.x = REG0_F(5) + 2000.0f;
        if (i_this->m10FC[0] == 0) {
            i_this->mDamage++;
    ...
    case 3:
        cVar3 = 1;
        cLib_addCalc0(&i_this->speedF, 1.0f, (REG0_F(0xd) + 3.0f));
        if (i_this->m10FC[0] == 0) {
            i_this->mActionType = bst_class::ACTION_FLY_e;
            i_this->mDamage = 0;
    ...
    cLib_addCalc2(&actor->current.pos.y, REG0_F(7) + 180.0f, 0.2f, 10.0);
...
static void beam_set(bst_class* i_this) {
    for (s32 i = 0; i < 10; i++) {
        ...
        i_this->m04E4[i] = 1;
        mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(REG0_S(5) + 3), *calc_mtx);
        cXyz vec(0.0f, 0.0f, 0.0f);
        MtxPosition(&vec, &i_this->m03B8[i]);
        i_this->m04A8[i] = i_this->shape_angle;
        mDoMtx_YrotS(*calc_mtx, i_this->m04A8[i].y);
        mDoMtx_XrotM(*calc_mtx, i_this->m04A8[i].x);
...
static void damage(bst_class* i_this) {
    ...
    if (i_this->m10FC[0] == 0) {
        if (i_this->mBstPartType != bst_class::Type_HEAD_e && i_this->health <= 0) {
...
static void head_damage(bst_class* i_this) {
    ...
    case 1:
        ...
        if (i_this->m10FC[0] == 0) {
            ...
            if (i_this->current.pos.y <= REG0_F(14)) {
                i_this->current.pos.y = REG0_F(14);
                dComIfGp_getVibration().StartShock(REG0_S(2) + 7, -0x21, cXyz(0.0f, 1.0f, 0.0f));
                dComIfGp_particle_setToon(
                    dPa_name::ID_SCENE_A1DB, &i_this->current.pos, NULL, NULL, 0xB9, &i_this->mPa_smokeEcallBack, fopAcM_GetRoomNo(i_this)
                );
                i_this->m112A = REG0_S(7) + 4;
                mDoAud_seStart(JA_SE_CM_BST_HEAD_FALL, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
                mDoAud_seStart(JA_SE_CM_BST_MOUTH_OPEN, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
    ...
    case 4: {
        ...
        if (bomb != NULL) {
            ...
            cLib_addCalc2(&bomb->current.pos.x, i_this->current.pos.x, 1.0f, 30.0f);
            cLib_addCalc2(&bomb->current.pos.y, i_this->current.pos.y + 90.0f + REG0_F(0xb), 1.0f, 30.0f);
            cLib_addCalc2(&bomb->current.pos.z, i_this->current.pos.z, 1.0f, 30.0f);
        }
        if (i_this->m10FC[0] == 0) {
            anm_init(i_this, BST_BCK_BOM_NOMI, 1.0f, J3DFrameCtrl::EMode_NONE, 1.0f, -1);
            i_this->mDamage++;
            i_this->m2EE0[0] = dComIfGp_particle_set(dPa_name::ID_SCENE_81E3, &i_this->current.pos);
    ...
    case 5:
        if (i_this->m10FC[0] == 0x32) {
            fopAcM_seStart(i_this, JA_SE_CM_BST_BOMB_EAT, 0);
        }
        if (i_this->m10FC[0] == 0x1e) {
            fopAcM_seStart(i_this, JA_SE_CM_BST_BEF_EXPLODE, 0);
        }
        if (i_this->m02B8->isStop()) {
            ...
            fopAcM_seStart(i_this, JA_SE_CM_BST_EXPLODE, 0);
            for (s32 i = 0; i < 4; i++) {
                i_this->m2EE0[i + 1] = dComIfGp_particle_set(bomb_eff_name[i], &i_this->current.pos);
            }
            i_this->m10FC[3] = 0x51;
        }
        break;
    case 6:
        if ((int)i_this->m02B8->getFrame() == 8) {
            fopAcM_seStart(i_this, JA_SE_CM_BST_JUMP_S, 0);
        }
        if (i_this->m02B8->isStop()) {
            i_this->m10FC[0] = 0x1e;
            i_this->mDamage++;
    ...
    for (s32 i = 0; i < 5; i++) {
        if (i_this->m2EE0[i] != NULL) {
            if ((i >= 1) && (i_this->m10FC[3] == 1)) {
                ...
                i_this->m2EE0[i]->setGlobalRTMatrix(i_this->m02B8->mpModel->getAnmMtx(0));
            }
        }
    }
}
...
static void head_hukki(bst_class* i_this) {
    ...
    i_this->shape_angle.x = i_this->m10F8 * cM_ssin(i_this->mUpdateLastFacingDirIfMultipleOf32 * (REG0_S(6) + 700));
    i_this->shape_angle.z = i_this->m10F8 * cM_ssin(i_this->mUpdateLastFacingDirIfMultipleOf32 * (REG0_S(6) + 600));
    cLib_addCalc2(&i_this->current.pos.y, REG0_F(11) + 400.0f, 0.05f, i_this->m10EC.y * 40.0f);
    cLib_addCalc2(&i_this->m10EC.z, REG0_F(11) + 50.0f, 1.0f, 2.0f);
...
static void col_set(bst_class* i_this) {
    ...
    cXyz pos_vec;
    if (i_this->mBstPartType == bst_class::Type_HEAD_e) {
        mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(8), *calc_mtx);
        ...
        i_this->mHandHurtCollisionCcD_Cyl.OffTgSetBit();
        ...
        vec.setall(0.0f);
        for (s32 i = 0; i < 2; i++) {
            mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(i + 4), *calc_mtx);
            MtxPosition(&vec, &pos_vec);
        ...
        mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(17), *calc_mtx);
...
static void damage_check(bst_class* i_this) {
    ...
    if (i_this->mDamageTimer == 0) {
        s8 bVar1 = false;
        if (i_this->mBstPartType == bst_class::Type_HEAD_e && i_this->mHeadHurtCollisionCcD_Cyl.ChkTgHit()) {
            atInfo.mpObj = i_this->mHeadHurtCollisionCcD_Cyl.GetTgHitObj();
            bVar1 = true;
        } else {
            for (s32 i = 0; i < (s32)ARRAY_SIZE(i_this->mCcD_fingers); i++) {
                if (i_this->mCcD_fingers[i].ChkTgHit()) {
                    atInfo.mpObj = i_this->mCcD_fingers[i].GetTgHitObj();
                    bVar1 = true;
                }
            }
        }
        if (bVar1) {
            def_se_set(i_this, atInfo.mpObj, 0x42);
        ...
        if (i_this->mBstPartType == bst_class::Type_HEAD_e) {
            if (i_this->mBstEyeCcD_Spheres[0].ChkTgHit() || i_this->mBstEyeCcD_Spheres[1].ChkTgHit() != 0) {
                ...
                if (player_way_check(i_this)) {
                    ...
                    if (i_this->mBstEyeCcD_Spheres[0].ChkTgHit()) {
                        atInfo.mpObj = i_this->mBstEyeCcD_Spheres[0].GetTgHitObj();
                        eye_index = 0;
                    } else {
                        atInfo.mpObj = i_this->mBstEyeCcD_Spheres[1].GetTgHitObj();
                        eye_index = 1;
                    }
                    ...
                    i_this->mDamage = 0;
                    mDoAud_seStart(JA_SE_CM_BST_EYE_HIT, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
                    ...
                    if (i_this->mEyeHealth[eye_index] <= 0) {
                        ...
                        mDoAud_seStart(JA_SE_CM_BST_EYE_BREAK, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
                    }
            ...
            if (i_this->mHandHurtCollisionCcD_Cyl.ChkTgHit()) {
                ...
                if (player_way_check(i_this)) {
                    ...
                    atInfo.mpObj = i_this->mHandHurtCollisionCcD_Cyl.GetTgHitObj();
                    atInfo.pParticlePos = i_this->mHandHurtCollisionCcD_Cyl.GetTgHitPosP();
...
static void hana_demo(bst_class* i_this) {
    ...
    case 1:
        ...
        if ((s32)i_this->m02B8->getFrame() == 26) {
            ...
            if (itemNo != -35) {
                mDoAud_seStart(JA_SE_OBJ_BOMB_EXPLODE, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
                cXyz pos_vec;
                MtxPosition(&vec, &pos_vec);
                i_this->mCreatedItemId = fopAcM_createItem(&pos_vec, itemNo, -1, fopAcM_GetRoomNo(i_this), 0, NULL, 0xB, NULL);
                mDoAud_seStart(JA_SE_CM_BST_ITEM_OUT_NOSE, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
                mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(0), *calc_mtx);
                vec.setall(0.0f);
                MtxPosition(&vec, &pos_vec);
                dComIfGp_particle_setToon(
                    item_smoke_name[i_this->m2E9E], &pos_vec, &i_this->shape_angle, NULL, 0xB9, &i_this->mPa_smokeEcallBack, fopAcM_GetRoomNo(i_this)
                );
            }
        }
        break;
...
static void end_demo(bst_class* i_this) {
    ...
    case 0:
        ...
        // fallthrough
    case 1:
        if ((s32)i_this->m02B8->mFrameCtrl.getFrame() == 20) {
    ...
    case 2:
        ...
        i_this->mDamage++;
        i_this->m2EF4 = dComIfGp_particle_set(dPa_name::ID_SCENE_81E8, &i_this->current.pos);
        ...
        for (s32 i = 0; i < 4; i++) {
            if (i_this->m2EE0[i + 1] != NULL) {
                if (i_this->m10FC[3] == 1) {
                    ...
                    i_this->m2EE0[i + 1]->setGlobalRTMatrix(i_this->m02B8->mpModel->getAnmMtx(0));
                }
            }
        }
    }
...
static void move(bst_class* i_this) {
    ...
    if (i_this->mBstPartType != bst_class::Type_HEAD_e && boss->m2E9A > 0 && boss->m2E9A < 10) {
        ...
        if (bVar1) {
            pos_diff = i_this->current.pos - i_this->old.pos;
...
void demo_camera(bst_class* i_this) {
    ...
    case 1:
        ...
        // fallthrough
    case 2:
        if (i_this->msFrameCount > REG0_S(4) + 33) {
            cLib_addCalc2(&i_this->m2EC4, -650.0f, 0.5f, REG0_F(3) + 50.0f);
        }
        if (i_this->msFrameCount > REG0_S(4) + 45) {
        ...
        i_this->m2EAC = i_this->eyePos + local_4c;
        mDoMtx_XrotM(*calc_mtx, REG0_S(2) + 4000);
        local_40.x = 0.0f;
        local_40.y = REG0_F(7);
        local_40.z = REG0_F(8) + 850.0f;
        MtxPosition(&local_40, &local_4c);
        i_this->m2EA0 = i_this->current.pos + local_4c;
    ...
    case 10:
        ...
        i_this->m2EC8 = 55.0f;
        i_this->m2EC4 = 0.0f;
        i_this->m2EA0.x = (boss->current.pos.x + REG0_F(1)) - 3200.0f;
        i_this->m2EA0.y = (boss->current.pos.y + REG0_F(2)) - 1100.0f;
        i_this->m2EA0.z = (boss->current.pos.z + REG0_F(3)) - 300.0f;
        i_this->m2EAC = boss->current.pos;
    ...
    case 11:
        ...
        local_4c.y = 0.0f;
        local_4c.z = 0.0f;
        apdVar4->setPlayerPosAndAngle(&local_4c, 0x4000);
        cLib_addCalc2(&i_this->m2EA0.z, boss->current.pos.z + REG0_F(3) + 300.0f, 0.02f, 1.4f);
        ...
        if ((int)i_this->msFrameCount == REG0_S(4) + 200) {
            message_set(i_this, 0x170e);
        }
        if (i_this->msFrameCount == REG0_S(4) + 0x122) {
            msg_end = 1;
        }
        if ((int)i_this->msFrameCount == REG0_S(4) + 0x14 + iVar14) {
            message_set(i_this, 0x170f);
        }
        if ((int)i_this->msFrameCount == (int)REG0_S(4) + ((int)REG0_S(4) + 0x14) + 0x186) {
            msg_end = 1;
        }
        if ((int)i_this->msFrameCount == (int)REG0_S(4) + ((int)REG0_S(4) + 0x14) + 0x19a) {
            i_this->m2E9A = 0xc;
            i_this->msFrameCount = 0;
            i_this->m2EAC = hand[0]->current.pos;
            i_this->m2EAC.y += REG0_F(4) + 200.0f;
            i_this->m2EA0 = hand[0]->current.pos;
            i_this->m2EA0.x += REG0_F(5) - 2000.0f;
            i_this->m2EA0.y += REG0_F(6);
        }
        // fallthrough
    case 12:
        cLib_addCalc2(&i_this->m2EA0.x, hand[0]->current.pos.x - 1200.0f, 0.1f, i_this->m2EC4 * 40.0f);
        cLib_addCalc2(&i_this->m2EC4, 1.0f, 1.0f, 0.01f);
        cLib_addCalc2(&i_this->m2EAC.x, hand[0]->current.pos.x, 0.02f, 50.0f);
        cLib_addCalc2(&i_this->m2EAC.z, hand[0]->current.pos.z, 0.02f, 50.0f);
        if (i_this->msFrameCount == 0x14) {
            hand[0]->m02D4 = 1;
            hand[0]->m02C4 = 1;
            pbVar5 = hand[0];
            mDoAud_seStart(JA_SE_CM_BST_HAND_LIGHT, &pbVar5->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(pbVar5)));
        }
        ...
        if (i_this->msFrameCount == 0x3c) {
            hand[0]->mDamage = 2;
            pbVar5 = hand[0];
            mDoAud_seStart(JA_SE_CM_BST_HAND_EYE_OPEN, &pbVar5->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(pbVar5)));
        }
        if (i_this->msFrameCount == 0x5a) {
            hand[0]->mDamage = 4;
            pbVar5 = hand[0];
            mDoAud_seStart(JA_SE_CM_BST_HAND_OUT_WALL, &pbVar5->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(pbVar5)));
        }
        ...
        if (i_this->msFrameCount == 0xaa) {
            ...
            i_this->msFrameCount = 0;
            i_this->m2EC4 = 0.0f;
            i_this->m2EAC = hand[1]->current.pos;
            i_this->m2EAC.y += REG0_F(4) + 200.0f;
            i_this->m2EA0 = hand[1]->current.pos;
            i_this->m2EA0.x += REG0_F(5) - 2000.0f;
            i_this->m2EA0.y += REG0_F(6);
        }
        // fallthrough
    case 13:
        cLib_addCalc2(&i_this->m2EA0.x, hand[1]->current.pos.x - 1200.0f, 0.1f, i_this->m2EC4 * 40.0f);
        cLib_addCalc2(&i_this->m2EC4, 1.0f, 1.0f, 0.01f);
        cLib_addCalc2(&i_this->m2EAC.x, hand[1]->current.pos.x, 0.02f, 50.0f);
        cLib_addCalc2(&i_this->m2EAC.z, hand[1]->current.pos.z, 0.02f, 50.0f);
        if (i_this->msFrameCount == 0x14) {
            hand[1]->m02D4 = 1;
            hand[1]->m02C4 = 1;
            pbVar5 = hand[1];
    ...
    case 14:
        if (i_this->msFrameCount >= 0x1E) {
            if (i_this->msFrameCount <= 0x5A) {
                cLib_addCalc2(&i_this->m2EA0.x, boss->current.pos.x - 1000.0f, 0.2f, i_this->m2EC4 * 100.0f);
            ...
            if (i_this->msFrameCount == 0x1e) {
                ...
                boss->m02C4 = 1;
                pbVar5 = boss;
                mDoAud_seStart(JA_SE_CM_BST_HEAD_LIGHT, &pbVar5->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(pbVar5)));
            }
            if (i_this->msFrameCount == 0x3b) {
                i_this->mRoomState = 1;
                i_this->m2ED0 = 0.0f;
            ...
            if (i_this->msFrameCount == 0x3c) {
                boss->mDamage = 2;
                pbVar5 = boss;
                mDoAud_seStart(JA_SE_CM_BST_HEAD_EYE_OPEN, &pbVar5->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(pbVar5)));
            }
            if (i_this->msFrameCount == 0x5a) {
                boss->mDamage = 4;
                pbVar5 = boss;
                mDoAud_seStart(JA_SE_CM_BST_HEAD_OUT_WALL, &pbVar5->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(pbVar5)));
            }
            if (i_this->msFrameCount == 0x96) {
                ...
                hand[1]->mDamage = 0;
                hand[0]->mDamage = 0;
                i_this->m2EAC = boss->eyePos;
                i_this->m2EAC.y += REG0_F(0);
                i_this->m2EA0 = apdVar4->current.pos;
        ...
        break;
    case 15:
        cLib_addCalc2(&i_this->m2EAC.x, boss->current.pos.x, 0.1f, 50.0f);
        cLib_addCalc2(&i_this->m2EAC.y, boss->eyePos.y + REG0_F(0), 0.1f, 50.0f);
        cLib_addCalc2(&i_this->m2EAC.z, boss->current.pos.z, 0.1f, 50.0f);
        i_this->m2EA0 = apdVar4->current.pos;
        i_this->m2EA0.x -= REG0_F(1) + 500.0f;
        i_this->m2EA0.y += REG0_F(2) + 100.0f;
    ...
    case 51:
        ...
        local_4c.y = 0.0f;
        local_4c.z = 0.0f;
        apdVar4[0].setPlayerPosAndAngle(&local_4c, 0x4000);
        i_this->current.pos.x = 300.0f;
        i_this->current.pos.z = 0.0f;
        i_this->shape_angle.y = -0x4000;
        i_this->m2EAC = i_this->current.pos;
        ...
        if ((i_this->msFrameCount > 100) && (i_this->msFrameCount < 0x212)) {
            mDoAud_seStart(JA_SE_CM_BST_GOKOU, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
        }
        ...
        if (i_this->msFrameCount == REG0_S(4) + 0x15e) {
            msg_end = 1;
        }
        if ((int)i_this->msFrameCount == REG0_S(4) + iVar14 + 0x14) {
            message_set(i_this, 0x1712);
        }
        if ((int)i_this->msFrameCount == (int)REG0_S(4) + iVar14 + 0x14 + 500) {
            msg_end = 1;
        }
        if ((int)i_this->msFrameCount == iVar14 * 2 + iVar14 + 0x14 + 0x208) {
            message_set(i_this, 0x1713);
        }
        if ((int)i_this->msFrameCount == iVar14 + (int)REG0_S(4) * 2 + 0x1cc + 0xaa) {
            msg_end = 1;
        }
        if ((int)i_this->msFrameCount == iVar14 + iVar13 + 200) {
            i_this->m2E9A = i_this->m2E9A + 1;
            i_this->msFrameCount = 0;
            i_this->m2EC4 = 0.0f;
        ...
        // fallthrough
    case 52:
        if (0x1e < i_this->msFrameCount) {
            i_this->m2E9A = i_this->m2E9A + 1;
            i_this->msFrameCount = 0;
            i_this->m2EC4 = 0.0f;
            apdVar4->mDemo.mDemoMode = 0x1a;
        }
        // fallthrough
    case 53:
        i_this->m2EAC = apdVar4->current.pos;
        ...
        if (i_this->msFrameCount > (short)(REG0_S(3) + 0x41)) {
            ...
            local_40.y = 0.0f;
            local_40.z = 0.0f;
            fopAcM_createWarpFlower(&local_40, 0, fopAcM_GetRoomNo(i_this), 0);
        }
        // fallthrough
    case 54:
        if ((int)i_this->msFrameCount == REG0_S(4) + 0x16) {
            (apdVar4->mDemo).mDemoMode = 0x18;
        }
        if ((int)i_this->msFrameCount == REG0_S(5) + 0x96) {
            (apdVar4->mDemo).mDemoMode = 0x1d;
        }
        ...
        // fallthrough
    case 55:
        cLib_addCalc2(&i_this->m2EAC.x, i_this->current.pos.x, 0.2f, 20.0f);
        cLib_addCalc2(&i_this->m2EAC.y, i_this->current.pos.y + 100.0f + REG0_F(10), 0.2f, 20.0f);
        cLib_addCalc2(&i_this->m2EAC.z, i_this->current.pos.z, 0.2f, 20.0f);
        i_this->m2EA0 = i_this->current.pos;
        ...
        local_40 = i_this->home.pos - i_this->current.pos;
        if (local_40.abs() < 10.0f) {
            ...
            dComIfGp_getVibration().StartShock(REG0_S(2) + 3, -0x21, cXyz(0.0f, 1.0f, 0.0f));
            mDoAud_seStart(JA_SE_CM_BST_HEAD_IN_WALL, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
            i_this->m2ECC = REG0_F(9) + 10.0f;
    ...
    case 56:
        if (i_this->msFrameCount == REG0_S(4) + 50) {
            anm_init(i_this, BST_BCK_HANA_DEMO, 5.0f, J3DFrameCtrl::EMode_NONE, 1.0f, -1);
            mDoAud_seStart(JA_SE_CM_BST_HAND_STRAIN, &i_this->current.pos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
            i_this->m2E98 = 1;
        }
        if ((s32)i_this->m02B8->getFrame() == 26) {
            cXyz boss_pos;
            mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(7), *calc_mtx);
            cXyz offset(REG0_F(2) + 60.0f, REG0_F(3), REG0_F(4));
            mDoAud_seStart(JA_SE_OBJ_BOMB_EXPLODE, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
            MtxPosition(&offset, &boss_pos);
            cXyz item_scale(1.0f, 1.0f, 1.0f);
            csXyz item_angle;
            item_angle.x = i_this->shape_angle.x;
            item_angle.z = i_this->shape_angle.z;
            item_angle.y += REG0_S(7) + (-300);
            i_this->mCreatedItemId = fopAcM_createItemForBoss(&boss_pos, 0, fopAcM_GetRoomNo(i_this), &item_angle, &item_scale, 1);
            mDoAud_seStart(JA_SE_CM_BST_ITEM_OUT_NOSE, &i_this->eyePos, 0, dComIfGp_getReverb(fopAcM_GetRoomNo(i_this)));
            mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(0), *calc_mtx);
            cXyz vec(0.0f, 0.0f, 0.0f);
            MtxPosition(&vec, &boss_pos);
            dComIfGp_particle_set(item_smoke_name[1], &boss_pos, &i_this->shape_angle, NULL, 0xB9, &i_this->mPa_smokeEcallBack, fopAcM_GetRoomNo(i_this));
        }
        if ((s32)i_this->m02B8->getFrame() > 26) {
    ...
    if ((i_this->m2E9A != 0) && bVar2) {
        ...
        camera->mCamera.Set(camera_center, camera_eye, bank, i_this->m2EC8);
...
static BOOL daBst_Execute(bst_class* i_this) {
    ...
    if (l_HIO.m5 == 0) {
        ...
        i_this->m02B8->play(&i_this->eyePos, 0, 0);
        if (i_this->m02C4 != 0) {
    ...
    f32 unk1 = i_this->m10EC.z * cM_ssin(i_this->mUpdateLastFacingDirIfMultipleOf32 * (REG0_S(6) + 700));
    f32 unk2 = i_this->m10EC.z * cM_ssin(i_this->mUpdateLastFacingDirIfMultipleOf32 * (REG0_S(6) + 750));
    f32 unk3 = i_this->m10EC.z * cM_scos(i_this->mUpdateLastFacingDirIfMultipleOf32 * (REG0_S(6) + 720));
    cLib_addCalc0(&i_this->m10EC.z, 1.0f, 1.0f);
    i_this->m02B8->getModel()->setBaseScale(i_this->scale);
    mDoMtx_stack_c::transS(i_this->current.pos.x + unk1, i_this->current.pos.y + unk2, i_this->current.pos.z + unk3);
    s16 unk4 = i_this->mHeadHurtAngle * (REG0_F(14) + 500.0f) * cM_ssin(i_this->mUpdateLastFacingDirIfMultipleOf32 * 0x2100);
    s16 unk5 = i_this->mHeadHurtAngle * (REG0_F(14) + 500.0f) * cM_scos(i_this->mUpdateLastFacingDirIfMultipleOf32 * 0x2300);
    s16 unk6 = i_this->m112A * (REG0_F(14) + 150.0f) * cM_ssin(boss->mUpdateLastFacingDirIfMultipleOf32 * 0x3600);
    s16 unk7 = i_this->m112A * (REG0_F(14) + 150.0f) * cM_scos(boss->mUpdateLastFacingDirIfMultipleOf32 * 0x4300);
    s16 unk8 = i_this->m112C * (REG0_F(14) + 100.0f) * cM_scos(boss->mUpdateLastFacingDirIfMultipleOf32 * 0x3A00);
    cLib_addCalc0(&i_this->m10F8, 1.0f, REG0_F(3) + 30.0f);
    mDoMtx_stack_c::YrotM(i_this->shape_angle.y + unk4 + unk6);
    mDoMtx_stack_c::XrotM(i_this->shape_angle.x + unk5 + unk7 + unk8);
    ...
    if (i_this->mBstPartType == bst_class::Type_HEAD_e) {
        ...
        for (s32 j = 0; j < 2; j++) {
            ...
            if (actor != NULL) {
                mDoMtx_copy(i_this->m02B8->getModel()->getAnmMtx(j + 4), *calc_mtx);
...
static cPhs_State daBst_Create(fopAc_ac_c* a_this) {
    ...
    if (res == cPhs_COMPLEATE_e) {
        ...
        i_this->mBgS_Acch.Set(&i_this->current.pos, &i_this->old.pos, i_this, 1, &i_this->mBgS_AcchCir, &i_this->speed);
        ...
        if (i_this->mBstPartType == bst_class::Type_HEAD_e) {
            ...
            i_this->mEyeHealth[0] = 2;
            for (s32 i = 0; i < (s32)ARRAY_SIZE(i_this->m2E90); i++) {
                i_this->m2E90[i] = fopAcM_create(PROC_ATT, 100, &i_this->current.pos, i_this->current.roomNo);
            }
