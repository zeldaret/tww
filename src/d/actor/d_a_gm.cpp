...
static BOOL daGM_Draw(gm_class* i_this) {
    ...
    if (i_this->m02CD == 0) {
        ...
        switch (i_this->m0324) {
            case 1:
                i_this->mDamageBrkAnm->entry(model->getModelData());
                break;
            case 2:
                i_this->mAtackBrkAnm->entry(model->getModelData());
                break;
            case 3:
                i_this->mDeadBrkAnm->entry(model->getModelData());
                break;
            default:
                i_this->mBodyGlowBrkAnm->entry(model->getModelData());
                break;
        }
        i_this->mpMorf->entryDL();
        switch (i_this->m0324) {
            case 1:
                i_this->mDamageBrkAnm->remove(model->getModelData());
                break;
            case 2:
                i_this->mAtackBrkAnm->remove(model->getModelData());
                break;
            case 3:
                i_this->mDeadBrkAnm->remove(model->getModelData());
                break;
            default:
                i_this->mBodyGlowBrkAnm->remove(model->getModelData());
                break;
        }
    } else {
    ...
    J3DModel* model2 = i_this->m0444;
    for (i = 0; i < modelData->getJointNum(); i++) {
        model2->setAnmMtx(i, model->getAnmMtx(i));
    }
    for (int i = 0; i < modelData->getWEvlpMtxNum(); i++) {
        model2->setWeightAnmMtx(i, model->getWeightAnmMtx(i));
    }
    local_30
        .set(actor->current.pos.x + i_this->m03B4.x, REG8_F(18) + (10.0f + (actor->current.pos.y + i_this->m03B4.y)), actor->current.pos.z + i_this->m03B4.z);
    i_this->mShadowId = dComIfGd_setRealShadow(
        ...
        &local_30,
        REG8_F(19) + 900.0f,
        (actor->current.pos.y + i_this->m03B4.y) - i_this->mAcch.GetGroundH(),
        NULL
    );
    if (REG8_S(1) == 0) {
        return TRUE;
    }
    if ((i_this->m0360[0].x) || (i_this->m0360[0].z)) {
        if ((i_this->m0360[1].x) || (i_this->m0360[1].z)) {
...
static void anm_init(gm_class* i_this, int bckFileIdx, f32 morf, u8 loopMode, f32 speed, int soundFileIdx) {
    ...
    if (soundFileIdx >= 0) {
        i_this->mpMorf
            ->setAnm((J3DAnmTransform*)dComIfG_getObjectRes("GM", bckFileIdx), loopMode, morf, speed, 0.0f, -1.0f, dComIfG_getObjectRes("GM", soundFileIdx));
    } else {
...
static BOOL wing_cut_stat(gm_class* i_this) {
    ...
    for (i = 0, uVar4 = 1; i < 4; i++, uVar4 *= 2) {
        if (!(i_this->m02CC & uVar4)) {
            ...
            local_88.y += -0x8000;
            fopAcM_create(fpcNm_GM_e, parameters, &cStack_44, fopAcM_GetRoomNo(actor), &local_88, &actor->scale, 0, NULL);
...
static BOOL body_atari_check(gm_class* i_this) {
    ...
    if ((hitObj->GetAtType() == AT_TYPE_LIGHT_ARROW) || (hitObj->GetAtType() == AT_TYPE_ICE_ARROW)) {
        return TRUE;
    }
...
static BOOL ks_set_rtn(gm_class* i_this) {
    ...
    fpc_ProcID fVar4 =
        fopAcM_createChild(fpcNm_KS_e, fopAcM_GetID(i_this), parameters, &i_this->m03FC, fopAcM_GetRoomNo(actor), &local_30, &actor->scale, 0, NULL);
...
static void action_dousa(gm_class* i_this) {
    ...
    switch (i_this->mMode) {
        case MODE_DOUSA_0:
            ...
            if (i_this->mCurrBckIdx != dRes_INDEX_GM_BCK_FLY_e) {
        ...
        case MODE_DOUSA_3:
            ...
            if (!((std::fabsf(actor->current.pos.y - i_this->m0348.y) > 40.0f) || (i_this->mCountDownTimers[1] != 0))) {
                ...
                actor->speedF = 50.0f;
                i_this->mAtSph.OnAtSPrmBit(cCcD_AtSPrm_Set_e);
        ...
        case MODE_DOUSA_4:
            if (i_this->mCountUpTimers[3] == 0) {
                if (i_this->mAtSph.ChkAtShieldHit()) {
                    i_this->mAtSph.OffAtSPrmBit(cCcD_AtSPrm_Set_e);
                    i_this->mAtSph.ClrAtSet();
                    actor->speedF = -25.0f;
                    i_this->mCountUpTimers[3] = 1;
                } else {
                    if (player->getDamageWaitTimer()) {
                        actor->speedF = -50.0f;
                        i_this->mCountUpTimers[3] = 1;
                    }
                }
...
static void action_uchiwa_dousa(gm_class* i_this) {
    ...
    switch (i_this->mMode) {
        case MODE_UCHIWA_DOUSA_20:
            ...
            i_this->mAtSph.OffAtSPrmBit(cCcD_AtSPrm_Set_e);
            i_this->mAtSph.ClrAtSet();
            i_this->mWingCyl.OffAtSPrmBit(cCcD_AtSPrm_Set_e);
            i_this->mWingCyl.ClrAtSet();
            actor->speedF = 60.0f;
            actor->speed.y = 24.0f;
            if (i_this->m02D1 == 3) {
                actor->speedF = 20.0f;
...
static void action_totugeki(gm_class* i_this) {
    ...
    switch (i_this->mMode) {
        ...
        case MODE_TOTUGEKI_33:
            ...
            i_this->mWingCyl.OnAtSPrmBit(cCcD_AtSPrm_Set_e);
        ...
        case MODE_TOTUGEKI_34:
            {
                ...
                if ((i_this->m0384[0].x) || (i_this->m0384[0].z) || (i_this->m0384[1].x) || (i_this->m0384[1].z)) {
                    if (((i_this->m0384[0].x) || (i_this->m0384[0].z)) && ((i_this->m0384[1].x) || (i_this->m0384[1].z))) {
                        actor->speedF = 0.0f;
                        actor->gravity = 0.0f;
                        actor->speed.y = 0.0f;
                        ...
                        if ((i_this->m0384[0].x) || (i_this->m0384[0].z)) {
    ...
    switch (i_this->mMode) {
        ...
        case MODE_KABEHARI_41:
            ...
            if (((fVar1 < 4.0f) || (i_this->mAcch.ChkWallHit())) || (i_this->mCountDownTimers[1] == 0)) {
                if (i_this->mCurrBckIdx != dRes_INDEX_GM_BCK_TOMARU_e) {
...
static void action_fly_damage(gm_class* i_this) {
    ...
    switch (i_this->mMode) {
        ...
        case MODE_FLY_DAMAGE_51:
            ...
            for (s32 i = 0; i < 6; i++) {
                cLib_addCalcAngleS2(&i_this->mCountUpTimers[i + 16], 0, 1, 0x50);
            }
            ...
            if (actor->speedF < 0.2f) {
                ...
                i_this->mBodyGlowBrkAnm->setFrame(0.0f);
                i_this->m0324 = 0;
                if ((i_this->mCurrBckIdx == dRes_INDEX_GM_BCK_PAUSE_e) && ((i_this->m02CC & 7) != 7)) {
                    i_this->mAction = ACTION_TOTUGEKI;
                    i_this->mMode = MODE_TOTUGEKI_37;
                } else {
                    i_this->mAction = ACTION_DOUSA;
...
static void action_ground_attack(gm_class* i_this) {
    ...
    switch (i_this->mMode) {
        ...
        case MODE_GROUND_ATTACK_72:
            ...
            if (!Line_check(i_this, local_60)) {
                ...
                if (i_this->m02CE != 1) {
                    if (i_this->mCurrBckIdx != dRes_INDEX_GM_BCK_G_WAIT_e) {
                        anm_init(i_this, dRes_INDEX_GM_BCK_G_WAIT_e, 2.0f, J3DFrameCtrl::EMode_LOOP, 1.0f, -1);
                    }
                    break;
                }
    ...
    if (((i_this->mCurrBckIdx != dRes_INDEX_GM_BCK_G_DEAD01_e) && (i_this->mCurrBckIdx != dRes_INDEX_GM_BCK_G_DEAD02_e)) && (body_atari_check(i_this))) {
        i_this->mAction = ACTION_GROUND_ATTACK;
        if (i_this->m02D1 == 7) {
            actor->health = 0;
        }
...
static void action_demo(gm_class* i_this) {
    ...
    switch (i_this->mMode) {
        ...
        case MODE_DEMO_202:
            ...
            local_f0.y = DEMO_SELECT(player_actor->current.pos.y, player->current.pos.y);
        ...
        case MODE_DEMO_208:
            fopAcM_seStart(actor, JA_SE_CM_GM_JET, 0);
            if (actor->current.pos.y < DEMO_SELECT(player_actor->current.pos.y, player->current.pos.y) + 220.0f) {
                actor->gravity = 0.0f;
                actor->speed.y = 0.0f;
                cLib_addCalc2(&actor->current.pos.y, DEMO_SELECT(player_actor->current.pos.y, player->current.pos.y) + 220.0f, 1.0f, 50.0f);
            }
            cLib_addCalcAngleS2(&actor->shape_angle.y, actor->current.angle.y, 1, 0x1000);
        ...
        case MODE_DEMO_221:
            {
#if VERSION == VERSION_DEMO
                if ((actor->speedF) && (i_this->mAcch.ChkGroundHit()))
#else
                if ((actor->speedF) && ((i_this->mCountDownTimers[0] == 0) || (i_this->mAcch.ChkGroundHit())))
#endif
                {
                    if (i_this->mCurrBckIdx != dRes_INDEX_GM_BCK_G_DEAD02_e) {
                        anm_init(i_this, dRes_INDEX_GM_BCK_G_DEAD02_e, 1.0f, J3DFrameCtrl::EMode_NONE, 1.0f, -1);
                    }
                    actor->speedF = 0.0f;
                }
                player->setPlayerPosAndAngle(
                    DEMO_SELECT(&player_actor->current.pos, &player->current.pos),
                    (int)(s16)(fopAcM_searchPlayerAngleY(actor) + 0x8000)
                );
                if (REG8_S(8) == 0) {
                    if ((!actor->speedF) && (i_this->mCurrBckIdx == dRes_INDEX_GM_BCK_G_DEAD02_e)) {
                        if (!i_this->mpMorf->isStop()) {
                            break;
                        }
                        i_this->mDeadBrkAnm->setFrame(0.0f);
    ...
    if (i_this->mMode >= MODE_DEMO_221) {
        ...
        mDoMtx_YrotS(*calc_mtx, fopAcM_searchActorAngleY(DEMO_SELECT(player_actor, player), actor));
        local_e4.set(0.0f, 0.0f, 40.0f);
        MtxPosition(&local_e4, &local_f0);
        local_f0 += DEMO_SELECT(player_actor->current.pos, player->current.pos);
...
static BOOL daGM_Execute(gm_class* i_this) {
    ...
    if (i_this->m02CD == 0) {
        fopAcM_setGbaName(actor, dItemNo_BOOMERANG_e, 9, 0x26);
        if (i_this->m044C != 1) {
            if (!(i_this->m02CC & 4)) {
                switch (i_this->m02D3) {
                    case 0:
...
static BOOL useHeapInit(fopAc_ac_c* a_this) {
    ...
    if (i_this->m02CD == 0) {
        ...
        J3DAnmTevRegKey* pBrk = (J3DAnmTevRegKey*)dComIfG_getObjectRes("GM", dRes_INDEX_GM_BRK_GM_e);
        if (!i_this->mBodyGlowBrkAnm->init(model->getModelData(), pBrk, true, J3DFrameCtrl::EMode_LOOP)) {
            return FALSE;
        }
        ...
        pBrk = (J3DAnmTevRegKey*)dComIfG_getObjectRes("GM", dRes_INDEX_GM_BRK_ATACK_e);
        if (!i_this->mAtackBrkAnm->init(model->getModelData(), pBrk, true, J3DFrameCtrl::EMode_LOOP)) {
            return FALSE;
        }
        ...
        pBrk = (J3DAnmTevRegKey*)dComIfG_getObjectRes("GM", dRes_INDEX_GM_BRK_DAMAGE_e);
        if (!i_this->mDamageBrkAnm->init(model->getModelData(), pBrk, true, J3DFrameCtrl::EMode_NONE)) {
            return FALSE;
        }
        ...
        pBrk = (J3DAnmTevRegKey*)dComIfG_getObjectRes("GM", dRes_INDEX_GM_BRK_DEAD_e);
        if (!i_this->mDeadBrkAnm->init(model->getModelData(), pBrk, true, J3DFrameCtrl::EMode_NONE)) {
            return FALSE;
        }
    ...
    i_this->m0444 = mDoExt_J3DModel__create(modelData, 0x20000, 0x11020203);
