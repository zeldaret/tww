...
void defence0(gnd_class* i_this) {
    ...
    case 0:
        if ((i_this->m03D8 != 2) || (player->current.pos.y - i_this->mAcch.GetGroundH() > REG0_F(0x13) + 300.0f)) {
            ...
            if (((player->mCutType == 15) && (fVar8 = cM_rndF(1.0f), fVar8 < 0.8f)) ||
                ((i_this->m03DB == 1 && (fVar8 = cM_rndF(1.0f), fVar8 < 0.05f)) && (i_this->m03D8 == 2)))
            {
                i_this->m03DC = 5;
        ...
        anm_init(i_this, bougyo_d[i_this->m03DC * 4], 4.0f, J3DFrameCtrl::EMode_NONE, 1.0f, -1);
        i_this->m03DA = 0;
    ...
    case 10:
        if (((i_this->m03D8 == 2) && (player->current.pos.y - i_this->mAcch.GetGroundH() > REG0_F(0x13) + 300.0f)) && (i_this->m03DC != 0)) {
            i_this->m03DC = 0;
            i_this->mBlockHitCount = 2;
            anm_init(i_this, bougyo_d[i_this->m03DC * 4], 2.0f, J3DFrameCtrl::EMode_NONE, 1.0f, -1);
            break;
        }
...
void demo_camera(gnd_class* i_this) {
    ...
    if (i_this->m155E != 0) {
        fVar2 = i_this->m15AC;
        local_f0.x = i_this->m1564.x + fVar2 * cM_ssin(i_this->m1560 * 0x3500);
        local_f0.y = i_this->m1564.y + fVar2 * cM_scos(i_this->m1560 * 0x3900);
        local_f0.z = i_this->m1564.z;
        local_e4.x = i_this->m1570.x + cM_ssin(i_this->m1560 * 0x3500);
        dVar20 = local_e4.x;
        local_e4.y = i_this->m1570.y + fVar2 * cM_scos(i_this->m1560 * 0x3900);
        local_e4.z = i_this->m1570.z;
        iVar9 = (dVar19 + (fVar2 * cM_scos(i_this->m02CC * 0x1c00) * 7.5f));
        pcVar18->mCamera.Set(local_e4, local_f0, (s16)iVar9, i_this->m15A4);
        cLib_addCalc0(&i_this->m15AC, (REG0_F(0x10) + 1.0f), dVar20);
...
static BOOL daGnd_Execute(gnd_class* i_this) {
    ...
    if ((actor->base.mParameters & 0xffU) == 0x23) {
        i_this->mMoveType = gnd_class::Move_Type_YAWAIT_e;
        i_this->m155E = 0x14;
        actor->base.mParameters = 0;
    }
    i_this->m02CC++;
    for (s32 i = 0; i < 5; i++) {
        if (i_this->m0302[i] != 0) {
    ...
    pJVar7->setBaseScale(actor->scale);
    mDoMtx_stack_c::transS(actor->current.pos.x, actor->current.pos.y, actor->current.pos.z);
    cLib_addCalc0(&i_this->m03E0, 0.5f, (REG0_F(0x12) + 6053.5f));
    ...
    for (s32 i = 0; i < 2; i++) {
        ...
        if ((i_this->m13B8[i] == 0) || (player->mCutType == 5 && (i_this->m03D8 < 2))) {
            ...
            if (i_this->m13B8[i] == 3) {
                ...
                i_this->m13BC[i] = at_spl_d[l_HIO.m84 * 4];
                ...
                i_this->m13BC[i] = at_spl_d[l_HIO.m82 * 4];
                if (i_this->m13B8[i] == 4) {
            ...
            MtxPosition(&local_8c, i_this->m13A0);
            sph->SetC(*i_this->m13A0);
            mDoMtx_YrotS(*calc_mtx, actor->shape_angle.y);
            local_8c.x = 0.0f;
            local_8c.y = 0.0f;
            local_8c.z = 1.0f;
