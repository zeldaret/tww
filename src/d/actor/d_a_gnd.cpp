...
static u8 j_demo(gnd_class* i_this) {
    ...
    if (actor->demoActorID == 0) {
        ...
        i_this->m15B0 = 1;
        dDemo_actor_c* demo_actor = dComIfGp_demo_getActor(actor->demoActorID);
        J3DAnmTexPattern* pBtp = (J3DAnmTexPattern*)demo_actor->getP_BtpData("Gnd");
        if (pBtp != NULL) {
            i_this->mpBtpAnm->init(i_this->mpMorf->getModel()->getModelData(), pBtp, 1, 0, 1.0f, 0, -1, true, 0);
        }
        J3DAnmTextureSRTKey* pAnm = (J3DAnmTextureSRTKey*)demo_actor->getP_BtkData("Gnd");
        if (pAnm != NULL) {
            i_this->mpBtkAnm->init(i_this->mpMorf->getModel()->getModelData(), pAnm, true, J3DFrameCtrl::EMode_NONE, 1.0f, 0, -1, true, 0);
        }
        J3DAnmTevRegKey* pBrk = (J3DAnmTevRegKey*)demo_actor->getP_BrkData("Gnd");
        if (pBrk != NULL) {
            i_this->mpBrkAnm->init(i_this->mpMorf->getModel()->getModelData(), pBrk, true, J3DFrameCtrl::EMode_LOOP, 1.0f, 0, -1, true, 0);
        }
        dDemo_setDemoData(
            actor,
...
static void splash_set(gnd_class* i_this) {
    ...
    if (emitter != NULL) {
        emitter->setGlobalPrmColor(i_this->mTevStr.mColorC0.r, i_this->mTevStr.mColorC0.g, i_this->mTevStr.mColorC0.b);
    }
}
...
static void attack_eff_set(gnd_class* i_this, int param_2) {
    fopAc_ac_c* actor = &i_this->actor;
    for (s32 i = 0; i < ARRAY_SSIZE(i_this->mpAttackEffEmitter); i++) {
...
static void attack_eff_move(gnd_class* i_this) {
    ...
    for (s32 i = 0; i < ARRAY_SSIZE(i_this->mpAttackEffEmitter); i++) {
        if (i_this->mpAttackEffEmitter[i] != NULL) {
            i_this->mpAttackEffEmitter[i]->setGlobalRTMatrix(model->getAnmMtx(attack_eff_joint_d[i]));
        }
    }
}
...
static void anm_init(gnd_class* i_this, int bckFileIdx, f32 morf, u8 loopMode, f32 speed, int soundFileIdx) {
    if (soundFileIdx >= 0) {
        i_this->mpMorf
            ->setAnm((J3DAnmTransform*)dComIfG_getObjectRes("Gnd", bckFileIdx), loopMode, morf, speed, 0.0f, -1.0f, dComIfG_getObjectRes("Gnd", soundFileIdx));
    } else {
    ...
    for (s32 i = 0; i < ARRAY_SSIZE(unk_array); i++) {
...
static s32 player_view_check(gnd_class* i_this, s16 param_2) {
    ...
    s16 angleDiff = actor->shape_angle.y - param_2;
    if (angleDiff < 0) {
        angleDiff = -angleDiff;
    }
    if ((u16)(angleDiff & 0xFFFF) < 0x4000) {
        return TRUE;
    }
    return FALSE;
}

// Unused, but exists according to the debug map.
static BOOL player_way_check(gnd_class* i_this) {
    fopAc_ac_c* actor = &i_this->actor;
    s16 angleDiff = actor->shape_angle.y - dComIfGp_getPlayer(0)->shape_angle.y;
    if (angleDiff < 0) {
        angleDiff = -angleDiff;
    }
...
static void ke_pos_set(gnd_class* i_this, gnd_ke_s* param_2, int index) {
    cXyz* pcVar3 = i_this->mLineMat.getPos(index);
    for (s32 i = 0; i < ARRAY_SSIZE(param_2->m000); pcVar3++, i++) {
...
static void ke_move(gnd_class* i_this) {
    ...
    pDst = &i_this->m03E4[0];
    model = i_this->mpMorf->getModel();
    for (s32 i = 0; i < ARRAY_SSIZE(i_this->m03E4); i++, pDst++) {
...
static void attack0(gnd_class* i_this) {
    ...
    if (bVar3) {
        cLib_addCalc2(&actor->current.pos.x, player->current.pos.x, 0.8f, std::fabsf(i_this->m02E0.x));
        cLib_addCalc2(&actor->current.pos.z, player->current.pos.z, 0.8f, std::fabsf(i_this->m02E0.z));
    }
    if (bVar4) {
        i_this->mAction = ACTION_MOVE0_e;
        i_this->mAnmType = 0;
...
static void attack1(gnd_class* i_this) {
    ...
    int frame = i_this->mpMorf->getFrame();
    s8 r31 = false;
    i_this->m13C4[0] = i_this->m13C4[1] = REG0_F(13) + 110.0f;
    switch (i_this->mAnmType) {
        case 0:
    ...
    switch (i_this->mAnmType) {
        ...
        case 1:
            ...
            if (i_this->m0302[0] == 0) {
                ...
                fopAcM_monsSeStart(actor, JA_SE_CV_GN_NOTICE, 0);
                J3DAnmTextureSRTKey* mBtk = (J3DAnmTextureSRTKey*)dComIfG_getObjectRes("Gnd", dRes_INDEX_GND_BTK_KIZUKU_EYE_e);
                i_this->mpBtkAnm->init(i_this->mpMorf->getModel()->getModelData(), mBtk, true, J3DFrameCtrl::EMode_NONE, 1.0f, 0, -1, true, 0);
                i_this->mAnmType = 2;
            }
            break;
        case 2:
        ...
        case 6:
            ...
            if (frame == 12) {
                ...
                i_this->m02F8 = (fVar17 * (REG0_F(18) + 0.018f));
                splash_set(i_this);
            }
            if (i_this->mpMorf->isStop()) {
        ...
        case 0xB:
            if (frame == REG0_S(5) + 2) {
                ...
                r28->setDown();
                r28 = pz;
                fopAcM_seStart(r28, JA_SE_LK_LAST_HIT, 0);
...
static void defence0(gnd_class* i_this) {
    ...
    switch (i_this->mAnmType) {
        case 0:
#if VERSION == VERSION_DEMO
            if (((player->getCutType() == daPy_py_c::CUT_TYPE_BT_ROLLCUT) && (cM_rndF(1.0f) < 0.8f)) ||
                ((i_this->m03DB == 1 && (cM_rndF(1.0f) < 0.05f)) && (i_this->m03D8 == 2)))
            {
                i_this->m03DC = 5;
...
static void damage_check(gnd_class* i_this) {
    ...
    daPy_py_c* player = (daPy_py_c*)dComIfGp_getPlayer(0);
    i_this->mStts.Move();
    for (s32 i = 0; i < ARRAY_SSIZE(i_this->mSwordSphs); i++) {
        if (i_this->mSwordSphs[i].ChkTgHit()) {
            def_se_set(actor, i_this->mSwordSphs[i].GetTgHitObj(), 0x40);
        ...
        if (i_this->mSwordSphs[i].ChkAtHit()) {
            fopAc_ac_c* hit_ac = dCc_GetAc(i_this->mSwordSphs[i].GetAtHitObj()->GetAc());
            if (fopAcM_GetName(hit_ac) == fpcNm_PLAYER_e) {
...
#endif
        arrow = (daArrow_c*)dCc_GetAc(atInfo.mpObj->GetAc());
        if (((arrow != NULL) && (fopAcM_GetName(arrow) == fpcNm_ARROW_e)) && (arrow->isSetByZelda())) {
            uVar7 = actor->shape_angle.y - arrow->current.angle.y;
            if (uVar7 < 0) {
                uVar7 = -uVar7;
            }
            if ((((u16)(uVar7 & 0xFFFF) < 0x4000) && (i_this->m03D8 < 2)) || (arrow->isLinkReflect())) {
                r22 = true;
            } else {
                i_this->m13CE = 2;
            }
        }
        if ((!r22) && (i_this->m13CE != 0)) {
    ...
    if ((i_this->mDamageInvulnerabilityTimer2 == 0) || (i_this->mChestSph.ChkTgHit())) {
        ...
        if ((i_this->mDamageCyl.ChkTgHit()) || (i_this->mHeadSph.ChkTgHit() || (i_this->mChestSph.ChkTgHit()))) {
            ...
            if (i_this->mHeadSph.ChkTgHit()) {
                ...
                atInfo.mpObj = i_this->mDamageCyl.GetTgHitObj();
                atInfo.pParticlePos = i_this->mDamageCyl.GetTgHitPosP();
            }
            atInfo.mpActor = cc_at_check(actor, &atInfo);
            if ((r21) && (atInfo.mpActor != NULL)) {
                dComIfGp_particle_set(dPa_name::ID_AK_SN_GNDLIGHTARROW00, atInfo.pParticlePos, &atInfo.mpActor->current.angle);
                dComIfGp_particle_set(dPa_name::ID_AK_SN_GNDLIGHTARROW01, atInfo.pParticlePos, &atInfo.mpActor->current.angle);
#if VERSION > VERSION_DEMO
                i_this->m142E = 1;
                i_this->mArrowHitFlashTimer = REG8_S(4) + 30;
                for (s32 i = 0; i < ARRAY_SSIZE(i_this->m14A8); i++) {
                ...
                dComIfGp_particle_set(dPa_name::ID_AK_SN_GNDASPECIALHIT00, atInfo.pParticlePos, &actor->shape_angle);
            } else {
                JPABaseEmitter* emitter = dComIfGp_particle_set(dPa_name::ID_AK_SN_GNDHITOK00, atInfo.pParticlePos, &actor->shape_angle);
                if (emitter != NULL) {
                    emitter->setGlobalPrmColor(i_this->mTevStr.mColorC0.r, i_this->mTevStr.mColorC0.g, i_this->mTevStr.mColorC0.b);
                }
            }
            ...
            if (actor->health <= 30) {
                ...
                if (fopAcM_GetName(atInfo.mpActor) == fpcNm_ARROW_e) {
                    ...
                    arrow = (daArrow_c*)atInfo.mpActor;
...
static void* shot_s_sub(void* param_1, void*) {
    ...
    if ((((fopAcM_IsActor(actor)) && (fopAcM_GetName(actor) == fpcNm_HIMO2_e)) || (fopAcM_GetName(actor) == fpcNm_BOOMERANG_e) ||
         (fopAcM_GetName(actor) == fpcNm_HOOKSHOT_e) || (fopAcM_GetName(actor) == fpcNm_ARROW_e)) &&
        (actor->speedF >= 10.0f))
    {
        return actor;
    }
...
static void gnd_move(gnd_class* i_this) {
    ...
    if (((player->getCutType() == daPy_py_c::CUT_TYPE_BT_JUMPCUT) || (player->getCutType() == daPy_py_c::CUT_TYPE_BT_ROLLCUT)) && (i_this->m03D8 < 2)) {
        i_this->m13CE = 0;
        i_this->m03D7 = 0;
    }
    if (i_this->m13CE != 0) {
        s8 r27 = false;
        arrow = (daArrow_c*)fpcM_Search(shot_s_sub, i_this);
        if (arrow != NULL) {
            local_28 = arrow->current.pos - actor->eyePos;
            if (fopAcM_GetName(arrow) == fpcNm_ARROW_e) {
                fVar10 = local_28.abs();
                if (((fVar10 < arrow->speedF * 10.0f) && (arrow->isSetByZelda())) && (i_this->m03D8 < 2)) {
                    uVar5 = actor->shape_angle.y - arrow->current.angle.y;
                    if (uVar5 < 0) {
                        uVar5 = -uVar5;
                    }
                ...
                fVar10 = local_28.abs();
                if (fVar10 < arrow->speedF * 10.0f) {
                    r27 = true;
                }
            }
        }
        dVar9 = fopAcM_searchPlayerDistance(actor);
        if ((r27) || ((player->getCutType() != daPy_py_c::CUT_TYPE_NONE) && (dVar9 < l_HIO.m40))) {
...
static void demo_camera(gnd_class* i_this) {
    ...
    switch (i_this->m155E) {
        ...
        case 0x2:
            {
                ...
                actor->current.angle.y = actor->shape_angle.y;
                if (i_this->mKSubCount > REG0_S(1) + 0x19) {
#if VERSION > VERSION_DEMO
                    i_this->m155C = 0x82;
#endif
                ...
                if (i_this->mKSubCount > REG0_S(2) + 60) {
                    i_this->m155E++;
                    i_this->mKSubCount = 0;
#if VERSION > VERSION_DEMO
                    i_this->m155C = 50;
        ...
        case 0x3:
            if (i_this->mKSubCount == DEMO_SELECT(REG0_S(3) + 10, 10)) {
                i_this->mAnmType++;
            }
        ...
        case 0xB:
            ...
            if (i_this->mKSubCount >= REG0_S(0) + 100) {
                i_this->mAction = ACTION_MOVE0_e;
                i_this->mAnmType = 0;
                actor->current.angle.y = actor->shape_angle.y;
                i_this->m0302[1] = l_HIO.m68;
        ...
        case 0x15:
            ...
            if (i_this->mKSubCount > (s16)(REG12_S(0) + 15)) {
                i_this->m155E++;
                i_this->mKSubCount = 0;
                break;
            }
...
// Fall-through
        ...
        case 0x65:
            ...
            // Fall-through
        case 0x66:
            for (s32 i = 0; i < ARRAY_SSIZE(i_this->m13F0); i++) {
            ...
            if ((((i_this->mKSubCount == DEMO_SELECT(REG0_S(3) + 19, 19)) || (i_this->mKSubCount == REG0_S(4) + 39)) ||
                 (i_this->mKSubCount == REG0_S(5) + 67)) ||
                (i_this->mKSubCount == REG0_S(6) + 69))
            {
                i_this->m1562++;
    ...
    if (i_this->m155E != 0) {
        ...
        i_this->mKSubCount++;
#else
        f32 fx = i_this->m15AC * cM_ssin(i_this->mKSubCount * 0x3500);
        f32 fy = i_this->m15AC * cM_scos(i_this->mKSubCount * 0x3900);
...
#if VERSION > VERSION_DEMO
static s32 eff_d[] = {
    GND_JNT_GND_ATAMA1_e,
    GND_JNT_GND_SUSO_L1_e,
    GND_JNT_GND_SUSO_R1_e,
    GND_JNT_GND_SUSO1_e,
    GND_JNT_GND_MUNE2_e,
    GND_JNT_GND_UDE_L1_e,
    GND_JNT_GND_UDE_L2_e,
    GND_JNT_GND_UDE_L3_e,
    GND_JNT_GND_SODE_L1_2_e,
    GND_JNT_GND_SODE_L2_2_e,
    GND_JNT_GND_UDE_R1_e,
    GND_JNT_GND_UDE_R2_e,
    GND_JNT_GND_UDE_R3_e,
    GND_JNT_GND_SODE_R1_2_e,
    GND_JNT_GND_SODE_R2_2_e
};
...
static void body_flash(gnd_class* i_this) {
    ...
    for (s32 i = 0; i < ARRAY_SSIZE(i_this->mArrowHitEffectTimer); i++) {
...
static BOOL daGnd_Execute(gnd_class* i_this) {
    static s32 w_d[] = {GND_JNT_GND_KENL_e, GND_JNT_GND_KENR_e};
    static s32 w_asi[] = {GND_JNT_GND_ASI_L3_e, GND_JNT_GND_ASI_R3_e};
    static dCcG_At_Spl at_spl_d[] =
        {dCcG_At_Spl_UNK0, dCcG_At_Spl_UNK1, dCcG_At_Spl_UNK2, dCcG_At_Spl_UNK5, dCcG_At_Spl_UNK6, dCcG_At_Spl_UNK7, dCcG_At_Spl_UNKA};
...
#endif
    i_this->m02CC++;
    for (s32 i = 0; i < ARRAY_SSIZE(i_this->m0302); i++) {
        if (i_this->m0302[i] != 0) {
            i_this->m0302[i]--;
        }
    }
    ...
    model = i_this->mpMorf->getModel();
    actor->scale.x = actor->scale.y = actor->scale.z = l_HIO.m0C;
    model->setBaseScale(actor->scale);
    mDoMtx_stack_c::transS(actor->current.pos.x, actor->current.pos.y, actor->current.pos.z);
    ...
    MTXCopy(model->getAnmMtx(GND_JNT_GND_DEKO_1_e), *calc_mtx);
    ...
    MTXCopy(model->getAnmMtx(GND_JNT_GND_MUNE2_e), *calc_mtx);
    ...
    for (s32 i = 0; i < ARRAY_SSIZE(i_this->mSwordSphs); i++) {
        ...
        if ((i_this->m13B8[i] != 0) && ((player->getCutType() != daPy_py_c::CUT_TYPE_BT_JUMPCUT) || (i_this->m03D8 >= 2))) {
            ...
            if (i_this->m13B8[i] == 3) {
                ...
                if (i_this->m13B8[i] == 4) {
                    i_this->mSwordSphs[i].SetAtHitMark(15);
                }
            }
            i_this->mSwordSphs[i].SetR(i_this->m13C4[i]);
            i_this->mSwordSphs[i].SetAtSpl(i_this->m13BC[i]);
...
static BOOL useHeapInit(fopAc_ac_c* a_this) {
    ...
    i_this->mpMorf = new mDoExt_McaMorf(
        ...
        NULL,
        NULL,
        (J3DAnmTransformKey*)dComIfG_getObjectRes("Gnd", dRes_INDEX_GND_BCK_SAMPLE_e),
    ...
    pBrk = (J3DAnmTevRegKey*)dComIfG_getObjectRes("Gnd", dRes_INDEX_GND_BRK_SAMPLE_e);
    ...
    pBtk = (J3DAnmTextureSRTKey*)dComIfG_getObjectRes("Gnd", dRes_INDEX_GND_BTK_SAMPLE_e);
    ...
    pBtp = (J3DAnmTexPattern*)dComIfG_getObjectRes("Gnd", dRes_INDEX_GND_BTP_SAMPLE_e);
...
static cPhs_State daGnd_Create(fopAc_ac_c* a_this) {
    ...
    if (res == cPhs_COMPLEATE_e) {
        pz = NULL;
        i_this->m02B4 = fopAcM_GetParam(a_this) & 0xF;
