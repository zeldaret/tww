...
BOOL daNpc_Co1_c::createInit() {
    ...
    *(u32*)((char*)this + 0x280) = 10;
    *(u8*)((char*)this + 0x26D) = 0x46;
    *(u8*)((char*)this + 0x26F) = 0x45;
    gravity = -4.5f;
...
void daNpc_Co1_c::setMtx(bool param_1) {
    ...
    mSinAngle += l_HIO.mNpc.mPrm34;

    field_0x84C = fabs(cM_ssin(mSinAngle));

    f32 diff = l_HIO.mNpc.mPrm2C - l_HIO.mNpc.mPrm30;
    f32 clamped1 = cLib_minLimit<f32>(diff, 0.0f);

    mFloatVal = cLib_minLimit<f32>(clamped1 * field_0x84C + l_HIO.mNpc.mPrm30, 0.001f);
...
BOOL daNpc_Co1_c::setBtp(bool param_1, int param_2) {
    ...
    int iVar4 = mBtpAnm.init(modelData, mpBtpRes, TRUE, J3DFrameCtrl::EMode_LOOP, 1.0f, 0, -1, param_1, FALSE);
    u32 uVar1 = __cntlzw(1 - iVar4);
    if ((uVar1 >> 5 & 0xFF) != 0) {
        mBtpFrame = 0;
        mBtpTimer = 0;
    }
...
BOOL daNpc_Co1_c::setBtk(bool param_1) {
    ...
    u32 v = __cntlzw(1 - mBtkAnm.init(modelData, mpBtkRes, TRUE, J3DFrameCtrl::EMode_LOOP, 1.0f, 0, -1, param_1));
    if ((v >> 5) & 0xff) {
        mBtkFrame = 0;
    }
    return v >> 5;
...
void daNpc_Co1_c::anmAtr(u16 i_msgStatus) {
    ...
    case fopMsgStts_MSG_TYPING_e: {
        ...
        if (tag != 0xFF && (u8)mCurrAnmTag != tag) {
            mCurrAnmTag = (s8)tag;
            chg_anmTag();
        }
        break;
...
BOOL daNpc_Co1_c::eMove_MDR_() {
    u32 frame = __cntlzw((int)mpMorf->getFrame());
    if ((frame >> 5 & 0xFF) != 0) {
        setAnm_NUM(11, TRUE);
    }
    return frame >> 5;
...
static const f32 l_pad_pool[2] = {999.0f, 888.0f};
...
BOOL daNpc_Co1_c::_draw() {
    ...
    u8 alpha_calc = (u8)(s32)((f32)(0xFF - (u8)l_HIO.mNpc.mAlpha) * field_0x84C);
    ((u8*)&tr)[0x18] = alpha_calc + (u8)l_HIO.mNpc.mAlpha;
