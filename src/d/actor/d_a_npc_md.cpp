    ...
    if (mAcch.ChkGroundHit()) {
        if (mType != 2) {
            if (dComIfG_Bgsp()->GetSpecialCode(mAcch.m_gnd) == 1) {
                ...
                f1 = l_HIO.m008.m08;
                f0 = speedF;
                speedF = (l_HIO.m008.m08 > f0) ? speedF : f1;
            } else {
                speedF = 0.0f;
            }
        }
        ...
        if (!isNoCarryAction()) {
            ...
            if (mActionStatus == ACTION_ONGOING_1) {
                if (iVar3 > 0) {
                    ...
                    f1 = player->speedF;
                    f0 = 0.0f;
                    if ((f0 == f1) && (player->getBaseAnimeFrame() <= mpMorf->getEndFrame())) {
...
s16 daNpc_Md_c::windProc() {
    ...
    if (!(std::fabsf(cStack_70.abs2XZ()) < G_CM3D_F_ABS_MIN)) {
        ...
        if (!(cStack_70.x * cStack_64.x + cStack_70.z * cStack_64.z > 0.7071f)) {
            sVar5 = 0;
            dVar6 = l_HIO.m130;
        }
    }
...
BOOL daNpc_Md_c::escapeNpcAction(void*) {
    ...
    if (mActionStatus == ACTION_STARTING) {
        ...
        if (mActionStatus == ACTION_ONGOING_1) {
            ...
            f32 f30 = 1.0f;
            if (m313F != 0) {
                sVar5 = (m313F < 0) ? DEMO_SELECT(-0x4000, (s16)-0x4000) : DEMO_SELECT(0x4000, (s16)0x4000);
...
void daNpc_Md_c::initialMovePosEvent(int staffIdx) {
    ...
    if (target != NULL) {
        ...
        if (pdVar10 != NULL) {
            ...
            if (!strcmp(acStack_ac, "LINK_DIST")) {
                ...
                if (pDist != NULL) {
                    cXyz pos(current.pos.x + *pDist * cM_ssin(uVar9), current.pos.y + 50.0f, current.pos.z + *pDist * cM_scos(uVar9));
                ...
                if (pAngle != NULL) {
                    pdVar10->setPlayerPosAndAngle(0, cM_deg2s(*pAngle) + (-0x8000 + uVar9));
                }
            } else {
                if (!strcmp(acStack_ac, "DIST")) {
                    ...
                    if (pDist != NULL) {
                        ...
                        current.pos.set(
                            pdVar10->current.pos.x + *pDist * fVar4, pdVar10->current.pos.y, pdVar10->current.pos.z + *pDist * cM_scos(pdVar10->shape_angle.y)
                        );
                    }
                    f32* pAngle = dComIfGp_evmng_getMyFloatP(staffIdx, "Angle");
                    if (pAngle != NULL) {
                        s16 angle = cM_deg2s(*pAngle) + (-0x8000 + pdVar10->shape_angle.y);
                        current.angle.y = angle;
                        shape_angle.y = angle;
                    }
                } else {
                    f32* pAngle = dComIfGp_evmng_getMyFloatP(staffIdx, "Angle");
                    if (pAngle != NULL) {
                        s16 angle = cM_deg2s(*pAngle) + pdVar10->shape_angle.y;
                        current.angle.y = angle;
                        shape_angle.y = angle;
                    }
                    cXyz* pPos = dComIfGp_evmng_getMyXyzP(staffIdx, "Pos");
                    if (pPos != NULL) {
                        current.pos.set(pdVar10->current.pos.x + pPos->x, pdVar10->current.pos.y + pPos->y, pdVar10->current.pos.z + pPos->z);
                    }
                }
            }
        }
    ...
    if (pName != NULL) {
        ...
        if (!strcmp(acStack_ac, *cut_anm_tbl)) {
...
BOOL daNpc_Md_c::initTexPatternAnm(u8 btpAnmTblIdx, bool param_2) {
    ...
    JUT_ASSERT(DEMO_SELECT(7355, 7502), eyeTexPtrn != NULL);
...
BOOL daNpc_Md_c::initLightBtkAnm(bool param_1) {
    ...
    JUT_ASSERT(DEMO_SELECT(7412, 7559), a_lightBtk != NULL);
    ...
    if (m312D == 0x10) {
        if (m0508[1] == NULL) {
            m0508[1] = dComIfGp_particle_set(
                dPa_name::ID_IT_SN_MD_PIYOPIYO00, &current.pos, NULL, NULL, 0xFF, NULL, fopAcM_GetRoomNo(this), &tevStr.mColorK0, &tevStr.mColorK0
            );
            if (m0508[1]) {
    ...
    if (mType != 7) {
        ...
        if ((m3135 & 1) != 0) {
            ...
            dComIfGp_particle_set(
                dPa_name::ID_IT_SN_MD_HANE01, &local_3c, NULL, NULL, 0xFF, NULL, fopAcM_GetRoomNo(this), &tevStr.mColorK0, &tevStr.mColorK0, NULL
            );
...
void daNpc_Md_c::emitterTrace(JPABaseEmitter* emitter, MtxP mtx, csXyz* angle) {
    ...
    if (angle) {
        emitter->setGlobalRotation(angle->x, angle->y, angle->z);
    }
}
...
void daNpc_Md_followEcallBack_c::execute(JPABaseEmitter* emitter) {
    ...
    emitter->setGlobalRotation(mAngle.x, mAngle.y, mAngle.z);
}
