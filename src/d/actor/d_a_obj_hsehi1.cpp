...
cPhs_State daObj_hsh_c::create() {
    ...
    cPhs_State phase;
    if (argument == 0 && VERSION_SELECT(dComIfGs_isTact(2), dComIfGs_isTact(2),
                                        dComIfGs_isEventBit(0x2510),
                                        dComIfGs_isEventBit(0x2510))) {
        return cPhs_ERROR_e;
    } else {
        if (argument == 0) {
...
BOOL daObj_hsh_c::eventProc() {
    ...
    int staffId = dComIfGp_evmng_getMyStaffId("Hsh");
    if (dComIfGp_event_runCheck() && !checkCommandTalk()) {
        ...
        if (mFlags & 1) {
            if (dComIfGp_evmng_endCheck(mEventIdx[mEventCutIdx])) {
                eventEnd();
            }
            return TRUE;
        }
